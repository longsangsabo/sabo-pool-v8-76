...
export const SABOSemifinals: React.FC<SABOSemifinalsProps> = ({
  ...
  const semifinalMatches = matches.filter(m => m.round_number === 250).sort((a, b) => a.match_number - b.match_number);

  const completedMatches = semifinalMatches.filter(match => match.status === 'completed').length;
      ...
      <Card className="border-purple-300 bg-gradient-to-r from-purple-50/50 to-pink-100/50">
        <CardContent className="p-6">
          <p className="text-muted-foreground text-center">
            Semifinals will be available when Winners and Losers Brackets are complete.
          </p>
        </CardContent>
      </Card>
    );
        ...
        <div className="space-y-4">
          ...
          <div className="mt-4 p-3 bg-purple-100 dark:bg-purple-900/30 rounded-lg border border-purple-200">
            <p className="text-xs text-muted-foreground">
              <strong>Semifinal Setup:</strong> The 4 remaining players compete in 2 matches. 
              Winners advance to the Grand Final, losers are eliminated.
            </p>
          </div>
        </div>
      </CardContent>
