...
export const SABOLosersBranchB: React.FC<SABOLosersBranchBProps> = ({
  ...
  const round201 = matches.filter(m => m.round_number === 201).sort((a, b) => a.match_number - b.match_number); // 2 matches
  const round202 = matches.filter(m => m.round_number === 202).sort((a, b) => a.match_number - b.match_number); // 1 match
        ...
        <div className="grid gap-6">
          ...
          <div className="mt-4 p-3 bg-purple-100 dark:bg-purple-900/30 rounded-lg border border-purple-200">
            <p className="text-xs text-muted-foreground">
              <strong>Branch B Path:</strong> Receives losers from Winner's Bracket Round 2. 
