...
export const SABOWinnersBracket: React.FC<SABOWinnersBracketProps> = ({
  ...
  const round1 = matches.filter(m => m.round_number === 1).sort((a, b) => a.match_number - b.match_number); // 8 matches
  const round2 = matches.filter(m => m.round_number === 2).sort((a, b) => a.match_number - b.match_number); // 4 matches  
  const round3 = matches.filter(m => m.round_number === 3).sort((a, b) => a.match_number - b.match_number); // 2 matches
        ...
        <div className="grid gap-6">
          ...
          <div className="mt-4 p-3 bg-yellow-100 dark:bg-yellow-900/30 rounded-lg border border-yellow-200">
            <p className="text-xs text-muted-foreground">
              <strong>Winners Path:</strong> Win every match to reach the Grand Final. 
              One loss sends you to the Losers Bracket for a second chance.
            </p>
          </div>
        </div>
      </CardContent>
