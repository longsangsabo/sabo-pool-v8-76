...
export const SABOLosersBranchA: React.FC<SABOLosersBranchAProps> = ({
  ...
  const round101 = matches.filter(m => m.round_number === 101).sort((a, b) => a.match_number - b.match_number); // 4 matches
  const round102 = matches.filter(m => m.round_number === 102).sort((a, b) => a.match_number - b.match_number); // 2 matches
  const round103 = matches.filter(m => m.round_number === 103).sort((a, b) => a.match_number - b.match_number); // 1 match
        ...
        <div className="grid gap-6">
          ...
          <div className="mt-4 p-3 bg-orange-100 dark:bg-orange-900/30 rounded-lg border border-orange-200">
            <p className="text-xs text-muted-foreground">
              <strong>Branch A Path:</strong> Receives losers from Winner's Bracket Round 1. 
              Winner advances to semifinals. All other participants eliminated.
            </p>
          </div>
        </div>
      </CardContent>
