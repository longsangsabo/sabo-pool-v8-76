...
export const useSABOScoreSubmission = (tournamentId: string, onRefresh?: () => void) => {
  const queryClient = useQueryClient();

  const submitScoreMutation = useMutation({
    mutationFn: async ({ matchId, scores }: { matchId: string; scores: MatchScore }) => {
    ...
    onSuccess: (data, variables) => {
      ...
      if (data && typeof data === 'object' && !Array.isArray(data)) {
        ...
        if (result.success) {
          ...
          toast.error(`❌ ${result.error || result.message || 'Failed to submit score'}`);
        }
      } else {
      ...
      setTimeout(() => {
        ...
        if (onRefresh) {
          console.log('🔄 Calling provided refresh callback with scroll preservation');
          onRefresh();
        } else {
