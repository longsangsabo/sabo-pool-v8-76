...
export const SABODoubleEliminationViewer: React.FC<SABODoubleEliminationViewerProps> = ({
  tournamentId,
  isClubOwner = false,
  adminMode = false,
  isTemplate = false
  ...
  const { data: matches, isLoading, refresh } = useSABOTournamentMatches(tournamentId);
  ...
  const { submitScore } = useSABOScoreSubmission(tournamentId, refreshWithScrollPreservation);
  ...
  const handleScoreSubmit = async (matchId: string, scores: { player1: number; player2: number }) => {
    await submitScore(matchId, scores);
  };

  const getStatusColor = (percentage: number) => {
    if (percentage === 100) return 'bg-green-500/20 text-green-700 border-green-200';
    ...
    <div className="sabo-tournament-container space-y-6">
        ...
        <CardHeader>
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              ...
              <Badge variant="outline" className={getStatusColor(saboProgress.progressPercentage)}>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm">
                {saboProgress.completedMatches}/{saboProgress.totalMatches} matches
              </span>
            </div>
            <div className="flex items-center gap-2">
      ...
      {!saboValidation.valid && (
        <Card className="border-destructive">
          <CardHeader>
            <CardTitle className="text-destructive">Structure Validation Issues</CardTitle>
          </CardHeader>
          <CardContent>
      ...
      <section className="losers-brackets">
        <div className="mb-4">
          ...
          <p className="text-sm text-muted-foreground">
            Two separate branches for players eliminated from different Winner's rounds.
          </p>
        </div>
      ...
      <Card className="border-muted">
        ...
        <CardContent>
          <div className="grid md:grid-cols-3 gap-4 text-sm">
            <div>
              <h4 className="font-semibold mb-2 text-yellow-600">Winners Bracket (14 matches)</h4>
              <p className="text-muted-foreground">
                Rounds 1-3: 8+4+2 matches. Winners advance, losers drop to appropriate Losers Branch.
              </p>
            </div>
            <div>
              <h4 className="font-semibold mb-2 text-orange-600">Losers Brackets (10 matches)</h4>
              <p className="text-muted-foreground">
                Branch A (R1 losers): 4+2+1 matches. Branch B (R2 losers): 2+1 matches.
              </p>
            </div>
            <div>
              <h4 className="font-semibold mb-2 text-purple-600">Finals (3 matches)</h4>
              <p className="text-muted-foreground">
          ...
          <div className="mt-4 p-3 bg-muted rounded-lg">
            <p className="text-xs text-muted-foreground">
              <strong>SABO Structure:</strong> 27 total matches across 5 stages ensuring every player gets adequate chances while maintaining competitive balance.
            </p>
          </div>
        </CardContent>
      </Card>
