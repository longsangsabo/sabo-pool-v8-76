...
export class SABOLogicCore {
   ...
   */
  static getBracketType(round: number): 'winners' | 'losers_a' | 'losers_b' | 'semifinals' | 'final' {
  ...
  static organizeMatches(matches: SABOMatch[]): SABOOrganizedMatches {
    return {
      ...
      losers_branch_a: matches.filter(m => [101, 102, 103].includes(m.round_number)),
   ...
   */
  static getAdvancementTarget(fromRound: number, isWinner: boolean): { round: number | null, position?: 'player1' | 'player2' } {
   ...
   */
  static validateSABOStructure(matches: SABOMatch[]): { valid: boolean; errors: string[] } {
    const errors: string[] = [];
    ...
    if (organized.winners.length !== 14) {
      errors.push(`Winners bracket should have 14 matches, found ${organized.winners.length}`);
    }
    
    // Check Losers Branch A (7 matches, 3 rounds)
    if (organized.losers_branch_a.length !== 7) {
      errors.push(`Losers Branch A should have 7 matches, found ${organized.losers_branch_a.length}`);
    }
    
    // Check Losers Branch B (3 matches, 2 rounds)
    if (organized.losers_branch_b.length !== 3) {
      errors.push(`Losers Branch B should have 3 matches, found ${organized.losers_branch_b.length}`);
    }
    
    // Check Finals (3 matches total: 2 semifinal + 1 final)
    const finalsTotal = organized.semifinals.length + organized.final.length;
    if (finalsTotal !== 3) {
      errors.push(`Finals should have 3 matches (2 semifinal + 1 final), found ${finalsTotal}`);
    }
   ...
   */
  static calculateAdvancementPosition(fromMatchNumber: number, fromRound: number): 'player1' | 'player2' {
  ...
  static getTournamentProgress(matches: SABOMatch[]): {
    ...
    const progressPercentage = Math.round((completedMatches / totalMatches) * 100);
    ...
    if (this.isRoundComplete(organized.winners, 3)) {
      if (this.isRoundComplete(organized.semifinals, 250)) {
        currentStage = 'Final';
      } else if (this.isRoundComplete(organized.losers_branch_a, 103) && 
