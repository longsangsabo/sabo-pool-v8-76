...
export const DatabaseTest = () => {
  ...
  const createDemoUsers = async () => {
    try {
      ...
      const demoRankings = [
        { user_id: '11111111-1111-1111-1111-111111111111', spa_points: 1500, elo_points: 1800, rank: 'Master' },
        { user_id: '22222222-2222-2222-2222-222222222222', spa_points: 800, elo_points: 1400, rank: 'C' },
        { user_id: '33333333-3333-3333-3333-333333333333', spa_points: 200, elo_points: 1100, rank: 'D' }
      ];

      const { error: rankingsError } = await adminClient
        .from('player_rankings')
        .upsert(demoRankings.map(ranking => ({
          ...ranking,
  ...
  return (
    <div className="p-6 bg-white border rounded-lg">
      <h3 className="text-lg font-semibold mb-4">🗄️ Database Connection Test (Service Role)</h3>
          ...
          <div className="space-y-3">
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              <div className="bg-blue-50 p-3 rounded">
                ...
                <p className="text-2xl font-bold text-blue-600">{results.profiles?.count || 0}</p>
                {results.profiles?.error && (
                  <p className="text-xs text-red-600">{results.profiles.error}</p>
                )}
              </div>
              
              <div className="bg-green-50 p-3 rounded">
                <h4 className="font-medium text-green-800">🏆 Tournaments</h4>
                <p className="text-2xl font-bold text-green-600">{results.tournaments?.count || 0}</p>
                {results.tournaments?.error && (
                  <p className="text-xs text-red-600">{results.tournaments.error}</p>
                )}
              </div>
              
              <div className="bg-purple-50 p-3 rounded">
                <h4 className="font-medium text-purple-800">⚔️ Challenges</h4>
                <p className="text-2xl font-bold text-purple-600">{results.challenges?.count || 0}</p>
                {results.challenges?.error && (
                  <p className="text-xs text-red-600">{results.challenges.error}</p>
                )}
              </div>
              
              <div className="bg-yellow-50 p-3 rounded">
                <h4 className="font-medium text-yellow-800">🏅 Rankings</h4>
                <p className="text-2xl font-bold text-yellow-600">{results.rankings?.count || 0}</p>
                {results.rankings?.error && (
                  <p className="text-xs text-red-600">{results.rankings.error}</p>
                )}
              </div>
            </div>
              ...
              <div className="mt-4">
                ...
                <div className="space-y-1">
                  {results.profiles.sample.map((user: any, i: number) => (
                    <div key={i} className="text-sm bg-gray-100 p-2 rounded flex justify-between">
                      <span>{user.full_name || 'No name'}</span>
                      <span className="text-gray-600">{user.email || 'No email'}</span>
                      <span className="text-blue-600">{user.role || 'no role'}</span>
                      <span className={user.is_admin ? 'text-red-600' : 'text-gray-400'}>
