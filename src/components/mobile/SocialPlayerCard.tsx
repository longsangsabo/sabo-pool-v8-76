...
const SocialPlayerCard: React.FC<PlayerCardProps> = ({
  ...
  const getStatusBadge = () => {
    switch (status) {
      case 'online':
        return <Badge className="bg-accent-green text-white text-xs">Online</Badge>;
      case 'in_game':
        return <Badge className="bg-accent-red text-white text-xs animate-pulse">Đang chơi</Badge>;
      default:
        return <Badge className="bg-muted text-muted-foreground text-xs">Offline</Badge>;
    }
  };
    ...
    >
      <div className="social-card-content">
            ...
            <div className="flex items-center space-x-1 mt-1">
              ...
              {eloChange !== undefined && (
                <div className={`flex items-center space-x-1 ml-2 ${
                  eloChange > 0 ? 'text-accent-green' : eloChange < 0 ? 'text-accent-red' : 'text-muted-foreground'
                }`}>
                  {eloChange > 0 ? (
        ...
        {eloChange !== undefined && Math.abs(eloChange) > 50 && (
          <div className={`flex items-center justify-center space-x-2 p-2 rounded-xl mt-2 ${
            eloChange > 50 
