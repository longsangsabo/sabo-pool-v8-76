...
const SocialChallengeCard: React.FC<ChallengeCardProps> = ({
  ...
  const getStatusBadge = () => {
    switch (status) {
      case 'pending':
        return <Badge className="bg-accent-blue text-white animate-pulse">Chờ phản hồi</Badge>;
      case 'accepted':
        return <Badge className="bg-accent-green text-white">Đã chấp nhận</Badge>;
      case 'declined':
        return <Badge className="bg-muted text-muted-foreground">Đã từ chối</Badge>;
      case 'completed':
        return <Badge className="bg-accent-purple text-white">Đã hoàn thành</Badge>;
      case 'expired':
