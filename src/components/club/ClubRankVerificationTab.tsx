...
import { 
  UserCheck, Clock, CheckCircle, XCircle, Eye, 
  AlertCircle, FileText, Camera, Users
} from 'lucide-react';
...
const ClubRankVerificationTab = () => {
  ...
  const handleStatusUpdate = async (requestId: string, newStatus: 'approved' | 'rejected', notes?: string) => {
    try {
      ...
      toast.success(newStatus === 'approved' ? 'Đã phê duyệt yêu cầu' : 'Đã từ chối yêu cầu');
      loadRankRequests();
    } catch (error: any) {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-2xl font-bold text-foreground">Xác thực hạng người chơi</h2>
            ...
            <Card>
              <CardContent className="p-8">
                <div className="text-center text-muted-foreground">
                  ...
                  <p className="font-medium">Không có yêu cầu nào đang chờ duyệt</p>
                ...
                <CardContent className="p-6">
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <h3 className="font-semibold">
                          {request.profile?.display_name || request.profile?.full_name || 'Người dùng'}
                        </h3>
                        <div className="flex items-center gap-2 mt-1">
                          <span className="text-sm text-muted-foreground">Hạng hiện tại:</span>
                          <Badge className={getRankBadgeColor(request.current_rank || 'K')}>
                  ...
                  {request.notes && (
                    <div className="mb-4 p-3 bg-white rounded-lg">
                      <p className="text-sm font-medium mb-1">Ghi chú từ người chơi:</p>
