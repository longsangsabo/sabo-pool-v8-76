...
const ImprovedRankVerificationTab = ({ clubId }: { clubId?: string }) => {
  ...
  const loadRankRequests = async () => {
    try {
      ...
      const requestsWithProfiles = rankData?.map(request => {
  ...
  const getStatusBadge = (status: string) => {
    const statusConfig = {
      ...
      'rejected': { label: 'Đã từ chối', variant: 'destructive' as const, icon: XCircle }
    };
                      ...
                      <div className="flex items-center justify-between">
                        <div className="flex-1">
                          <div className="flex items-center gap-3 mb-2">
                            <h4 className="font-medium">
                              {request.profiles?.full_name || request.profiles?.display_name || 'Unknown User'}
                            </h4>
                            {getStatusBadge(request.status)}
                          </div>
                          
                          <div className="text-sm text-muted-foreground space-y-1">
                            <p>Hạng hiện tại: <span className="font-medium">{request.current_rank || 'Chưa có'}</span></p>
                            <p>Hạng yêu cầu: <span className="font-medium text-primary">{request.requested_rank}</span></p>
                            <p>Ngày gửi: {new Date(request.created_at).toLocaleDateString('vi-VN')}</p>
                        ...
                        <div className="flex gap-2">
                          <Button
                            size="sm"
                            variant="outline"
                            onClick={() => handleReject(request.id, 'Không đủ bằng chứng')}
                          ...
                          <Button
                            ...
                            disabled={processing === request.id}
                          >
                            {processing === request.id ? 'Đang xử lý...' : 'Phê duyệt'}
                          </Button>
                        </div>
                      </div>
                    </CardContent>
          ...
          <TabsContent value="processed" className="space-y-4">
            {processedRequests.length === 0 ? (
              <div className="text-center py-8">
                ...
                <p className="text-muted-foreground">Chưa có yêu cầu nào được xử lý</p>
              </div>
            ) : (
              <div className="space-y-4">
                    ...
                    <CardContent className="p-4">
                      <div className="flex items-center justify-between">
                        <div className="flex-1">
                          ...
                          <div className="text-sm text-muted-foreground space-y-1">
                            <p>Hạng yêu cầu: <span className="font-medium">{request.requested_rank}</span></p>
                            <p>Ngày xử lý: {new Date(request.created_at).toLocaleDateString('vi-VN')}</p>
                            {request.rejection_reason && (
                              <p className="text-red-600">Lý do từ chối: {request.rejection_reason}</p>
                            )}
                          </div>
                        </div>
                      </div>
