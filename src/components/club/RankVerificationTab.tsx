...
import { Trophy, Clock, CheckCircle, XCircle, RefreshCw, AlertCircle } from 'lucide-react';
...
const RankVerificationTab = () => {
  ...
  const loadRankRequests = async () => {
    ...
    try {
      ...
      console.log('Rank verification feature not implemented - using placeholder');
      setRequests([]);
    } catch (error) {
      console.error('Error loading rank requests:', error);
      toast.error('Lỗi khi tải danh sách xác thực hạng: ' + (error as any)?.message);
    } finally {
      setLoading(false);
    }
  };
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      case 'pending':
        return <Badge variant="outline" className="bg-yellow-50 text-yellow-700 border-yellow-200"><Clock className="w-3 h-3 mr-1" />Chờ duyệt</Badge>;
      case 'approved':
      ...
      case 'on_site_test':
        return <Badge variant="outline" className="bg-blue-50 text-blue-700 border-blue-200"><Trophy className="w-3 h-3 mr-1" />Đang test</Badge>;
      default:
                ...
                <div className="flex items-start justify-between">
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground">
                      Muốn xác thực hạng <span className="font-medium text-primary">
