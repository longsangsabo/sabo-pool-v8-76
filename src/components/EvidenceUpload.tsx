  ...
  const handleFiles = async (newFiles: File[]) => {
    ...
    const uploadPromises = newFiles.map(async (file) => {
      ...
      if (!allowedTypes.includes(file.type)) {
        toast.error(`File ${file.name} không được hỗ trợ. Chỉ chấp nhận: JPG, PNG, PDF`);
        return null;
      }
    ...
    const validFiles = uploadedFiles.filter((file): file is EvidenceFile => file !== null);
    ...
    <div className="space-y-4">
      ...
      {files.length > 0 && (
        <div className="space-y-2">
          <h4 className="font-medium text-sm">File đã upload ({files.length})</h4>
          {files.map((file) => (
                    ...
                    <Button
                      ...
                      onClick={() => {
                        ...
                        const imageUrl = file.url.startsWith('http') 
                          ? file.url 
                          : supabase.storage.from('rank-evidence').getPublicUrl(file.url).data.publicUrl;
