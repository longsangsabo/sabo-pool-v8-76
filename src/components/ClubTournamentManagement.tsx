...
import { Plus, Trophy, Calendar, Users, Settings, Eye, ArrowLeft, RefreshCw, Play, Target, Clock, Square, Table } from 'lucide-react';
...
const ClubTournamentManagement = forwardRef<ClubTournamentManagementRef>((props, ref) => {
  const { user } = useAuth();
  ...
  const [currentView, setCurrentView] = useState<'list' | 'details' | 'participants' | 'settings' | 'bracket'>('list');
  ...
  const loadTournaments = async (explicitClubId?: string) => {
    ...
    try {
      ...
      if (cleanTournaments.length === 0) {
        console.log('✅ No tournaments found for club - this is expected if admin deleted all');
      }
  ...
  const statusFilters = {
    active: ['registration_open', 'registration_closed', 'ongoing', 'upcoming'], // Bao gồm cả registration_closed
    ...
    return (
      <div className="space-y-6">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h2 className="text-2xl font-bold text-foreground">{selectedTournament.name}</h2>
          ...
          {filteredTournaments.length === 0 ? (
            <Card>
              <CardContent className="flex flex-col items-center justify-center py-12">
                ...
                <h3 className="text-lg font-semibold mb-2">Chưa có giải đấu sắp diễn ra</h3>
                <p className="text-muted-foreground text-center mb-4">
        ...
        <TabsContent value="completed" className="space-y-4">
          {filteredTournaments.length === 0 ? (
            <Card>
              <CardContent className="flex flex-col items-center justify-center py-12">
                <Trophy className="h-12 w-12 text-muted-foreground mb-4" />
                <h3 className="text-lg font-semibold mb-2">Chưa có giải đấu nào đã kết thúc</h3>
                <p className="text-muted-foreground text-center mb-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              ...
              <div className="space-y-2">
                <div>
                  <span className="text-sm text-muted-foreground">Tên giải:</span>
                ...
                <div>
                  <span className="text-sm text-muted-foreground">Loại giải:</span>
            ...
            <div>
              ...
              <div className="space-y-2">
                 <div>
                   ...
                   <p>{format(new Date(tournament.start_date), 'dd/MM/yyyy HH:mm', { locale: vi })}</p>
                 </div>
                 <div>
                   <span className="text-sm text-muted-foreground">Kết thúc:</span>
                   <p>{format(new Date(tournament.end_date), 'dd/MM/yyyy HH:mm', { locale: vi })}</p>
                 </div>
                 <div>
...
const TournamentParticipantsView = ({ tournament }: { tournament: Tournament }) => {
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex-1">
                      <div className="flex items-center gap-3 mb-2">
                        ...
                        {getRegistrationStatusBadge(registration.registration_status)}
                        {getPaymentStatusBadge(registration.payment_status)}
                      </div>
                      
                      <div className="text-sm text-muted-foreground space-y-1">
                        <p>📞 {registration.profiles?.phone || 'Chưa có SĐT'}</p>
                        <p>✉️ {registration.profiles?.email || 'Chưa có email'}</p>
                        <p>📅 Đăng ký: {new Date(registration.created_at).toLocaleDateString('vi-VN')}</p>
                        {registration.confirmed_at && (
                          <p>✅ Xác nhận: {new Date(registration.confirmed_at).toLocaleDateString('vi-VN')}</p>
                        )}
                        {registration.notes && (
...
const TournamentSettingsView = ({ tournament }: { tournament: Tournament }) => {
  const navigate = useNavigate();
  const { validateTournament, generateBracket, isGenerating, isValidating } = useBracketGeneration();
  ...
  const handleCloseTournament = async () => {
    try {
      ...
      if (pendingMatches && pendingMatches.length > 0) {
        toast.error(`Không thể đóng giải đấu! Còn ${pendingMatches.length} trận đấu chưa hoàn thành.`);
        return;
      }
      ...
      toast.success('🏆 Giải đấu đã được đóng thành công! Đã trao giải thưởng và thông báo tới tất cả thành viên.');
  ...
  const renderTournamentActions = () => {
    const canStart = tournament.status === 'registration_open' && 
                    tournament.max_participants >= 2;
    
    const isOngoing = tournament.status === 'ongoing' || tournament.status === 'registration_closed';
            ...
            <CardContent>
              <div className="space-y-4">
                ...
                <div className="bg-white rounded p-4 border">
                  <h4 className="font-medium mb-2">Quy trình bắt đầu giải đấu:</h4>
              ...
              <div className="space-y-4">
                ...
                <div className="bg-yellow-50 border border-yellow-200 rounded p-3">
                  <p className="text-sm text-yellow-800">
                    ⚠️ <strong>Lưu ý:</strong> Sau khi đóng giải đấu, bạn không thể thay đổi kết quả nữa.
                  </p>
                </div>
            ...
            <div className="space-y-4">
              <div className="border rounded-lg p-4">
                ...
                <div className="grid grid-cols-2 gap-4 text-sm">
                  ...
                  <div>
                    ...
                    <p className="font-medium">0/{tournament.max_participants}</p>
                  </div>
                  <div>
                  ...
                  <div>
                    ...
                    <p className="font-medium">{tournament.entry_fee.toLocaleString('vi-VN')} VNĐ</p>
                  </div>
                </div>
              </div>
              ...
              {!canStart && tournament.status === 'registration_open' && (
                <div className="border rounded-lg p-4 bg-yellow-50 border-yellow-200">
                  <h4 className="font-medium mb-2 text-yellow-800">Chờ đủ người tham gia</h4>
                  <p className="text-sm text-yellow-700">
                    Cần thêm {tournament.max_participants} người để có thể bắt đầu giải đấu
                  </p>
                </div>
              )}
            </div>
...
const TournamentCard = ({ tournament, onViewDetails, onManageParticipants, onTournamentSettings, onViewBracket, onUpdate }: TournamentCardProps) => {
    ...
    <Card>
      ...
      <CardContent>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-4">
          ...
          <div>
           ...
           <div>
             ...
             <p className="text-sm text-muted-foreground">
               {format(new Date(tournament.start_date), 'dd/MM/yyyy HH:mm', { locale: vi })}
             </p>
           </div>
        </div>
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          {(tournament.status === 'registration_closed' || tournament.status === 'ongoing' || tournament.status === 'completed') && onViewBracket && (
            <Button 
              variant="outline" 
              size="sm"
          ...
          <ForceStartTournamentButton
            ...
            onStatusChanged={onUpdate ? async () => {
