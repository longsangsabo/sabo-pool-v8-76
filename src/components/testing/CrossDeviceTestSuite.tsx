...
export const CrossDeviceTestSuite: React.FC = () => {
  ...
  const runBreakpointTests = async () => {
    ...
    const expectedBreakpoint = responsive.width < BREAKPOINTS.mobile ? 'mobile' : 
                              responsive.width < BREAKPOINTS.tablet ? 'tablet' : 'desktop';
    ...
    const loadTime = performanceEntries.length > 0 ? performanceEntries[0].loadEventEnd : 0;
    ...
    try {
      ...
      setTimeout(() => {
        ...
        results.push({
          ...
          status: cumulativeLayoutShift < 0.1 ? 'pass' : cumulativeLayoutShift < 0.25 ? 'warning' : 'fail',
      ...
      <CardContent className="space-y-4">
        ...
        <div className="grid grid-cols-3 gap-4 p-4 bg-muted/50 rounded-lg">
          ...
          <div>
            ...
            <div>{responsive.width} × {responsive.height}</div>
          </div>
          <div>
            <div className="text-sm font-medium">Layout Shift</div>
            <div className={layoutShiftScore < 0.1 ? 'text-green-600' : layoutShiftScore < 0.25 ? 'text-yellow-600' : 'text-red-600'}>
        ...
        <Card className="bg-blue-50">
          ...
          <CardContent>
            <div className="space-y-2 text-sm">
              <div>1. 📱 <strong>Mobile Test:</strong> Resize window to &lt; 768px width</div>
              <div>2. 📋 <strong>Tablet Test:</strong> Resize window to 768px - 1023px width</div>
              <div>3. 🖥️ <strong>Desktop Test:</strong> Resize window to ≥ 1024px width</div>
              <div>4. 🔄 <strong>Transition Test:</strong> Gradually resize and watch for layout shifts</div>
              <div>5. ⚡ <strong>Performance Test:</strong> Monitor CLS scores during transitions</div>
            </div>
          </CardContent>
        </Card>
      </CardContent>
