      ...
      const newMetrics: PerformanceMetric[] = [
        {
          ...
          value: renderTime,
          unit: 'ms',
          status: renderTime < thresholds.renderTime ? 'good' : renderTime < 33 ? 'warning' : 'error',
        ...
        },
        {
          ...
          unit: 'MB',
          status: memory ? 
            (memory.usedJSHeapSize < thresholds.memoryUsage ? 'good' : 
             memory.usedJSHeapSize < thresholds.memoryUsage * 2 ? 'warning' : 'error') : 'good',
        ...
        },
        {
          ...
          value: renderCount,
          unit: 'renders',
          status: renderCount < 10 ? 'good' : renderCount < 50 ? 'warning' : 'error',
          threshold: 10
        }
      ];
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
            ...
            <div className="space-y-2">
              ...
              <div className="grid grid-cols-3 gap-4 text-sm">
                <div>
                  ...
                  <div className="font-mono">{formatBytes(memoryInfo.usedJSHeapSize)} MB</div>
                </div>
                <div>
                  <label>Total Heap</label>
                  <div className="font-mono">{formatBytes(memoryInfo.totalJSHeapSize)} MB</div>
                </div>
                <div>
                  <label>Heap Limit</label>
                  <div className="font-mono">{formatBytes(memoryInfo.jsHeapSizeLimit)} MB</div>
                </div>
              </div>
            </div>
          )}
          ...
          <div className="space-y-2">
            ...
            <div className="flex items-center gap-2">
              <span className="text-lg font-bold">{layoutShift.toFixed(4)}</span>
              <Badge variant={layoutShift < 0.1 ? 'default' : layoutShift < 0.25 ? 'secondary' : 'destructive'}>
                CLS Score
              </Badge>
            </div>
            <div className="text-xs text-muted-foreground">
          ...
          {metrics.some(m => m.status === 'error') && (
            <Alert variant="destructive">
              <AlertDescription>
                Performance issues detected! Check render times and memory usage.
              </AlertDescription>
            </Alert>
          )}
