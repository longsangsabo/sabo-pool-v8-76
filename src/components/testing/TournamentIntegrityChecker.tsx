  ...
  const runIntegrityCheck = async (tournamentId?: string) => {
    ...
    try {
      ...
      const { data: duplicateMatches } = await supabase
        ...
        .eq('tournament_id', tournamentId || 'd528882d-bf18-4db7-b4d6-7b9f80cc7939')
      ...
      if (allMatches) {
        ...
        Object.entries(roundGroups).forEach(([roundKey, matches]) => {
          ...
          if (completedMatches.length === matches.length && completedMatches.length > 0) {
            ...
            const emptyNextMatches = nextRoundMatches.filter(m => !m.player1_id || !m.player2_id);
      ...
      if (tournament) {
        ...
        if (finalMatch?.status === 'completed' && finalMatch.winner_id && tournament.status !== 'completed') {
          checkResults.push({
            type: 'warning',
            category: 'Tournament Status',
            description: 'Final match completed but tournament status not updated',
        ...
        <div className="space-y-3">
          {issues.length === 0 && !isRunning && (
            <Alert>
              ...
              <AlertDescription>
                {lastCheck ? 'No integrity issues found!' : 'Click "Run Check" to validate tournament data'}
              </AlertDescription>
            </Alert>
          )}
                  ...
                  <AlertDescription className="mt-1">
                    ...
                    {issue.affected_matches && (
                      <div className="mt-1 text-xs text-muted-foreground">
                        Affected matches: {issue.affected_matches.slice(0, 3).join(', ')}
                        {issue.affected_matches.length > 3 && ` (+${issue.affected_matches.length - 3} more)`}
                      </div>
                    )}
                  </AlertDescription>
                </div>
