...
import { CheckCircle, XCircle, Play, Square, Navigation, TouchpadIcon, Eye } from 'lucide-react';
...
export const UserExperienceValidator: React.FC = () => {
  ...
  const runUXTests = useCallback(async () => {
    ...
    const hasSwipeableElements = document.querySelectorAll('[data-swipeable]').length > 0;
    ...
    results.push({
      ...
      status: images.length === 0 ? 'pass' : responsiveImages > 0 ? 'pass' : 'manual',
        ...
        <Card className="bg-muted/50">
          <CardContent className="pt-4">
            <div className="grid grid-cols-2 gap-4 text-sm">
              ...
              <div>
                <strong>Test Context:</strong> {responsive.isMobile ? 'Mobile UX' : responsive.isTablet ? 'Tablet UX' : 'Desktop UX'}
              </div>
            </div>
          </CardContent>
        </Card>
        ...
        <Card className="bg-yellow-50">
          ...
          <CardContent>
            <div className="space-y-2 text-sm">
              <div>⌨️ <strong>Keyboard Navigation:</strong> Tab through all interactive elements</div>
              <div>📱 <strong>Touch Gestures:</strong> Test swipe, pinch, and tap interactions</div>
              <div>🔄 <strong>Loading States:</strong> Verify spinners and skeleton screens</div>
              <div>❌ <strong>Error Handling:</strong> Test error states and recovery flows</div>
              <div>🎨 <strong>Visual Consistency:</strong> Check alignment and spacing across breakpoints</div>
              <div>♿ <strong>Screen Reader:</strong> Test with assistive technologies</div>
            </div>
          </CardContent>
        </Card>
