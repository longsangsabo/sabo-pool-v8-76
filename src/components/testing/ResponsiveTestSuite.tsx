      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          ...
          <div className="space-y-2">
            ...
            <div className="grid grid-cols-3 gap-2">
              <Badge variant={checkMediaQuery(MEDIA_QUERIES.mobile) ? 'default' : 'outline'}>
                Mobile Query: {checkMediaQuery(MEDIA_QUERIES.mobile) ? '✅' : '❌'}
              </Badge>
              <Badge variant={checkMediaQuery(MEDIA_QUERIES.tablet) ? 'default' : 'outline'}>
                Tablet Query: {checkMediaQuery(MEDIA_QUERIES.tablet) ? '✅' : '❌'}
              </Badge>
              <Badge variant={checkMediaQuery(MEDIA_QUERIES.desktop) ? 'default' : 'outline'}>
                Desktop Query: {checkMediaQuery(MEDIA_QUERIES.desktop) ? '✅' : '❌'}
              </Badge>
            </div>
          </div>
          ...
          <div className="space-y-2">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <div className="font-mono">{performanceMetrics.renderTime.toFixed(2)}ms</div>
              </div>
              <div>
                <label className="text-sm">Last Update</label>
                <div className="font-mono">{new Date(performanceMetrics.lastUpdate).toLocaleTimeString()}</div>
              </div>
            </div>
          </div>
          ...
          <div className="space-y-2">
            ...
            <div className="grid grid-cols-2 gap-2 text-sm font-mono">
              ...
              <div>Tablet: {BREAKPOINTS.mobile}px - {BREAKPOINTS.tablet - 1}px</div>
