...
import { RANK_ELO, SPA_TOURNAMENT_REWARDS, type RankCode } from '@/utils/eloConstants';
import { getRankByElo, isEligibleForPromotion, calculateRankProgress } from '@/utils/rankUtils';
...
export const RankSystemValidator: React.FC = () => {
  ...
  const runValidation = async () => {
    ...
    const ranks: RankCode[] = ['K', 'K+', 'I', 'I+', 'H', 'H+', 'G', 'G+', 'F', 'F+', 'E', 'E+'];
    ...
    testResults.push({
      ...
      message: eloProgression ? 'ELO values increase properly' : 'ELO progression has issues'
    });
    ...
    testResults.push({
      ...
      status: getRankByElo(1000) === 'K' && getRankByElo(2100) === 'E+' ? 'pass' : 'fail',
    ...
    testResults.push({
      ...
      message: promotionTest ? 'Promotion logic working' : 'Promotion logic failed'
    });
      ...
      <CardContent>
        {results.length > 0 && (
          <>
            ...
            <div className="mt-6 p-4 bg-gray-50 rounded">
              <h3 className="font-medium mb-3">Current Rank System (12 Tiers)</h3>
              <div className="grid grid-cols-6 gap-2">
                {Object.entries(RANK_ELO).map(([rank, elo]) => (
                  <div key={rank} className="text-center p-2 bg-white rounded border">
        ...
        {isRunning && (
          <div className="text-center py-8">
            ...
            <p className="mt-2 text-muted-foreground">Running validation tests...</p>
          </div>
        )}
      </CardContent>
    </Card>
