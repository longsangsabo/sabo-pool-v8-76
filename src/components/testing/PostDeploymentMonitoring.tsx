...
export const PostDeploymentMonitoring: React.FC = () => {
  ...
  const [feedbackCategory, setFeedbackCategory] = useState<FeedbackData['category']>('usability');
  const [sentiment, setSentiment] = useState<UserSentiment>({ positive: 0, neutral: 0, negative: 0 });
    ...
    const sampleFeedback: FeedbackData[] = [
      {
        ...
        rating: 5,
        category: 'usability',
        message: 'The new responsive design is amazing! Much better touch targets on mobile.',
      ...
      },
      {
        ...
        rating: 4,
        category: 'design',
        message: 'Love the tablet optimizations. The spacing feels much more natural.',
  ...
  const submitFeedback = async () => {
    ...
    const positive = [...feedbackList, newFeedback].filter(f => f.rating >= 4).length;
    const negative = [...feedbackList, newFeedback].filter(f => f.rating <= 2).length;
    const neutral = total - positive - negative;
      ...
      <CardContent className="space-y-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <div className="text-center p-3 bg-green-50 rounded-lg">
            <div className="text-2xl font-bold text-green-600">{sentiment.positive.toFixed(0)}%</div>
          ...
          <div className="text-center p-3 bg-gray-50 rounded-lg">
            <div className="text-2xl font-bold text-gray-600">{sentiment.neutral.toFixed(0)}%</div>
            <div className="text-sm text-gray-600">Neutral</div>
          </div>
          
          <div className="text-center p-3 bg-red-50 rounded-lg">
            <div className="text-2xl font-bold text-red-600">{sentiment.negative.toFixed(0)}%</div>
        ...
        <Card className="bg-muted/50">
          <CardHeader>
            <CardTitle className="text-lg">User Sentiment Distribution</CardTitle>
          </CardHeader>
          <CardContent className="space-y-3">
            <div className="space-y-2">
              ...
              <Progress value={sentiment.positive} className="h-2 bg-green-100" />
            </div>
          ...
          <CardContent className="space-y-4">
            {hasSubmitted ? (
              <Alert>
                ...
                <AlertDescription>
                  Thank you for your feedback! Your input helps us improve the responsive experience.
                </AlertDescription>
              </Alert>
            ) : (
              <>
                <div className="space-y-2">
                  <label className="text-sm font-medium">How would you rate the new responsive design?</label>
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex gap-2 flex-wrap">
                    {(['performance', 'usability', 'design', 'bugs'] as const).map((category) => (
                      <Button
                        key={category}
          ...
          <CardContent>
            <div className="space-y-3 text-sm">
              <div className="flex items-center gap-2">
                ...
                <span><strong>Performance:</strong> 95% of users report improved load times</span>
              </div>
              <div className="flex items-center gap-2">
                <Users className="h-4 w-4 text-green-600" />
                <span><strong>Usability:</strong> Mobile touch targets receive positive feedback</span>
              </div>
              <div className="flex items-center gap-2">
                <BarChart3 className="h-4 w-4 text-purple-600" />
                <span><strong>Engagement:</strong> 12% increase in time spent on site</span>
              </div>
              <div className="flex items-center gap-2">
                <MessageSquare className="h-4 w-4 text-orange-600" />
                <span><strong>Feedback:</strong> 88% satisfaction rate with new responsive design</span>
              </div>
            </div>
          </CardContent>
        </Card>
