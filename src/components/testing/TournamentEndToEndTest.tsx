...
export const TournamentEndToEndTest: React.FC = () => {
  ...
  const [overallResult, setOverallResult] = useState<'success' | 'failure' | null>(null);

  const updateStepStatus = (stepIndex: number, status: TestStep['status'], result?: any, error?: string) => {
    ...
    try {
      ...
      const { data: testMatches, error: matchError } = await supabase
        ...
        .insert([
          ...
          { tournament_id: tournament.id, bracket_type: 'winners', round_number: 1, match_number: 1, player1_id: 'p1', player2_id: 'p2', status: 'scheduled' },
          { tournament_id: tournament.id, bracket_type: 'winners', round_number: 1, match_number: 2, player1_id: 'p3', player2_id: 'p4', status: 'scheduled' },
          { tournament_id: tournament.id, bracket_type: 'winners', round_number: 1, match_number: 3, player1_id: 'p5', player2_id: 'p6', status: 'scheduled' },
          { tournament_id: tournament.id, bracket_type: 'winners', round_number: 1, match_number: 4, player1_id: 'p7', player2_id: 'p8', status: 'scheduled' },
        ]);
      ...
      const steps = [
        ...
        { stepIndex: 2, description: 'WB Round 1', matches: [1, 2, 3, 4], round: 1, bracket: 'winners' },
        { stepIndex: 3, description: 'LB Round 1', matches: [1, 2], round: 101, bracket: 'losers' },
        { stepIndex: 4, description: 'WB Round 2', matches: [1, 2], round: 2, bracket: 'winners' },
        { stepIndex: 5, description: 'LB Round 2', matches: [1, 2], round: 201, bracket: 'losers' },
        { stepIndex: 6, description: 'WB Round 3', matches: [1], round: 3, bracket: 'winners' },
        { stepIndex: 7, description: 'LB Round 3', matches: [1, 2], round: 102, bracket: 'losers' },
        { stepIndex: 8, description: 'Semifinals', matches: [1, 2], round: 250, bracket: 'semifinals' },
        { stepIndex: 9, description: 'Grand Final', matches: [1], round: 300, bracket: 'finals' },
      ];
      ...
      for (const step of steps) {
        ...
        for (const match of matches.slice(0, step.matches.length)) {
          ...
          if (updateError) throw new Error(`Failed to update match ${match.id}: ${updateError.message}`);

          // Run advancement with new simplified logic
          const { data: advancementResult } = await supabase.rpc('advance_winner_simplified', {
            p_match_id: match.id
          });

          if (advancementResult && typeof advancementResult === 'object' && 'success' in advancementResult && !advancementResult.success) {
            console.warn(`Advancement warning for match ${match.id}:`, advancementResult);
          }
        }

        updateStepStatus(step.stepIndex, 'completed', { matches_completed: matches.length });
      }
      ...
      const currentStep = testSteps.findIndex(step => step.status === 'running');
      if (currentStep !== -1) {
        updateStepStatus(currentStep, 'failed', undefined, error instanceof Error ? error.message : 'Unknown error');
      }
      ...
      <CardContent className="space-y-4">
        ...
        <div className="space-y-2">
          <div className="flex justify-between text-sm">
            <span>Progress</span>
            <span>{completedSteps}/{testSteps.length} steps</span>
          </div>
        ...
        {/* Overall Result */}
        {overallResult && (
          <Alert className={overallResult === 'success' ? 'border-green-200' : 'border-red-200'}>
        ...
        <div className="space-y-2">
          {testSteps.map((step, index) => (
            <div key={index} className="flex items-center justify-between p-3 border rounded">
              ...
              {step.error && (
                <span className="text-sm text-red-600 max-w-md truncate">{step.error}</span>
              )}
        ...
        <Alert>
          <AlertDescription>
            This test creates a complete 8-player double elimination tournament and simulates the entire workflow
            from bracket generation to champion determination. It validates data integrity at each step.
          </AlertDescription>
        </Alert>
      </CardContent>
    </Card>
