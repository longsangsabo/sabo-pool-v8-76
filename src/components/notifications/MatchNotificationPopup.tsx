...
export const MatchNotificationPopup: React.FC<MatchNotificationProps> = ({
  ...
  const getStatusBadge = () => {
    switch (match.status) {
      case 'final':
        return <Badge className="bg-gradient-to-r from-yellow-500 to-amber-500 text-black font-bold">CHUNG KẾT</Badge>;
      case 'semifinal':
        return <Badge className="bg-gradient-to-r from-purple-500 to-pink-500 text-white font-bold">BÁN KẾT</Badge>;
      case 'scheduled':
        return <Badge className="bg-gradient-to-r from-blue-500 to-cyan-500 text-white font-bold">ĐÃ LỊCH HẸN</Badge>;
      default:
        return <Badge className="bg-gradient-to-r from-green-500 to-emerald-500 text-white font-bold">ĐÃ BẮT ĐẦU</Badge>;
    }
  };
      ...
      >
        ...
        <div className="relative bg-gradient-to-r from-yellow-500/20 via-amber-500/20 to-orange-500/20 p-5 text-center border-b border-yellow-500/30">
          ...
          <div className="flex items-center justify-center gap-2">
            ...
            <span className="text-yellow-100 font-medium text-lg">{match.tournament_name}</span>
          </div>
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex flex-col items-center space-y-2 flex-1">
              <div className="relative">
                <Avatar className="w-12 h-12 border-2 border-blue-500 shadow-lg shadow-blue-500/30">
                  {currentPlayer.avatar_url ? (
                    <img src={currentPlayer.avatar_url} alt={currentPlayer.name} className="w-full h-full object-cover rounded-full" />
                  ) : (
              ...
              <div className="text-center">
                ...
                {currentPlayer.rank && (
                  <p className="text-blue-400 text-xs font-medium">Rank: {currentPlayer.rank}</p>
                )}
                {currentPlayer.elo && (
                  <p className="text-slate-400 text-xs">ELO: {currentPlayer.elo}</p>
                )}
              </div>
            </div>
            ...
            <div className="flex flex-col items-center mx-4">
              ...
              {match.head_to_head && (
                <div className="text-xs text-slate-400 mt-1">
                  {match.head_to_head.player1_wins} - {match.head_to_head.player2_wins}
                </div>
              )}
            </div>
            ...
            <div className="flex flex-col items-center space-y-2 flex-1">
              <div className="relative">
                <Avatar className="w-12 h-12 border-2 border-slate-500 shadow-lg">
                  {opponent.avatar_url ? (
                    <img src={opponent.avatar_url} alt={opponent.name} className="w-full h-full object-cover rounded-full" />
                  ) : (
              ...
              <div className="text-center">
                ...
                {opponent.rank && (
                  <p className="text-slate-400 text-xs font-medium">Rank: {opponent.rank}</p>
                )}
                {opponent.elo && (
          ...
          <div className="bg-slate-800/50 rounded-lg p-3 space-y-2">
            {match.start_time && (
              <div className="flex items-center gap-2 text-slate-300 text-sm">
                ...
                <span>Bắt đầu: {new Date(match.start_time).toLocaleTimeString('vi-VN')}</span>
              </div>
            )}
