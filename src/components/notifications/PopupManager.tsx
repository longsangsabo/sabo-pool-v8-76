...
export const PopupManager: React.FC = () => {
  ...
  const { currentMatchNotification, closeNotification } = useRealtimeMatchNotifications();
  ...
  if (currentMatchNotification && user?.id) {
    ...
    const matchData = {
      ...
      tournament_name: currentMatchNotification.tournament?.name || 'Tournament',
      round: currentMatchNotification.match.round_name || `Round ${currentMatchNotification.match.round_number || 1}`,
      player1: {
        id: player1.id,
        name: player1.full_name || player1.display_name || player1.name || 'Player 1',
      ...
      player2: {
        id: player2.id,
        name: player2.full_name || player2.display_name || player2.name || 'Player 2',
      ...
      location: currentMatchNotification.table ? `Bàn ${currentMatchNotification.table.table_number}` : undefined,
