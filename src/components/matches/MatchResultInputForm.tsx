...
import { CalendarIcon, Trophy, Users, MapPin, Clock, AlertCircle, CheckCircle, Target } from 'lucide-react';
...
const matchResultSchema = z.object({
  ...
  player1_score: z.number().min(0, 'Điểm số không hợp lệ').max(50, 'Điểm số quá lớn'),
  player2_score: z.number().min(0, 'Điểm số không hợp lệ').max(50, 'Điểm số quá lớn'),
  match_format: z.enum(['race_to_5', 'race_to_7', 'race_to_9', 'race_to_10', 'race_to_11']),
...
export const MatchResultInputForm: React.FC<MatchResultInputFormProps> = ({
  ...
  const selectedPlayer1 = players.find(p => p.user_id === watchedValues.player1_id);
  const selectedPlayer2 = players.find(p => p.user_id === watchedValues.player2_id);
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
            ...
            <div className="space-y-2">
              ...
              <Select 
                ...
                onValueChange={(value) => setValue('tournament_id', value || undefined)}
              >
                <SelectTrigger>
                  ...
                  {tournaments.map((tournament) => (
                    <SelectItem key={tournament.id} value={tournament.id}>
                      ...
                      <Badge variant="outline" className="ml-2">
                        {tournament.status === 'ongoing' ? 'Đang diễn ra' : 'Mở đăng ký'}
                      </Badge>
                    </SelectItem>
                  ))}
                </SelectContent>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="space-y-3">
                ...
                <div className="space-y-2">
                  ...
                  {searchPlayer1 && (
                    <div className="border rounded-lg max-h-40 overflow-y-auto">
                      {getFilteredPlayers(searchPlayer1, watchedValues.player2_id).slice(0, 5).map((player) => (
                        <div
                          key={player.user_id}
                        ...
                        >
                          ...
                          <div>
                            ...
                            <div className="text-sm text-muted-foreground">
                              {player.verified_rank} • ELO: {player.elo || 1000}
                            </div>
                          </div>
                        </div>
                      ))}
                    ...
                    <div className="p-3 border rounded-lg bg-muted/50">
                      <div className="flex items-center gap-3">
                        <Avatar>
                          ...
                          <AvatarFallback>{selectedPlayer1.display_name[0]}</AvatarFallback>
                        </Avatar>
                        <div>
                          <div className="font-medium">{selectedPlayer1.display_name}</div>
                          <div className="text-sm text-muted-foreground">
                            Hạng: {selectedPlayer1.verified_rank} • ELO: {selectedPlayer1.elo || 1000}
                          </div>
                        </div>
                      </div>
                    </div>
                ...
                {errors.player1_id && (
                  <p className="text-sm text-destructive">{errors.player1_id.message}</p>
                )}
              </div>
              ...
              <div className="space-y-3">
                ...
                <div className="space-y-2">
                  ...
                  {searchPlayer2 && (
                    <div className="border rounded-lg max-h-40 overflow-y-auto">
                      {getFilteredPlayers(searchPlayer2, watchedValues.player1_id).slice(0, 5).map((player) => (
                        <div
                          key={player.user_id}
                          className="p-2 hover:bg-muted cursor-pointer flex items-center gap-2"
                          onClick={() => {
                    ...
                    <div className="p-3 border rounded-lg bg-muted/50">
                      <div className="flex items-center gap-3">
                        <Avatar>
                          ...
                          <AvatarFallback>{selectedPlayer2.display_name[0]}</AvatarFallback>
                        </Avatar>
                        <div>
                          <div className="font-medium">{selectedPlayer2.display_name}</div>
                          <div className="text-sm text-muted-foreground">
                            Hạng: {selectedPlayer2.verified_rank} • ELO: {selectedPlayer2.elo || 1000}
                          </div>
                        </div>
                      </div>
                    </div>
                ...
                {errors.player2_id && (
                  <p className="text-sm text-destructive">{errors.player2_id.message}</p>
                )}
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="space-y-2">
                ...
                <Select
                  ...
                  onValueChange={(value) => setValue('match_format', value as any)}
                >
                  <SelectTrigger>
                    <SelectValue />
                  </SelectTrigger>
              ...
              <div className="grid grid-cols-3 gap-4 items-center">
                <div className="text-center">
                  ...
                  {errors.player1_score && (
                    <p className="text-sm text-destructive mt-1">{errors.player1_score.message}</p>
                  )}
                </div>
                ...
                <div className="text-center">
                  ...
                  {errors.player2_score && (
                    <p className="text-sm text-destructive mt-1">{errors.player2_score.message}</p>
                  )}
                </div>
              </div>
              ...
              {winnerInfo.winner && (
                <div className="text-center p-4 bg-primary/10 border border-primary/20 rounded-lg">
                  <div className="flex items-center justify-center gap-2 mb-2">
                    ...
                    <span className="font-semibold text-primary">Người thắng cuộc</span>
                  </div>
                  <div className="flex items-center justify-center gap-3">
                    <Avatar>
                      <AvatarImage src={winnerInfo.winner.avatar_url} />
                      <AvatarFallback>{winnerInfo.winner.display_name[0]}</AvatarFallback>
                    </Avatar>
                    <div>
                      <div className="font-bold">{winnerInfo.winner.display_name}</div>
                      <div className="text-sm text-muted-foreground">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="space-y-2">
                ...
                <Select
                  ...
                  onValueChange={(value) => setValue('club_id', value || undefined)}
                >
                  <SelectTrigger>
            ...
            <div className="space-y-4">
              ...
              >
                ...
                {showAdvancedStats ? 'Ẩn thống kê chi tiết' : 'Thêm thống kê chi tiết'}
              </Button>
                ...
                <div className="space-y-6 p-4 border rounded-lg bg-muted/20">
                  ...
                  <div className="space-y-4">
                    <h4 className="font-medium">
                      Thống kê của {selectedPlayer1?.display_name || 'Người chơi 1'}
                    </h4>
                    <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                      <div className="space-y-2">
                        ...
                        <Input
                          type="number"
                          min="0"
                          {...register('player1_longest_run', { valueAsNumber: true })}
                        />
                      </div>
                      <div className="space-y-2">
                        ...
                        <Input
                          type="number"
                          min="0"
                          {...register('player1_total_shots', { valueAsNumber: true })}
                        />
                      </div>
                      <div className="space-y-2">
                        ...
                        <Input
                          ...
                          min="0"
                          max="100"
                          {...register('player1_potting_percentage', { valueAsNumber: true })}
                        />
                      </div>
                      <div className="space-y-2">
                        ...
                        <Input
                          type="number"
                          min="0"
                          {...register('player1_safety_shots', { valueAsNumber: true })}
                        />
                      </div>
                      <div className="space-y-2">
                        ...
                        <Input
                          type="number"
                          min="0"
                          {...register('player1_fouls', { valueAsNumber: true })}
                        />
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="space-y-4">
                    <h4 className="font-medium">
                      Thống kê của {selectedPlayer2?.display_name || 'Người chơi 2'}
                    </h4>
                    <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                      <div className="space-y-2">
                        <Label className="text-sm">Run dài nhất</Label>
                        <Input
                          type="number"
                          min="0"
                          {...register('player2_longest_run', { valueAsNumber: true })}
                        />
                      </div>
                      <div className="space-y-2">
                        <Label className="text-sm">Tổng số shot</Label>
                        <Input
                          type="number"
                          min="0"
                          {...register('player2_total_shots', { valueAsNumber: true })}
                        />
                      </div>
                      <div className="space-y-2">
                        <Label className="text-sm">% Pot thành công</Label>
                        <Input
                          ...
                          min="0"
                          max="100"
                          {...register('player2_potting_percentage', { valueAsNumber: true })}
                        />
                      </div>
                      <div className="space-y-2">
                        <Label className="text-sm">Shot an toàn</Label>
                        <Input
                          type="number"
                          min="0"
                          {...register('player2_safety_shots', { valueAsNumber: true })}
                        />
                      </div>
                      <div className="space-y-2">
                        <Label className="text-sm">Số lỗi</Label>
                        <Input
                          type="number"
                          min="0"
                          {...register('player2_fouls', { valueAsNumber: true })}
                        />
                      </div>
                    </div>
                  </div>
            ...
            {!isValid && Object.keys(errors).length > 0 && (
              <div className="p-4 bg-destructive/10 border border-destructive/20 rounded-lg">
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <span className="font-medium text-destructive">Vui lòng kiểm tra lại thông tin</span>
                </div>
