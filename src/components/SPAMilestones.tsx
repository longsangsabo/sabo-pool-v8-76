...
import { Trophy, Target, Clock, Award, Loader2, Users, Star } from 'lucide-react';
import { useSPAMilestones } from '@/hooks/useSPAMilestones';

export function SPAMilestones() {
  const { milestones, isLoading, completedCount, totalRewards } = useSPAMilestones();
  ...
  const getProgressText = (milestone: any) => {
    switch (milestone.milestone.milestone_type) {
      ...
      case 'win_rate':
        return milestone.progress >= milestone.maxProgress ? 'Đã đạt 50% tỷ lệ thắng' : 'Chưa đạt 50% tỷ lệ thắng';
      case 'spa_points':
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  <div className={`p-1 rounded-full ${
                    ...
                    {getIconForMilestoneType(milestone.milestone.milestone_type)}
                  </div>
                  <div>
                    <p className="text-sm font-medium">{milestone.milestone.milestone_name}</p>
