...
import { Search, Trophy, DollarSign, MapPin, Users, ArrowLeft } from 'lucide-react';
...
export const UnifiedChallengeCreator: React.FC<UnifiedChallengeCreatorProps> = ({
  onSuccess,
  onCancel,
}) => {
  const { user } = useAuth();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const selectedConfig = BET_OPTIONS.find(config => config.points === betPoints);
                  ...
                  {players.map((player) => (
                    <div
                      ...
                      className={`p-3 border rounded-lg cursor-pointer hover:bg-muted/50 ${
                        selectedPlayer?.user_id === player.user_id ? 'border-primary bg-primary/5' : ''
                      }`}
                ...
                <div className="p-3 bg-primary/5 border border-primary/20 rounded-lg">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <div className="text-sm text-muted-foreground">
                        Hạng {selectedPlayer.current_rank} • {selectedPlayer.spa_points} SPA
                      </div>
                    </div>
                  </div>
                </div>
              ...
              <Select value={betPoints.toString()} onValueChange={(value) => setBetPoints(parseInt(value))}>
                ...
                <SelectContent>
                  {BET_OPTIONS.map((option) => (
                    <SelectItem key={option.points} value={option.points.toString()}>
