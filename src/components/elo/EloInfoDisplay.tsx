...
export const EloInfoDisplay: React.FC<EloInfoDisplayProps> = ({ 
  ...
  const rankOrder = ['K', 'K+', 'I', 'I+', 'H', 'H+', 'G', 'G+', 'F', 'F+', 'E', 'E+'];
  ...
  const nextRankElo = nextRank ? RANK_ELO[nextRank as keyof typeof RANK_ELO] : null;
  
  const progress = nextRankElo ? Math.min(100, ((actualElo - RANK_ELO[actualRank as keyof typeof RANK_ELO]) / (nextRankElo - RANK_ELO[actualRank as keyof typeof RANK_ELO])) * 100) : 100;
      ...
      <CardContent className="p-4">
        <div className="space-y-3">
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              <Trophy className={`w-5 h-5 ${arenaMode ? 'text-cyan-400' : 'text-primary'}`} />
              <span className="font-semibold">Hạng hiện tại</span>
            </div>
            <div className="text-right">
              <div className={`text-2xl font-racing-sans-one ${arenaMode ? 'text-cyan-300' : 'text-primary'}`}>
                {actualRank}
              </div>
              <div className={`text-sm ${arenaMode ? 'text-cyan-400' : 'text-primary'}`}>
                {actualElo} ELO
              </div>
            </div>
          </div>
          ...
          {nextRank && (
            <div className="space-y-2">
              <div className="flex items-center justify-between text-sm">
                <span className={arenaMode ? 'text-slate-300' : 'text-muted-foreground'}>
              ...
              <div className="flex items-center justify-between text-xs">
                <span className={arenaMode ? 'text-slate-400' : 'text-muted-foreground'}>
          ...
          {/* System Info */}
          <div className={`text-xs ${arenaMode ? 'text-slate-400' : 'text-muted-foreground'} pt-2 border-t border-border/50`}>
            <div className="flex items-center gap-1">
