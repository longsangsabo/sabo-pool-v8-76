  ...
  const testRankRequestsAccess = async () => {
    try {
      ...
      if (requestsError) {
        ...
        toast.success(`Found ${basicRequests?.length || 0} rank requests (basic query)`);
        console.log('Basic rank requests:', basicRequests);
        return;
      }

      toast.success(`✅ Found ${rankRequests?.length || 0} rank requests for club ${clubProfile.club_name}`);
      ...
      <CardContent className="space-y-4">
        ...
        <div className="bg-blue-50 p-4 rounded-lg text-sm">
          ...
          <ol className="text-blue-800 space-y-1">
            <li>1. Click "Test Rank Requests Access" to verify database access</li>
            <li>2. Check browser console for detailed logs</li>
            <li>3. Check toast notifications for results</li>
            <li>4. Navigate to "Xác thực hạng người chơi" tab to see if data loads</li>
          </ol>
        </div>
