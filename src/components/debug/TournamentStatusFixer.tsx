  ...
  const fixTournamentStatus = async () => {
    ...
    try {
      ...
      if (data && typeof data === 'object' && 'success' in data && (data as any).success) {
        ...
        toast.success(`🏆 Đã cập nhật ${resultData.updated_matches} trận đấu từ pending thành scheduled!`);
      } else {
    ...
    <Card className="border-orange-200 bg-orange-50">
      ...
      <CardContent className="space-y-4">
        <div className="text-sm text-orange-700">
          <p>Tournament: <strong>{tournamentName}</strong></p>
          <p>ID: <code className="bg-orange-100 px-1 py-0.5 rounded">{tournamentId}</code></p>
        </div>
        ...
        {lastResult && (
          <div className="mt-4 p-3 bg-white rounded border">
            ...
            <div className="text-sm space-y-1">
              <div>
                <Badge variant={(lastResult as any)?.success ? 'default' : 'destructive'}>
                  {(lastResult as any)?.success ? 'Thành công' : 'Thất bại'}
                </Badge>
              </div>
              {(lastResult as any)?.updated_matches !== undefined && (
                <p>Đã cập nhật: <strong>{(lastResult as any).updated_matches}</strong> trận đấu</p>
              )}
        ...
        <div className="text-xs text-orange-600">
          <p><strong>Lưu ý:</strong> Tool này sẽ chuyển các trận đấu từ trạng thái "pending" sang "scheduled" nếu đủ điều kiện.</p>
        </div>
      </CardContent>
    </Card>
  );
