...
const SystemHealthDashboard = () => {
  const { healthChecks, criticalIssues, overallHealth, performHealthCheck, manualFix } = useSystemHealthCheck();
      ...
      <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
        ...
        <div className="flex items-center gap-2">
          <Badge variant={getStatusVariant(overallHealth)} className="gap-1">
            ...
            {overallHealth === 'healthy' ? 'Khỏe mạnh' : 
             overallHealth === 'warning' ? 'Cảnh báo' : 'Lỗi'}
          </Badge>
          <Button 
            variant="outline" 
            size="sm" 
          ...
          <div className="flex items-center justify-between">
            <div>
              <h3 className={`font-medium ${getOverallHealthColor()}`}>
                {overallHealth === 'healthy' ? 'Hệ thống hoạt động bình thường' :
                 overallHealth === 'warning' ? 'Có một số cảnh báo cần chú ý' :
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <div className="text-xs text-muted-foreground">
                    {check.details} • Kiểm tra lúc {check.lastChecked.toLocaleTimeString('vi-VN')}
                  </div>
                </div>
              </div>
              <div className="flex items-center gap-2">
                <Badge variant={getStatusVariant(check.status)} className="text-xs">
                  {check.status === 'healthy' ? 'OK' :
                   check.status === 'warning' ? 'Cảnh báo' :
                   check.status === 'error' ? 'Lỗi' : 'Đang kiểm tra'}
                </Badge>
                {check.status === 'error' && (
                  <Button 
                    variant="ghost" 
