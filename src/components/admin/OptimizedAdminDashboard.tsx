...
const OptimizedAdminDashboard: React.FC = () => {
  ...
  const { stats, recentActivity, isLoading: dashboardLoading, error } = useAdminDashboard();
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground">{stats?.active_users || 0} hoạt động gần đây</p>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{stats?.tournaments_count || 0}</div>
            <p className="text-xs text-muted-foreground">{stats?.active_tournaments || 0} đang diễn ra</p>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground">{stats?.pending_clubs || 0} chờ phê duyệt</p>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground">{stats?.completed_matches || 0} đã hoàn thành</p>
          </CardContent>
        </Card>
      </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Các sự kiện và hoạt động trong hệ thống</CardDescription>
        </CardHeader>
        <CardContent>
              ...
              recentActivity.map((activity) => (
                <div key={activity.id} className="flex justify-between items-center py-2 border-b last:border-b-0">
                  <div className="flex items-center gap-2">
                    ...
                    {activity.type === 'tournament_created' && <Trophy className="h-4 w-4 text-yellow-500" />}
                    {activity.type === 'club_registration' && <Building2 className="h-4 w-4 text-green-500" />}
                    {activity.type === 'match_created' && <BarChart3 className="h-4 w-4 text-purple-500" />}
