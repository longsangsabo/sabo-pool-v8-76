...
export const MigrationStatusCard: React.FC = () => {
  ...
  const getStatusColor = () => {
    switch (healthStatus.status) {
      ...
      case 'background': return 'bg-yellow-500';
      ...
      <CardContent className="space-y-4">
        ...
        <div className="grid grid-cols-2 gap-4 text-sm">
          ...
          <div>
            ...
            <span className="ml-2 font-medium">{status.hasBackgroundProcessor ? 'Active' : 'Inactive'}</span>
          </div>
          <div>
            <span className="text-muted-foreground">Last Verification:</span>
            <span className="ml-2 font-medium">{lastVerificationTime || 'Never'}</span>
          </div>
        </div>
