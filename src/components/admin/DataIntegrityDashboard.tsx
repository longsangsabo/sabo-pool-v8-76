...
import { RefreshCw, Database, AlertTriangle, CheckCircle, Trash, RotateCcw } from 'lucide-react';
...
export const DataIntegrityDashboard = () => {
  ...
  const { data: integrityReport, isLoading: integrityLoading, refetch: refetchIntegrity } = useQuery({
  ...
  const migrationMutation = useMutation({
    ...
    onSuccess: (data) => {
      toast.success(`Migration completed: ${data.migrated_tournaments + data.migrated_challenges + data.migrated_club_profiles} records migrated`);
  ...
  const cleanupMutation = useMutation({
    ...
    onSuccess: (data) => {
      toast.success(`Cleanup completed: ${data.orphaned_registrations_cleaned} orphaned records cleaned`);
      refetchIntegrity();
    },
    onError: () => {
  ...
  const bulkDeleteMutation = useMutation({
    ...
    onSuccess: (data) => {
      toast.success(`Bulk delete completed: ${data.affected_count} records deleted`);
      queryClient.invalidateQueries({ queryKey: ['soft-delete-stats'] });
      setSelectedIds([]);
    },
    onError: () => {
  ...
  const bulkRestoreMutation = useMutation({
    ...
    onSuccess: (data) => {
      toast.success(`Bulk restore completed: ${data.affected_count} records restored`);
      queryClient.invalidateQueries({ queryKey: ['soft-delete-stats'] });
      setSelectedIds([]);
    },
    onError: () => {
  ...
  const handleBulkOperation = (operation: 'delete' | 'restore') => {
    ...
    const mutation = operation === 'delete' ? bulkDeleteMutation : bulkRestoreMutation;
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-gray-600">Monitor và quản lý tình trạng dữ liệu hệ thống</p>
        </div>
        <Button 
          ...
          {integrityLoading ? (
            ...
            <div className="space-y-4">
              <div className="flex items-center gap-2">
                ...
                <Badge variant={integrityReport.status === 'healthy' ? 'default' : 'destructive'}>
                  {integrityReport.status === 'healthy' ? 'Healthy' : 'Issues Found'}
                </Badge>
                <span className="text-sm text-muted-foreground">
                  Last check: {new Date(integrityReport.check_timestamp).toLocaleString()}
                </span>
              </div>
              ...
              {integrityReport.status !== 'healthy' && (
                <Alert>
                  ...
                  <AlertDescription>
                    Found {integrityReport.orphaned_tournament_registrations} orphaned registrations and{' '}
                    {integrityReport.inconsistent_visibility_tournaments} visibility inconsistencies
                  </AlertDescription>
                </Alert>
              )}
            </div>
          ) : (
            <p className="text-muted-foreground">Unable to load integrity report</p>
          )}
        </CardContent>
      </Card>
      ...
      {stats && (
        <Card>
          <CardHeader>
            ...
            <CardDescription>Tổng quan về dữ liệu đã xóa trong hệ thống</CardDescription>
          </CardHeader>
          <CardContent>
            <div className="grid gap-4 md:grid-cols-3">
              {Object.entries(stats as Record<string, any>).map(([table, data]) => (
                <div key={table} className="p-4 border rounded-lg">
                  ...
                  <div className="mt-2 space-y-1 text-sm">
                    ...
                    <div className="text-destructive">Deleted: {data?.deleted || 0}</div>
                    <div className="text-muted-foreground">Hidden: {data?.hidden || 0}</div>
                  </div>
                </div>
              ))}
            </div>
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          <div className="grid gap-4 md:grid-cols-2">
            ...
            <div>
              <label className="block text-sm font-medium mb-2">Entity IDs (comma separated)</label>
              <input 
                type="text"
