...
import { ArrowRight, Download, RefreshCw, CheckCircle2, AlertTriangle } from 'lucide-react';
...
export const SABOMigrationManager = () => {
  ...
  const migrateSingle = useMutation({
    ...
    onSuccess: (result) => {
      if (result.success) {
        if (result.migrated) {
          toast.success(`✅ Migrated "${result.tournament_name}" to SABO structure`);
        } else if (result.already_sabo) {
          toast.info(`ℹ️ "${result.tournament_name}" already has SABO structure`);
        }
      } else {
  ...
  const migrateAll = useMutation({
    mutationFn: async () => {
      const { data, error } = await supabase.rpc('migrate_all_tournaments_to_sabo');
      if (error) throw error;
  ...
  const getTournamentStatus = (tournament: any) => {
    ...
    if (matchCount === 0) {
      return { status: 'no_matches', color: 'bg-yellow-500', text: 'No Matches' };
    } else if (matchCount === 27) {
      return { status: 'sabo_compliant', color: 'bg-green-500', text: '27 Matches (SABO?)' };
    } else {
      return { status: 'needs_migration', color: 'bg-red-500', text: `${matchCount} Matches` };
    }
  };
      ...
      <Card>
        <CardHeader>
          ...
          <p className="text-sm text-muted-foreground">
            Migrate double elimination tournaments to SABO structure (27 matches)
          </p>
        </CardHeader>
        <CardContent className="space-y-4">
          <div className="flex gap-4">
            ...
            <Button
              variant="outline"
              onClick={() => queryClient.invalidateQueries({ queryKey: ['migration-tournaments'] })}
            >
              ...
              return (
                ...
                >
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <Button
                      size="sm"
                      variant={status.status === 'sabo_compliant' ? 'outline' : 'default'}
                      onClick={() => migrateSingle.mutate(tournament.id)}
                      disabled={migrateSingle.isPending}
                      className="flex items-center gap-1"
                    >
                      {migrateSingle.isPending && selectedTournament === tournament.id ? (
                        <RefreshCw className="h-3 w-3 animate-spin" />
                      ) : (
                        <ArrowRight className="h-3 w-3" />
                      )}
                      {status.status === 'sabo_compliant' ? 'Verify' : 'Migrate'}
                    </Button>
                  </div>
                </div>
              );
