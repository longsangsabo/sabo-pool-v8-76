          ...
          {searchQuery.length >= 2 && (
            <div className="space-y-2">
              ...
              <Select value={selectedUserId} onValueChange={setSelectedUserId}>
                <SelectTrigger>
                  <SelectValue placeholder={isSearching ? "Đang tìm kiếm..." : "Chọn người dùng"} />
                </SelectTrigger>
                <SelectContent>
                  {users.map((user) => (
          ...
          {selectedUser && (
            <div className="p-3 bg-muted rounded-lg">
              <div className="text-sm space-y-1">
                <div><strong>Tên:</strong> {selectedUser.full_name}</div>
                <div><strong>SĐT:</strong> {selectedUser.phone}</div>
                <div><strong>SPA hiện tại:</strong> {selectedUser.current_spa}</div>
              </div>
            </div>
          )}
