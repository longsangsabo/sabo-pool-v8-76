...
import { useTournamentRewardsSync } from '@/hooks/useTournamentRewardsSync';
...
export const TournamentRewardsSync: React.FC<TournamentRewardsSyncProps> = ({
  ...
  const { syncTournamentRewards, syncing, error } = useTournamentRewardsSync();

  const handleSync = async (syncAll = false) => {
    try {
      const result = await syncTournamentRewards(syncAll ? undefined : tournamentId);
      ...
      <CardContent className="space-y-4">
        ...
        <div className="pt-3 border-t">
          <Badge variant="secondary" className="text-xs">
            This will update tournament results to match prize tier configurations
          </Badge>
        </div>
      </CardContent>
    </Card>
