  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-3xl font-bold tracking-tight">Automated Migration System</h2>
        ...
        <div className="flex items-center gap-2">
          {getStatusIcon()}
          <Badge 
            variant={healthStatus.status === 'complete' ? 'default' : 'secondary'}
            className={`${
              healthStatus.status === 'complete' ? 'bg-green-500' : 
              healthStatus.status === 'running' ? 'bg-blue-500' : 
              healthStatus.status === 'background' ? 'bg-yellow-500' : 
              'bg-gray-500'
            } text-white`}
          >
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium">Background Process</CardTitle>
      ...
      {isComplete && (
        <Card className="border-green-200 bg-green-50">
          <CardHeader>
            ...
            <CardDescription>
              All player_id references have been successfully migrated to user_id.
            </CardDescription>
          </CardHeader>
          <CardContent>
