...
interface MatchInputValidatorProps {
  match: MatchData;
  onSubmit: (matchId: string, scores: { player1: number; player2: number }) => Promise<void>;
...
export const MatchInputValidator: React.FC<MatchInputValidatorProps> = ({
  ...
  const handleSubmit = async () => {
    if (!validateInput()) {
      toast({
        ...
        description: "Please fix the errors before submitting the match result.",
        variant: "destructive",
      });
      return;
    }
  ...
  const getBracketTypeColor = (type: string) => {
    switch (type) {
      case 'winner': return 'bg-green-500/10 text-green-700 border-green-200';
      case 'loser': return 'bg-red-500/10 text-red-700 border-red-200';
      case 'semifinal': return 'bg-blue-500/10 text-blue-700 border-blue-200';
      case 'final': return 'bg-purple-500/10 text-purple-700 border-purple-200';
  ...
  const hasChanges = player1Score !== (match.score_player1?.toString() || '') || 
  ...
  return (
    <Card className="w-full max-w-md">
      <CardHeader className="pb-4">
        ...
        <CardDescription>
          Enter match scores with validation
        </CardDescription>
      </CardHeader>
