...
import { RefreshCw, Database, AlertTriangle, CheckCircle2, RotateCcw } from 'lucide-react';
...
export function AdminDataSync() {
  ...
  // Check data health
  const { data: healthStats, isLoading: isLoadingHealth, refetch: refetchHealth } = useQuery({
    ...
    queryFn: async () => {
      ...
      const playerWalletMap = new Map(wallets.map(w => [w.user_id, w.points_balance]));
  ...
  const syncDataMutation = useMutation({
    mutationFn: async () => {
      ...
      const data = { success: true, synced_count: 0, created_count: 0, message: 'Sync function not available' };
      const error = null;
      ...
      <CardContent className="space-y-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <div className="space-y-2">
            ...
            <p className="text-2xl font-bold">{healthStats?.total_players || 0}</p>
          </div>
          <div className="space-y-2">
            <p className="text-sm text-muted-foreground">With SPA Points</p>
            <p className="text-2xl font-bold">{healthStats?.players_with_spa || 0}</p>
          </div>
          <div className="space-y-2">
            <p className="text-sm text-muted-foreground">With Wallets</p>
            <p className="text-2xl font-bold">{healthStats?.players_with_wallets || 0}</p>
          </div>
          <div className="space-y-2">
            <p className="text-sm text-muted-foreground">Sync Status</p>
            <Badge className={getSyncStatusColor(healthStats?.sync_issues || 0)}>
              {getSyncStatusIcon(healthStats?.sync_issues || 0)}
              <span className="ml-1">
                {(healthStats?.sync_issues || 0) === 0 ? 'Synced' : `${healthStats?.sync_issues} Issues`}
              </span>
            </Badge>
          </div>
        </div>
        ...
        {healthStats && healthStats.sync_issues > 0 && (
          <Alert>
            ...
            <AlertDescription>
              <strong>{healthStats.sync_issues} players</strong> have mismatched SPA points between 
              player rankings and wallet balances. Run synchronization to fix these issues.
            </AlertDescription>
          </Alert>
        )}
