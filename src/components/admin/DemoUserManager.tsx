...
export const DemoUserManager: React.FC<DemoUserManagerProps> = ({ addLog }) => {
  ...
  const initializeDemoUsers = async () => {
    ...
    try {
      ...
      if (!data?.success) {
        log(`❌ Error initializing: ${data?.error || 'Unknown error'}`, 'error');
        toast.error('Failed to initialize demo users');
        return;
      }
      
      log(`✅ ${data.created || 0} demo users initialized successfully!`, 'success');
  ...
  const loadStats = async () => {
    try {
      ...
      const usage_percentage = total_demo_users > 0 ? Math.round((in_use_users / total_demo_users) * 100) : 0;
