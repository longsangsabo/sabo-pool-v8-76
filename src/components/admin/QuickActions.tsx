  ...
  return (
    <Card className="enhanced-card">
      <CardHeader>
        ...
        <CardDescription className="text-muted-foreground">{t('admin.common_actions')}</CardDescription>
      </CardHeader>
            ...
            >
              <div className={`p-3 rounded-xl transition-all duration-200 ${
                action.urgent 
                  ...
                  : action.bgColor.includes('yellow') ? 'bg-primary/20 dark:bg-primary/30' :
                    action.bgColor.includes('green') ? 'bg-accent-green/20 dark:bg-accent-green/30' :
                    action.bgColor.includes('purple') ? 'bg-accent-purple/20 dark:bg-accent-purple/30' :
                    action.bgColor.includes('red') ? 'bg-accent-red/20 dark:bg-accent-red/30' :
                    'bg-muted/50 dark:bg-muted/80'
              }`}>
                <Icon className={`w-5 h-5 ${
                  action.urgent 
                    ? 'text-accent-red' 
                    : action.color.includes('yellow') ? 'text-primary' :
                      action.color.includes('green') ? 'text-accent-green' :
                      action.color.includes('purple') ? 'text-accent-purple' :
                      action.color.includes('red') ? 'text-accent-red' :
                      'text-muted-foreground'
                }`} />
              </div>
              <div className="flex-1 text-left">
                <div className="font-semibold text-card-foreground">{action.title}</div>
