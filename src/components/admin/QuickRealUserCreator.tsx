    ...
    try {
      ...
      for (let i = 0; i < userCount; i++) {
        ...
        try {
          ...
          if (!data?.success) {
            addLog(`❌ Lỗi tạo user: ${data?.error || 'Unknown error'}`, 'error');
            continue;
          }

          const realUserId = data.user_id;
          addLog(`✅ User tạo thành công: ${fullName} (ID: ${realUserId.slice(0, 8)}...)`, 'success');
        ...
        {(isCreating || logs.length > 0) && (
          <div className="space-y-4 mt-4">
            ...
            <div className="bg-gray-50 dark:bg-gray-800 p-4 rounded-lg border max-h-60 overflow-y-auto">
              ...
              {logs.length === 0 && (
                <p className="body-small text-muted-foreground">Chưa có log nào...</p>
              )}
            </div>
          </div>
        )}
