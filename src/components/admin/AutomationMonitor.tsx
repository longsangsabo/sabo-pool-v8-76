...
const AutomationMonitor = () => {
  ...
  const loadAutomationData = async () => {
    try {
      ...
      try {
        const { data: syncData, error: syncError } = await supabase.functions.invoke('sync-club-data', {
          method: 'GET'
        });
    ...
    try {
      ...
      switch (functionName) {
        ...
        case 'sync_club_data':
          setSyncing(true);
          try {
            const { data: syncResult, error: syncError } = await supabase.functions.invoke('sync-club-data', {
              method: 'POST'
            });
            ...
            <CardContent>
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-4">
                <div className="text-center">
                  ...
                  <div className="text-xs text-muted-foreground">Club Profiles</div>
                </div>
                <div className="text-center">
                  <div className="text-xl font-bold text-secondary">
                    {syncStatus.clubs_count}
                  </div>
                  <div className="text-xs text-muted-foreground">Clubs (Synced)</div>
                </div>
                <div className="text-center">
                  <div className="text-xl font-bold text-destructive">
                    {syncStatus.inconsistencies_count}
                  </div>
                  <div className="text-xs text-muted-foreground">Inconsistencies</div>
                </div>
                <div className="text-center">
                  <Button 
              ...
              {syncStatus.last_sync && (
                <div className="text-xs text-muted-foreground">
                  Lần sync cuối: {new Date(syncStatus.last_sync).toLocaleString('vi-VN')} 
                  <Badge variant={syncStatus.last_sync_success ? 'default' : 'destructive'} className="ml-2">
              ...
              <Card>
                ...
                <CardContent className="space-y-3">
                  {[
                    ...
                    { name: 'sync_club_data', icon: RefreshCw, label: 'Club Data Sync', desc: 'Đồng bộ dữ liệu từ club_profiles sang clubs' }
                  ].map((func) => {
