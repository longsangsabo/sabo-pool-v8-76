...
export const ProfileStats: React.FC<ProfileStatsProps> = ({ 
  ...
  const stats = {
    ...
    monthlyMatches: statistics?.monthly_matches || profile?.monthly_matches || profile?.matches_played || 0,
    weeklyMatches: statistics?.weekly_matches || 0,
    dailyMatches: statistics?.daily_matches || 0,
    winStreak: statistics?.current_win_streak || profile?.current_win_streak || 0,
    ...
    currentRank: statistics?.current_ranking || profile?.current_ranking || null,
    totalHours: Math.round((statistics?.total_play_time_minutes || 0) / 60),
    totalMatches: statistics?.total_matches || profile?.total_matches || profile?.matches_played || 0,
  ...
  if (showLoading) {
    return (
      <div className={`grid ${variant === 'mobile' ? 'grid-cols-2' : 'grid-cols-1 md:grid-cols-2 lg:grid-cols-4'} gap-3`}>
        {[...Array(4)].map((_, i) => (
          <Card key={i} className={`h-20 ${arenaMode ? 'bg-slate-800/50' : ''}`}>
  ...
  if (variant === 'mobile') {
    return (
      <div className="grid grid-cols-2 gap-3">
        {/* ELO/Rank Card */}
        <Card className={`h-20 ${arenaMode ? 'bg-slate-800/50 border-cyan-500/30' : ''}`}>
          <CardContent className="p-3 h-full flex flex-col justify-center">
            <div className="flex items-center justify-between mb-1">
              <Trophy className={`w-4 h-4 ${arenaMode ? 'text-cyan-400' : 'text-primary'}`} />
              <span className={`text-2xl font-racing-sans-one ${arenaMode ? 'text-cyan-300' : 'text-primary'}`}>
                {stats.rank}
              </span>
            </div>
            <div className={`text-xs ${arenaMode ? 'text-slate-400' : 'text-muted-foreground'}`}>Hạng hiện tại</div>
            <div className={`text-xs font-medium ${arenaMode ? 'text-cyan-400' : 'text-primary'}`}>
              {stats.elo} ELO
            </div>
          </CardContent>
        </Card>
        ...
        {/* SPA Points Card */}
        <Card className={`h-20 ${arenaMode ? 'bg-slate-800/50 border-yellow-500/30' : ''}`}>
          <CardContent className="p-3 h-full flex flex-col justify-center">
            <div className="flex items-center justify-between mb-1">
            ...
            <div className={`text-xs ${arenaMode ? 'text-slate-400' : 'text-muted-foreground'}`}>SPA Points</div>
        ...
        {/* Ranking Card */}
        <Card className={`h-20 ${arenaMode ? 'bg-slate-800/50 border-blue-500/30' : ''}`}>
          <CardContent className="p-3 h-full flex flex-col justify-center">
            <div className="flex items-center justify-between mb-1">
            ...
            <div className="text-xs text-green-600 font-medium">
              {stats.winStreak > 0 ? `${stats.winStreak} thắng liên tiếp` : 'Cần cải thiện'}
            </div>
          </CardContent>
        </Card>

        {/* Activity Card */}
        <Card className={`h-20 ${arenaMode ? 'bg-slate-800/50 border-orange-500/30' : ''}`}>
          <CardContent className="p-3 h-full flex flex-col justify-center">
            <div className="flex items-center justify-between mb-1">
            ...
            <div className={`text-xs ${arenaMode ? 'text-slate-400' : 'text-muted-foreground'}`}>Trận tháng này</div>
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
      ...
      <Card>
        ...
        <CardContent>
          <div className="text-2xl font-racing-sans-one text-primary">{stats.elo}</div>
      ...
      <Card>
        ...
        <CardContent>
          <div className="text-2xl font-racing-sans-one text-yellow-600">{stats.spaPoints}</div>
          <p className="text-xs text-muted-foreground">
            +{Math.floor(stats.weeklyMatches * 25)} tuần này
          </p>
          <div className="flex items-center gap-1 mt-2">
            <div className="h-1 bg-muted rounded-full flex-1">
              <div 
                ...
                style={{ width: `${Math.min((stats.spaPoints / 5000) * 100, 100)}%` }}
              />
            </div>
      ...
      <Card>
        ...
        <CardContent>
          ...
          <p className="text-xs text-muted-foreground">
            Tổng thể ({stats.weeklyRank ? `#${stats.weeklyRank} tuần` : 'chưa xếp hạng tuần'})
          </p>
          <div className="flex items-center gap-2 mt-2">
            <Badge variant={stats.winStreak > 0 ? "default" : "secondary"} className="text-xs">
              {stats.winStreak > 0 ? `${stats.winStreak} thắng liên tiếp` : 'Chưa có streak'}
            </Badge>
          </div>
        </CardContent>
      </Card>
      ...
      <Card>
        ...
        <CardContent>
          <div className="text-2xl font-racing-sans-one text-green-600">{stats.monthlyMatches}</div>
          <p className="text-xs text-muted-foreground">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="space-y-2">
              <div className="flex justify-between items-center">
                ...
                <span className="font-medium text-green-600">{stats.winPercentage.toFixed(1)}%</span>
              </div>
              <div className="flex justify-between items-center">
              ...
              <div className="flex justify-between items-center">
                <span className="text-sm text-muted-foreground">Thắng / Thua</span>
            ...
            <div className="space-y-2">
              ...
              <div className="flex justify-between items-center">
                <span className="text-sm text-muted-foreground">Streak tốt nhất</span>
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                <span className="text-sm font-medium">{stats.dailyMatches} trận</span>
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                <span className="text-sm font-medium">{stats.weeklyMatches} trận</span>
                <Badge variant="outline" className="text-xs">
                  {stats.weeklyMatches >= 10 ? 'Xuất sắc' : stats.weeklyMatches >= 5 ? 'Tốt' : 'Cần cải thiện'}
                </Badge>
              </div>
            </div>
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                <span className="text-sm font-medium">{stats.monthlyMatches} trận</span>
                <div className="w-20 h-2 bg-muted rounded-full overflow-hidden">
                  <div 
                    className="h-full bg-gradient-to-r from-blue-500 to-purple-500 transition-all"
                    style={{ width: `${Math.min((stats.monthlyMatches / 50) * 100, 100)}%` }}
                  />
                </div>
              </div>
            </div>
