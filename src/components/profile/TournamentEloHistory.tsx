...
import { Trophy, Medal, Star, TrendingUp, Calendar, Target } from 'lucide-react';
...
export const TournamentEloHistory: React.FC = () => {
  ...
  const fetchTournamentHistory = async () => {
    ...
    try {
      ...
      // Transform data
      const transformedResults: TournamentResult[] = (results || []).map(result => ({
        id: result.id,
        tournament_id: result.tournament_id,
        tournament_name: (result.tournaments as any)?.name || 'Unknown Tournament',
        ...
        tournament_date: (result.tournaments as any)?.start_date || (result.tournaments as any)?.created_at || result.created_at
      }));
      ...
      const summary: EloSummary = {
        ...
        total_elo_earned: transformedResults.reduce((sum, r) => sum + r.elo_points_earned, 0),
        total_spa_earned: transformedResults.reduce((sum, r) => sum + r.spa_points_earned, 0),
        championships: transformedResults.filter(r => r.final_position === 1).length,
        runner_ups: transformedResults.filter(r => r.final_position === 2).length,
        top_3_finishes: transformedResults.filter(r => r.final_position <= 3).length,
        average_position: transformedResults.length > 0 
          ? transformedResults.reduce((sum, r) => sum + r.final_position, 0) / transformedResults.length
          : 0
      };
        ...
        <CardContent>
          ...
          <div className="mt-4 pt-4 border-t">
            <div className="grid grid-cols-2 md:grid-cols-3 gap-4 text-sm">
              ...
              <div className="flex justify-between">
                <span>Vị trí TB:</span>
                <span className="font-medium">{eloSummary.average_position.toFixed(1)}</span>
              </div>
            </div>
          </div>
        </CardContent>
              ...
              <div key={result.id} className="border rounded-lg p-4 space-y-3">
                <div className="flex justify-between items-start">
                  <div className="flex-1">
                    <h4 className="font-medium text-lg">{result.tournament_name}</h4>
                    <p className="text-sm text-gray-500">{formatDate(result.tournament_date)}</p>
                  </div>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                  <div>
                    ...
                    <div className="font-medium">{result.matches_won}W - {result.matches_lost}L</div>
                  </div>
                  <div>
                    <span className="text-gray-600">ELO nhận:</span>
                    <div className="font-medium text-blue-600">+{result.elo_points_earned}</div>
                  </div>
                  <div>
                    <span className="text-gray-600">SPA nhận:</span>
                    <div className="font-medium text-green-600">+{result.spa_points_earned.toLocaleString()}</div>
                  </div>
                  {result.prize_amount > 0 && (
                    <div>
                      <span className="text-gray-600">Tiền thưởng:</span>
                      <div className="font-medium text-yellow-600">{result.prize_amount.toLocaleString()}đ</div>
                    </div>
                  )}
                </div>
              </div>
