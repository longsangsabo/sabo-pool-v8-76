...
const PerformanceTab = () => {
  ...
  if (isLoading) {
    return (
      <div className="space-y-6">
        <div>
          <h2 className="text-2xl font-bold text-foreground mb-2">Thành tích & Thách đấu</h2>
  ...
  const transformedPlayerData = {
    rank: (playerData?.verified_rank || getRankByElo(playerData?.elo_points || 1000)) as RankCode,
      ...
      <div className="performance-overview grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <Card className="performance-card elo-theme">
          <CardContent className="pt-6">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-medium text-muted-foreground">Hạng hiện tại</p>
                <div className="flex items-center gap-2 mt-1">
                  <p className="text-2xl font-bold text-foreground">{transformedPlayerData.rank}</p>
                  <span className="text-lg text-muted-foreground">•</span>
                  <p className="text-xl font-semibold text-blue-600">{transformedPlayerData.elo_points} ELO</p>
                </div>
              </div>
        ...
        <Card className="performance-card spa-theme">
          <CardContent className="pt-6">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-medium text-muted-foreground">SPA Points</p>
                <div className="flex items-center gap-2 mt-1">
                  <p className="text-2xl font-bold text-foreground">{transformedPlayerData.spa_points}</p>
                  <span className="text-sm text-green-600 font-medium">(+300 tuần này)</span>
                </div>
              </div>
        ...
        <Card className="performance-card progress-theme">
          <CardContent className="pt-6">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-medium text-muted-foreground">Tiến độ thăng hạng</p>
                <div className="flex items-center gap-2 mt-1">
                  <p className="text-lg font-bold text-foreground">250 ELO nữa</p>
                  <span className="text-sm text-muted-foreground">→ Hạng F</span>
                </div>
              </div>
          ...
          <Card className="quick-actions">
            ...
            <CardContent className="space-y-3">
              <Button 
                className="w-full" 
                size="lg"
                onClick={() => window.location.href = '/ranking?tab=registration'}
              >
              ...
              <div className="space-y-2">
                {transformedPlayerData.weekly_spa_rank && (
                  <div className="flex justify-between items-center p-3 bg-blue-50 rounded-lg">
                    ...
                    <span className="text-lg font-bold text-blue-600">#{transformedPlayerData.weekly_spa_rank}</span>
                  </div>
                )}
                {transformedPlayerData.monthly_spa_rank && (
                  <div className="flex justify-between items-center p-3 bg-green-50 rounded-lg">
                    <span className="text-sm font-medium">Tháng này</span>
                    <span className="text-lg font-bold text-green-600">#{transformedPlayerData.monthly_spa_rank}</span>
                  </div>
                )}
              </div>
              <div className="text-center py-2 text-muted-foreground">
                <p className="text-xs">Xem bảng xếp hạng đầy đủ tại trang Leaderboard</p>
              </div>
            </CardContent>
          </Card>
            ...
            <CardContent>
              <div className="space-y-3">
                <div className="p-3 bg-orange-50 rounded-lg">
                  <p className="text-sm text-orange-800 font-medium">Thách đấu hàng ngày</p>
                  <p className="text-xs text-orange-600">+50 SPA mỗi ngày</p>
                </div>
                <div className="p-3 bg-green-50 rounded-lg">
                  <p className="text-sm text-green-800 font-medium">Chuỗi thắng</p>
                  <p className="text-xs text-green-600">+25 SPA mỗi trận</p>
                </div>
                <div className="p-3 bg-purple-50 rounded-lg">
                  <p className="text-sm text-purple-800 font-medium">Tham gia giải đấu</p>
                  <p className="text-xs text-purple-600">Phần thưởng tùy hạng</p>
                </div>
              </div>
            </CardContent>
          </Card>
