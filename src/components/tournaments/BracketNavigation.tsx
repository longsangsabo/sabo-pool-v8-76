...
export const BracketNavigation: React.FC<BracketNavigationProps> = ({
  ...
  const getRoundIcon = (bracketType: string) => {
    switch (bracketType) {
      case 'winner': return <Trophy className="w-4 h-4 text-accent-blue" />;
      case 'loser': return <Medal className="w-4 h-4 text-accent-red" />;
      case 'final': return <Crown className="w-4 h-4 text-tournament-gold" />;
      default: return <Trophy className="w-4 h-4 text-muted-foreground" />;
    }
  };

  const getRoundProgress = (round: Round) => {
    const percentage = round.matches > 0 ? (round.completedMatches / round.matches) * 100 : 0;
    return percentage;
  };
  ...
  const getStatusColor = () => {
    switch (tournamentStatus) {
      ...
      case 'ongoing': return 'text-accent-blue';
    ...
    <Card className="mb-6">
      <CardContent className="p-4">
        ...
        <div className="space-y-3">
          <h4 className="text-sm font-medium text-muted-foreground">Round Navigation</h4>
                ...
                return (
                  ...
                  >
                    ...
                    <div 
                      ...
                      style={{ 
                        background: `linear-gradient(to right, ${
                          isCompleted ? 'hsl(var(--accent-green) / 0.2)' : 'hsl(var(--accent-blue) / 0.1)'
                    ...
                    <div className="relative flex items-center gap-1">
                      ...
                      {isCompleted && <CheckCircle className="w-3 h-3 text-accent-green" />}
                    </div>
                  </Button>
                );
              })}
        ...
        <div className="flex items-center justify-center gap-4 mt-4 pt-3 border-t border-border/50">
          <Button
            ...
            size="sm"
            onClick={() => {
              const currentIndex = rounds.findIndex(r => r.number === currentRound);
              if (currentIndex > 0) {
                onRoundSelect(rounds[currentIndex - 1].number);
              }
            }}
            disabled={!currentRound || rounds.findIndex(r => r.number === currentRound) === 0}
          ...
          <div className="text-center">
            <div className="text-sm font-medium">
              {currentRound ? rounds.find(r => r.number === currentRound)?.name : 'Select Round'}
            </div>
          ...
          <Button
            ...
            disabled={!currentRound || rounds.findIndex(r => r.number === currentRound) === rounds.length - 1}
            className="flex items-center gap-2"
          >
            Next Round
