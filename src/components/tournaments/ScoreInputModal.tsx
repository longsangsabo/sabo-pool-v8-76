...
export const ScoreInputModal: React.FC<ScoreInputModalProps> = ({
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const { data: scoreResult, error: scoreError } = await supabase.rpc('update_match_score_safe', {
        p_match_id: match.id,
      ...
      if (scoreResult && typeof scoreResult === 'object' && 'error' in scoreResult && scoreResult.error) {
      ...
      if (result?.tournament_completed && result?.is_final_match) {
        ...
        toast.success('🏆 Trận chung kết đã hoàn thành! Chúc mừng nhà vô địch!');
      } else if (tournamentType === 'single_elimination' && result?.winner_id && !result?.is_final_match) {
        // For single elimination and non-final matches, advance winner to next round
        console.log('🎯 Advancing winner to next round...');
        
        const { data: advanceResult, error: advanceError } = await supabase.rpc('advance_winner_safe', {
          p_match_id: match.id
        });

        if (advanceError) {
          console.error('❌ Error advancing winner:', advanceError);
          toast.error('Đã cập nhật tỷ số nhưng không thể chuyển vòng: ' + advanceError.message);
        } else {
          console.log('✅ Winner advanced successfully:', advanceResult);
          toast.success('✅ Đã cập nhật tỷ số và chuyển người thắng lên vòng tiếp theo!');
        }
        ...
        if (advanceError) {
          console.error('❌ Error advancing in double elimination:', advanceError);
          toast.error('Đã cập nhật tỷ số nhưng không thể chuyển vòng: ' + advanceError.message);
        } else {
          console.log('✅ Enhanced double elimination advancement successful:', advanceResult);
          const advanceData = advanceResult as any;
          if (advanceData?.success) {
            const advancements = (advanceData?.advancements || 0);
            toast.success(`✅ Đã cập nhật tỷ số và chuyển ${advancements} người chơi!`);
          } else {
      ...
      toast.error('Lỗi khi cập nhật tỷ số: ' + (error.message || 'Unknown error'));
    } finally {
