...
export const FinalSection: React.FC<FinalSectionProps> = ({
  ...
  const finalMatches = matches.filter(match => 
    (
      ...
      (match.match_stage === 'grand_final' || match.match_stage === 'finals' || match.match_stage === 'final')
    ) &&
  ...
  const runnerUp = winner === finalMatch.player1_id ? finalMatch.player2_id : finalMatch.player1_id;
      ...
      <CardHeader>
        <div className="flex items-center justify-between">
          <CardTitle className="flex items-center gap-2">
            ...
            Championship Final
            <Badge variant="outline" className={`ml-2 ${isCompleted ? 'border-green-300 text-green-700' : 'border-yellow-300 text-yellow-700'}`}>
        ...
        <div className="grid gap-6">
                ...
                <div className="grid md:grid-cols-2 gap-4">
                  ...
                  <div className="p-4 bg-white dark:bg-gray-800 rounded-lg border-2 border-yellow-400 shadow-lg">
                    <div className="flex items-center justify-center gap-2 mb-2">
                      <Trophy className="h-6 w-6 text-yellow-600" />
                      <span className="font-bold text-yellow-800 dark:text-yellow-200">CHAMPION</span>
                    </div>
                    <div className="text-center">
                      <p className="font-semibold">
                        {winner === finalMatch.player1_id ? 
                          (finalMatch.player1?.full_name || finalMatch.player1?.display_name || 'Player 1') :
                          (finalMatch.player2?.full_name || finalMatch.player2?.display_name || 'Player 2')
                        }
                      </p>
                      <div className="text-sm text-muted-foreground mt-1">
                        Score: {winner === finalMatch.player1_id ? finalMatch.score_player1 : finalMatch.score_player2}
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="p-4 bg-white dark:bg-gray-800 rounded-lg border-2 border-gray-300 shadow-lg">
                    <div className="flex items-center justify-center gap-2 mb-2">
                      ...
                      <span className="font-bold text-gray-700 dark:text-gray-300">RUNNER-UP</span>
                    </div>
                    <div className="text-center">
                      <p className="font-semibold">
                      ...
                      <div className="text-sm text-muted-foreground mt-1">
                        Score: {runnerUp === finalMatch.player1_id ? finalMatch.score_player1 : finalMatch.score_player2}
                      </div>
                    </div>
                  </div>
                </div>
          ...
          <div className="mt-4 p-3 bg-yellow-100 dark:bg-yellow-900/30 rounded-lg border border-yellow-200">
            <p className="text-xs text-muted-foreground text-center">
              <strong>Championship Final (R300):</strong> Single elimination final between 2 semifinal winners. 
              Winner becomes champion, loser becomes runner-up. No reset needed.
            </p>
          </div>
        </div>
      </CardContent>
