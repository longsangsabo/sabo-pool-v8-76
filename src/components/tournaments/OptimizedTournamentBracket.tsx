...
export const OptimizedTournamentBracket: React.FC<OptimizedTournamentBracketProps> = ({
  tournamentId,
  isClubOwner = false,
  adminMode = false
}) => {
  ...
  const rounds = useMemo(() => {
    ...
    return Object.keys(grouped)
      ...
      .map(round => ({
        ...
        matches: grouped[parseInt(round)].sort((a, b) => a.match_number - b.match_number)
      }));
  }, [matches]);
  ...
  const progressionCheck = useMemo(() => {
    ...
    for (let i = 0; i < rounds.length - 1; i++) {
      ...
      if (completedInCurrent.length > 0 && emptyInNext.length > 0) {
        issues.push(`Round ${currentRound.round} -> ${nextRound.round}: ${completedInCurrent.length} completed, ${emptyInNext.length} empty slots`);
      }
    }
        ...
        <CardHeader className="pb-3">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2 text-sm text-muted-foreground">
              <span>Cập nhật lúc: {lastUpdateTime?.toLocaleTimeString() || 'N/A'}</span>
            </div>
          </div>
        </CardHeader>
        <CardContent className="space-y-2">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
            ...
            <div>
              ...
              <div className={`font-semibold ${needsProgression ? 'text-orange-600' : 'text-green-600'}`}>
