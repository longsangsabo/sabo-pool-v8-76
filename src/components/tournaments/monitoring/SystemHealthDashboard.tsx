...
export const SystemHealthDashboard: React.FC<SystemHealthDashboardProps> = ({ className }) => {
  ...
  const fetchSystemHealth = async () => {
    try {
      const { data, error } = await supabase.functions.invoke('tournament-automation-monitor', {
  ...
  const runAutoRecovery = async () => {
    ...
    try {
      ...
      toast.success(`✅ Auto-recovery completed: ${data.result.recovered_tournaments} tournaments fixed`);
    ...
    <div className={cn("space-y-6", className)}>
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          >
            <RefreshCw className={cn("w-4 h-4 mr-1", isRunningAction && "animate-spin")} />
            Refresh
          </Button>
          <Button
            variant="outline"
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <CardContent className="p-4">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-accent-blue">{stats.success_rate}%</p>
              </div>
        ...
        <Card className={cn("border-2", stats.problem_tournaments > 0 ? "border-red-200" : "border-green-200")}>
          <CardContent className="p-4">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm text-muted-foreground">Problem Tournaments</p>
                <p className={cn("text-2xl font-bold", stats.problem_tournaments > 0 ? "text-red-600" : "text-green-600")}>
          ...
          <CardContent className="p-4">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-purple-600">{stats.triggers_active}</p>
              </div>
          ...
          <CardContent className="p-4">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-blue-600">{stats.total_logs_24h}</p>
              </div>
        ...
        <CardContent>
          ...
          <div className="mt-4 pt-3 border-t text-xs text-muted-foreground">
            <div className="flex justify-between">
              <span>Last health check: {new Date(stats.last_check).toLocaleTimeString()}</span>
