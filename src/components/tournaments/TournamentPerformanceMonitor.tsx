...
export const TournamentPerformanceMonitor: React.FC<TournamentPerformanceMonitorProps> = ({
  tournamentId,
  onPerformanceIssue
}) => {
  ...
  useEffect(() => {
    setIsVisible(process.env.NODE_ENV === 'development' || window.location.search.includes('debug=true'));
  }, []);

  if (!isVisible) return null;

  const getPerformanceStatus = () => {
    if (metrics.errors.length > 0) return { status: 'error', color: 'destructive' };
    if (metrics.warnings.length > 0) return { status: 'warning', color: 'secondary' };
        ...
        {metrics.errors.length > 0 && (
          <div className="space-y-1">
            ...
            {metrics.errors.map((error, i) => (
              <div key={i} className="text-xs text-destructive opacity-80 truncate">
                {error}
              </div>
            ))}
          </div>
        ...
        {metrics.warnings.length > 0 && (
          <div className="space-y-1">
            ...
            {metrics.warnings.slice(-2).map((warning, i) => (
              <div key={i} className="text-xs text-orange-600 opacity-80 truncate">
                {warning}
              </div>
            ))}
          </div>
