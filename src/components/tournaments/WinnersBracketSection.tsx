...
export const WinnersBracketSection: React.FC<WinnersBracketSectionProps> = ({
  ...
  const winnersMatches = matches.filter(match => 
    match.bracket_type === 'winners' &&
    [1, 2, 3].includes(match.round_number)
  );
  
  // Only show matches that have players assigned (exclude empty matches)
  const validWinnersMatches = winnersMatches.filter(match => 
  ...
  const round1Matches = validWinnersMatches.filter(match => 
    match.round_number === 1
  ).sort((a, b) => a.match_number - b.match_number);
  
  const round2Matches = validWinnersMatches.filter(match => 
    match.round_number === 2
  ).sort((a, b) => a.match_number - b.match_number);
  
  const round3Matches = validWinnersMatches.filter(match => 
    match.round_number === 3
  ).sort((a, b) => a.match_number - b.match_number);

  const completedMatches = validWinnersMatches.filter(match => match.status === 'completed').length;
        ...
        <div className="grid gap-6">
          ...
          <div>
            <div className="flex items-center gap-2 mb-3">
              ...
              <span className="text-sm text-muted-foreground">
                {round1Matches.length} matches • {round1Matches.length * 2} → {round1Matches.length}
              </span>
            </div>
          ...
          {round2Matches.length > 0 && (
            <div>
              <div className="flex items-center gap-2 mb-3">
                ...
                <span className="text-sm text-muted-foreground">
                  {round2Matches.length} matches • {round2Matches.length * 2} → {round2Matches.length}
                </span>
              </div>
          ...
          {round3Matches.length > 0 && (
            <div>
              <div className="flex items-center gap-2 mb-3">
                ...
                <span className="text-sm text-muted-foreground">
                  {round3Matches.length} matches • {round3Matches.length * 2} → {round3Matches.length}
                </span>
              </div>
          ...
          <div className="mt-4 p-3 bg-primary/10 rounded-lg border border-primary/20">
            <p className="text-xs text-muted-foreground">
              ...
              Losers from Round 1 → Branch A, Round 2 → Branch B, Round 3 → Eliminated.
            </p>
          </div>
        </div>
      </CardContent>
