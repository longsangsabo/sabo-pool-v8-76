...
export const TournamentAutomationDashboard: React.FC<TournamentAutomationDashboardProps> = ({
  tournamentId,
  isAdmin = false,
  className
}) => {
  const { automationStatus, fixTournamentProgression, isFixing } = useTournamentAutomation(tournamentId);
        ...
        <CardContent className="space-y-4">
          <div className="grid grid-cols-2 lg:grid-cols-3 gap-4">
            <div className="text-center p-3 bg-green-50 rounded-lg border border-green-200">
              <div className="text-2xl font-bold text-green-600">{automationStatus.successCount}</div>
              <div className="text-sm text-muted-foreground">Successful Operations</div>
            </div>
            
            <div className="text-center p-3 bg-red-50 rounded-lg border border-red-200">
              <div className="text-2xl font-bold text-red-600">{automationStatus.errorCount}</div>
