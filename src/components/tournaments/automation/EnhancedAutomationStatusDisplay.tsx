...
export const EnhancedAutomationStatusDisplay: React.FC<EnhancedAutomationStatusDisplayProps> = ({
  status,
  className
}) => {
  const getStatusInfo = () => {
  ...
  const successRate = totalActions > 0 ? (status.successCount / totalActions) * 100 : 100;
      ...
      <CardHeader className="pb-3">
        <CardTitle className="flex items-center gap-2 text-sm">
          ...
          <Badge className={cn("gap-1", statusInfo.bgColor, statusInfo.color)}>
            <statusInfo.icon className={cn("w-3 h-3", status.isProcessing && "animate-spin")} />
            {statusInfo.text}
          </Badge>
        </CardTitle>
      </CardHeader>
      <CardContent className="space-y-3">
        ...
        <div className="grid grid-cols-3 gap-2 text-center">
          ...
          <div className="p-2 bg-blue-50 rounded border border-blue-200">
            <div className="text-lg font-bold text-blue-600">{status.pendingActions}</div>
        ...
        {status.isProcessing && (
          <div className="flex items-center justify-center gap-2 p-2 bg-blue-50 rounded border border-blue-200">
            ...
            <span className="text-sm text-blue-600">Automation đang chạy...</span>
          </div>
        )}
      </CardContent>
    </Card>
