...
export function SingleEliminationBracket({ tournamentId, isClubOwner = false, adminMode = false }: SingleEliminationBracketProps) {
                  ...
                  <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
                    {roundMatches.map((match) => (
                      <Card key={match.id} className="cursor-pointer hover:shadow-md transition-shadow"
                            onClick={() => isClubOwner && match.status === 'scheduled' && match.player1_id && match.player2_id ? setSelectedMatch(match) : null}>
                        <CardContent className="p-4">
                          ...
                          {isClubOwner && match.status === 'scheduled' && match.player1_id && match.player2_id && (
