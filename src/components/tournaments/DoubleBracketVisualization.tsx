...
import { Crown, Trophy, Users, GitBranch, Target, Settings } from 'lucide-react';
...
const MatchCard: React.FC<MatchCardProps> = ({ match, isClubOwner, onScoreSubmit }) => {
...
export const DoubleBracketVisualization: React.FC<DoubleBracketVisualizationProps> = ({
    ...
    return (
      <div className="flex items-center justify-center p-8">
        <div className="text-center">
          ...
          <p className="text-sm text-muted-foreground">Loading double elimination bracket...</p>
        </div>
      </div>
    );
  }
  ...
  const renderRound = (roundMatches: any[], roundNumber: number, title: string) => (
              ...
              .map(([roundNum, roundMatches]) => {
                ...
                return (
                  <div key={roundNum} className="flex flex-col items-center space-y-4">
                    ...
                    {branchA.length > 0 && (
                      <div className="space-y-2">
                        <Badge variant="outline" className="text-xs">Branch A</Badge>
                        {branchA.map((match) => (
                          <MatchCard 
                            key={match.id} 
                            match={match} 
                    ...
                    {branchB.length > 0 && (
                      <div className="space-y-2">
                        <Badge variant="outline" className="text-xs">Branch B</Badge>
                        {branchB.map((match) => (
                          <MatchCard 
                            key={match.id} 
                            match={match} 
      ...
      <Card>
        <CardContent className="pt-6">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-center">
            ...
            <div>
              <div className="text-2xl font-bold text-blue-600">{winnerMatches.length}</div>
              <div className="text-sm text-muted-foreground">Winner's Bracket</div>
            </div>
            <div>
              <div className="text-2xl font-bold text-orange-600">{loserMatches.length}</div>
              <div className="text-sm text-muted-foreground">Loser's Bracket</div>
            </div>
          </div>
        </CardContent>
      </Card>
