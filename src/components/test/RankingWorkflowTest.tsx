...
import { 
  TestTube, CheckCircle, XCircle, Clock, 
  Database, Shield, Bell, Zap 
} from 'lucide-react';
...
const RankingWorkflowTest = () => {
  ...
  const updateTestResult = (test: string, status: 'pass' | 'fail' | 'running', message: string, details?: any) => {
    setTestResults(prev => {
  ...
  const runComprehensiveTest = async () => {
    ...
    try {
      ...
      updateTestResult('rls', 'pass', `RLS policies working - ${userRequests?.length || 0} user requests found`);
    } catch (error: any) {
    ...
    try {
      ...
      if (clubProfile) {
        ...
        updateTestResult('club-access', 'pass', `Club owner access verified - ${clubRequests?.length || 0} club requests found`);
      } else {
        updateTestResult('club-access', 'pass', 'User is not a club owner - skipping club access test');
      }
    } catch (error: any) {
      updateTestResult('club-access', 'fail', `Club access error: ${error.message}`);
    }
    ...
    try {
      ...
      updateTestResult('notifications', 'pass', `Notification system working - ${notifications?.length || 0} rank notifications found`);
    } catch (error: any) {
      updateTestResult('notifications', 'fail', `Notification error: ${error.message}`);
    }

    // 5. Test Create Rank Request (if user is not club owner)
    updateTestResult('create-request', 'running', 'Testing rank request creation...');
    try {
      const { data: clubProfile } = await supabase
        .from('club_profiles')
        .select('id')
      ...
      if (clubProfile) {
        ...
        updateTestResult('create-request', 'pass', 'Rank request creation successful');
      } else {
        updateTestResult('create-request', 'pass', 'No other clubs found - skipping creation test');
      }
    } catch (error: any) {
      updateTestResult('create-request', 'fail', `Creation error: ${error.message}`);
    }
    ...
    try {
      ...
      const subscription = channel
        .on('postgres_changes', {
          ...
          updateTestResult('realtime', 'pass', 'Realtime subscription working');
        })
        .subscribe();

      // Test subscription status
      setTimeout(() => {
        updateTestResult('realtime', 'pass', 'Realtime subscription established');
    ...
    try {
      ...
      updateTestResult('functions', 'fail', `Functions error: ${error.message}`);
    }
    ...
    <Card className="w-full max-w-4xl mx-auto">
      ...
      <CardContent className="space-y-6">
        ...
        <div className="bg-blue-50 p-4 rounded-lg">
          <h4 className="font-semibold text-blue-900 mb-2">Workflow Steps Being Tested:</h4>
          <ol className="text-sm text-blue-800 space-y-1">
            <li>1. User tạo rank request → Trigger notification đến club owner</li>
            <li>2. Club owner xem requests trong dashboard → RLS security check</li>
