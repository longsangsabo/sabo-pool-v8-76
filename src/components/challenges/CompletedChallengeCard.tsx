      ...
      <CardContent className="p-4">
        ...
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="flex items-center gap-3">
            <Avatar className={`w-10 h-10 border-2 ${challengerWon ? 'border-yellow-400' : 'border-gray-300'}`}>
              <AvatarImage src={challenge.challenger_profile?.avatar_url} />
              <AvatarFallback className={challengerWon ? 'bg-yellow-100 text-yellow-700' : 'bg-gray-100'}>
                {challenge.challenger_profile?.full_name?.charAt(0) || 'C'}
              </AvatarFallback>
            </Avatar>
            <div>
              <div className={`font-semibold ${challengerWon ? 'text-yellow-700' : 'text-gray-800'}`}>
          ...
          <div className="flex items-center gap-3">
            <div className="text-right">
              <div className={`font-semibold ${opponentWon ? 'text-yellow-700' : 'text-gray-800'}`}>
                {opponentWon && '🏆 '}
                {challenge.opponent_profile?.full_name || 'Opponent'}
              </div>
              <div className="text-xs text-gray-500">Opponent</div>
            </div>
            <Avatar className={`w-10 h-10 border-2 ${opponentWon ? 'border-yellow-400' : 'border-gray-300'}`}>
              <AvatarImage src={challenge.opponent_profile?.avatar_url} />
              <AvatarFallback className={opponentWon ? 'bg-yellow-100 text-yellow-700' : 'bg-gray-100'}>
        ...
        <div className="text-center mb-3">
          <div className="text-sm font-medium text-green-700">
            🎉 Người thắng: {challenge.winner_profile?.full_name || 'Chưa xác định'}
          </div>
        </div>
