...
const RecentResultCard: React.FC<RecentResultCardProps> = ({ result, onView }) => {
      ...
      <CardContent className="p-4">
        ...
        <div className="grid grid-cols-5 gap-3 items-center mb-4">
          ...
          <div className="col-span-2 text-center">
            <div className="relative">
              <Avatar className={`w-12 h-12 mx-auto mb-2 ${
                result.winner === 'player1' ? 'ring-2 ring-green-500' : 'opacity-75'
              }`}>
            ...
            <div className={`text-sm font-semibold truncate ${
              result.winner === 'player1' ? 'text-green-700' : 'text-muted-foreground'
            }`}>
              {result.player1.name}
            </div>
            <div className="text-xs text-muted-foreground">{result.player1.rank}</div>
            {result.eloChanges && (
              <div className={`text-xs font-medium ${
                result.eloChanges.player1 > 0 ? 'text-green-600' : 'text-red-600'
              }`}>
                {result.eloChanges.player1 > 0 ? '+' : ''}{result.eloChanges.player1} ELO
              </div>
            )}
          </div>
          ...
          <div className="col-span-2 text-center">
            <div className="relative">
              <Avatar className={`w-12 h-12 mx-auto mb-2 ${
                result.winner === 'player2' ? 'ring-2 ring-green-500' : 'opacity-75'
              }`}>
            ...
            <div className={`text-sm font-semibold truncate ${
              result.winner === 'player2' ? 'text-green-700' : 'text-muted-foreground'
            }`}>
              {result.player2.name}
            </div>
            <div className="text-xs text-muted-foreground">{result.player2.rank}</div>
            {result.eloChanges && (
              <div className={`text-xs font-medium ${
                result.eloChanges.player2 > 0 ? 'text-green-600' : 'text-red-600'
              }`}>
                {result.eloChanges.player2 > 0 ? '+' : ''}{result.eloChanges.player2} ELO
              </div>
            )}
          </div>
        </div>
