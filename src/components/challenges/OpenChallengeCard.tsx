      ...
      <CardContent className="p-4">
        <div className="flex items-start justify-between mb-3">
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-2 text-xs text-muted-foreground">
              <span>Rank: {challenge.challenger_profile?.current_rank || 'K'}</span>
              <span>•</span>
              <span>{(challenge.challenger_profile as any)?.spa_points || 0} SPA</span>
            </div>
            </div>
          </div>
          ...
          <div className="flex items-center justify-between text-xs text-muted-foreground">
            ...
            {challenge.expires_at && (
              <span className={isExpired ? 'text-destructive' : ''}>
                Hết hạn {formatDistanceToNow(new Date(challenge.expires_at), { 
                  addSuffix: true, 
                  locale: vi 
                })}
              </span>
        ...
        >
          {isJoining ? (
            ...
            challenge.challenger_id === currentUser?.id ? 'Thách đấu của bạn' : 'Không thể tham gia'
          ) : isExpired ? (
            'Đã hết hạn'
          ) : (
            <>
