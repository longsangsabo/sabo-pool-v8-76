...
import { Search, Users, Trophy, Eye, MessageSquare, RefreshCw } from 'lucide-react';
        ...
        <div className="flex flex-col items-end space-y-2">
          ...
          {member.verification_date && (
            <p className="text-xs text-gray-500">
              Xác thực: {new Date(member.verification_date).toLocaleDateString('vi-VN')}
            </p>
          )}
...
const ClubMemberManagement = () => {
  ...
  const unverifiedCount = members.filter(m => m.verification_status === 'unverified').length;
      ...
      <Card>
        <CardContent className="pt-6">
          <div className="bg-red-50 border border-red-200 rounded-md p-4">
            <p className="text-red-800">Lỗi tải danh sách thành viên: {error}</p>
            <button 
              onClick={refetch}
