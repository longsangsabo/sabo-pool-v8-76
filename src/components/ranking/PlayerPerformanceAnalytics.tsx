...
export const PlayerPerformanceAnalytics: React.FC<PlayerPerformanceAnalyticsProps> = ({
  ...
  const [performanceData, setPerformanceData] = useState<PerformanceData>({
    strongestOpponents: [
      { name: "Nguyễn Văn A", elo: 2200, matchCount: 5, winRate: 40, lastPlayed: "2024-01-15" },
      { name: "Trần Thị B", elo: 2100, matchCount: 3, winRate: 33, lastPlayed: "2024-01-10" },
      { name: "Lê Văn C", elo: 1950, matchCount: 4, winRate: 50, lastPlayed: "2024-01-12" },
    ],
    weaknesses: [
      {
        ...
        severity: 'medium',
        suggestions: ["Luyện tập kiểm soát thời gian", "Cải thiện sự tập trung"]
      },
      {
        category: "Đối thủ cấp cao",
        description: "Khó khăn khi đối mặt với ELO >2000",
        severity: 'high',
        suggestions: ["Phân tích chiến thuật của cao thủ", "Tăng cường luyện tập"]
      }
    ],
    strengths: [
      {
                  ...
                  <div key={index} className="p-3 rounded-lg border bg-card">
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <Badge className={getLevelColor(strength.level)}>
                        {strength.level === 'exceptional' ? 'Xuất sắc' :
                         strength.level === 'excellent' ? 'Rất tốt' : 'Tốt'}
                      </Badge>
                    </div>
                  ...
                  <div key={index} className="p-3 rounded-lg border bg-card">
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <Badge className={getSeverityColor(weakness.severity)}>
                        {weakness.severity === 'high' ? 'Cao' :
                         weakness.severity === 'medium' ? 'Trung bình' : 'Thấp'}
                      </Badge>
                    </div>
                    <p className="text-sm text-muted-foreground mb-2">{weakness.description}</p>
                    <div className="space-y-1">
                      <p className="text-xs font-medium text-primary">Gợi ý cải thiện:</p>
          ...
          <Card>
            ...
            <CardContent>
              <div className="space-y-4">
                ...
                <Progress value={performanceData.consistency.score} className="h-2" />
                ...
                {performanceData.strongestOpponents.map((opponent, index) => (
                  <div key={index} className="flex items-center justify-between p-3 rounded-lg border bg-card">
                    ...
                    <div className="text-right">
                      <p className={`font-bold ${opponent.winRate >= 50 ? 'text-green-600' : 'text-red-600'}`}>
                        {opponent.winRate}%
                      </p>
                      <p className="text-xs text-muted-foreground">
                        {new Date(opponent.lastPlayed).toLocaleDateString('vi-VN')}
                      </p>
                    </div>
                  </div>
                ))}
              ...
              <CardContent className="space-y-4">
                <div className="space-y-2">
                  <div className="flex justify-between">
                    ...
                    <span className="font-bold text-primary">{performanceData.playingPatterns.bestTimeOfDay}</span>
                  </div>
                  <div className="flex justify-between">
                    <span>Ngày tốt nhất:</span>
                    <span className="font-bold text-primary">{performanceData.playingPatterns.bestDayOfWeek}</span>
                  </div>
                  <div className="flex justify-between">
                    <span>Thời lượng TB:</span>
                    <span className="font-bold text-primary">{performanceData.playingPatterns.averageMatchDuration} phút</span>
                  </div>
                </div>
              </CardContent>
            </Card>
            ...
            <Card>
              ...
              <CardContent className="space-y-4">
                <div className="text-center space-y-2">
                  ...
                  <p className="text-sm text-muted-foreground">Khả năng thăng hạng</p>
                </div>
                <div className="space-y-2">
                  <div className="flex justify-between">
                    <span>Ước tính số trận:</span>
                    <span className="font-bold">{performanceData.predictions.estimatedGamesToNextRank}</span>
                  </div>
                  <div className="flex justify-between">
                    <span>ELO dự báo (30 ngày):</span>
                    <span className="font-bold text-green-600">{performanceData.predictions.predictedEloIn30Days}</span>
                  </div>
                </div>
              </CardContent>
            </Card>
            ...
            <Card>
              ...
              <CardContent>
                <div className="space-y-3">
                  {performanceData.predictions.riskFactors.map((risk, index) => (
