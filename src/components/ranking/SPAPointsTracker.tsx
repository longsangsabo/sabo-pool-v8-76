...
import { Trophy, Swords, CalendarCheck, Video, TrendingDown } from 'lucide-react';
...
export const SPAPointsTracker: React.FC<SPAPointsTrackerProps> = ({
  ...
  const getSourceIcon = (type: string) => {
    switch (type) {
      ...
      case 'decay': return <TrendingDown className="h-4 w-4" />;
      default: return null;
    }
  };
  ...
  const getSourceColor = (type: string) => {
    switch (type) {
      ...
      case 'decay': return 'text-gray-600 bg-gray-100';
  ...
  const getSourceName = (type: string) => {
    switch (type) {
      ...
      case 'challenge': return 'Thách đấu';
      case 'checkin': return 'Check-in';
      case 'video': return 'Video';
      case 'decay': return 'Giảm điểm';
      default: return type;
    }
  };

  const totalPositivePoints = Object.values(pointsBreakdown).reduce((sum, points) => sum + Math.max(0, points), 0);
        ...
        <CardContent>
          ...
          <div className="mt-6 p-4 bg-muted rounded-lg">
            <div className="flex justify-between items-center mb-2">
              ...
              <span className="text-sm text-muted-foreground">{dailyChallengeCount}/2</span>
            </div>
            <Progress value={(dailyChallengeCount / 2) * 100} className="h-2" />
            <p className="text-xs text-muted-foreground mt-1">
              {dailyChallengeCount >= 2 ? 'Đã đạt giới hạn - thách đấu tiếp theo chỉ nhận 30% điểm' : 'Còn lại để nhận 100% điểm'}
            </p>
          </div>
        </CardContent>
      </Card>
        ...
        <CardContent>
          <div className="space-y-4">
            {Object.entries(pointsBreakdown).map(([source, points]) => {
              ...
              const percentage = totalPositivePoints > 0 ? (Math.max(0, points) / totalPositivePoints) * 100 : 0;
                ...
                <div key={source} className="space-y-2">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="font-medium">{getSourceName(source)}</span>
                    </div>
                    <span className={`font-semibold ${points >= 0 ? 'text-green-600' : 'text-red-600'}`}>
                      {points >= 0 ? '+' : ''}{points.toLocaleString('vi-VN')}
                    </span>
                  </div>
                  {points > 0 && (
          ...
          <div className="space-y-3">
            {recentEntries.slice(0, 5).map((entry) => (
              <div key={entry.id} className="flex items-center justify-between p-3 rounded-lg border">
                <div className="flex items-center gap-3">
                  <span className={`p-2 rounded ${getSourceColor(entry.source_type)}`}>
            ...
            {recentEntries.length === 0 && (
              <div className="text-center py-8 text-muted-foreground">
                ...
                <p className="text-sm">Tham gia giải đấu hoặc thách đấu để bắt đầu kiếm SPA points!</p>
              </div>
            )}
          </div>
        </CardContent>
