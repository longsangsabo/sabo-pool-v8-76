...
import { InfoIcon, StarIcon, GiftIcon, Trophy, Swords, CalendarCheck, Video, TrendingDown, History, Calendar } from 'lucide-react';
...
export const SPAPointsCard: React.FC<SPAPointsCardProps> = ({
  ...
  const [recentTransactions, setRecentTransactions] = useState<SPATransaction[]>([]);
  ...
  useEffect(() => {
    ...
    const fetchTransactions = async () => {
      console.log('🔍 [SPAPointsCard] Fetching SPA transactions for user:', user.id);
      setIsLoading(true);
      try {
        ...
        if (error) {
          console.error('❌ [SPAPointsCard] Error fetching transactions:', error);
          throw error;
        }
        
        console.log('✅ [SPAPointsCard] Fetched', data?.length || 0, 'transactions');
        ...
        console.error('❌ [SPAPointsCard] Failed to fetch SPA transactions:', error);
      } finally {
        setIsLoading(false);
      }
    };
    ...
    const subscription = supabase
      ...
      .on(
        ...
        (payload) => {
          console.log('🆕 [SPAPointsCard] New SPA transaction received:', payload);
      ...
      <CardContent className="space-y-4">
        ...
        <div className="grid grid-cols-3 gap-3 mb-4">
          <div className="text-center p-3 bg-green-50 rounded-lg">
            <div className="text-lg font-bold text-green-600">{points.toLocaleString('vi-VN')}</div>
            <div className="text-xs text-green-600">Số dư hiện tại</div>
          </div>
          <div className="text-center p-3 bg-blue-50 rounded-lg">
            <div className="text-lg font-bold text-blue-600">{totalEarned.toLocaleString('vi-VN')}</div>
            <div className="text-xs text-blue-600">Tổng đã kiếm</div>
          </div>
          <div className="text-center p-3 bg-orange-50 rounded-lg">
            <div className="text-lg font-bold text-orange-600">{totalSpent.toLocaleString('vi-VN')}</div>
          ...
          <div className="grid grid-cols-2 gap-4 mb-4">
            {weeklyRank && (
              <div className="text-center p-3 bg-purple-50 rounded-lg">
                <div className="text-lg font-bold text-purple-600">#{weeklyRank}</div>
                <div className="text-xs text-purple-600">Xếp hạng tuần</div>
              </div>
            )}
            {monthlyRank && (
              <div className="text-center p-3 bg-indigo-50 rounded-lg">
                <div className="text-lg font-bold text-indigo-600">#{monthlyRank}</div>
        ...
        <div className="space-y-2">
          <div className="flex items-center justify-between">
            <h4 className="text-sm font-medium flex items-center gap-2">
              ...
              Biến động {!isLoading && `(${recentTransactions.slice(0, 5).length})`}
            </h4>
          ...
          {isLoading ? (
            ...
            <div className="space-y-1">
              {recentTransactions.slice(0, showHistory ? 5 : 3).map((transaction) => (
                <div 
                  key={transaction.id}
                ...
                >
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="font-medium truncate">{transaction.description}</p>
                       <div className="flex items-center gap-2 text-xs text-gray-500">
                        <span>{getTransactionTypeName(transaction.source_type)}</span>
                        <span>•</span>
                  ...
                  <div className={`font-bold text-sm ${transaction.points_earned >= 0 ? 'text-green-600' : 'text-red-600'}`}>
            ...
            <div className="text-center py-4 text-gray-500">
              ...
              <p className="text-xs">Tham gia thách đấu để bắt đầu kiếm điểm!</p>
            </div>
          )}
        </div>
