...
interface RankBadgeProps {
  rank: {
    ...
    name: string;
    level: number;
  } | string | number; // Support legacy formats
  size?: 'sm' | 'md' | 'lg';
  showTooltip?: boolean;
}
...
export const RankBadge: React.FC<RankBadgeProps> = ({ 
  ...
  const normalizeRank = () => {
    if (typeof rank === 'string') {
      ...
      if (rank.code && (rank.code.includes('K') || rank.code.includes('I') || rank.code.includes('H') || 
                       rank.code.includes('G') || rank.code.includes('F') || rank.code.includes('E'))) {
