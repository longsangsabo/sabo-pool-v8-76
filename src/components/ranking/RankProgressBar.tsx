  ...
  if (!current) {
    return (
      <div className="bg-card p-4 rounded-lg border">
        <p className="text-center text-muted-foreground">Chưa có thông tin hạng</p>
      </div>
    );
  }
      ...
      {next ? (
        <>
          ...
          <div className="flex justify-between text-sm text-muted-foreground">
            <span>Cần thêm {pointsToNext.toLocaleString('vi-VN')} SPA points</span>
            <span>{pointsNeeded.toLocaleString('vi-VN')} points để thăng hạng</span>
          </div>
        </>
      ) : (
