...
export const TournamentStatsRealtime: React.FC<TournamentStatsRealtimeProps> = ({ 
  tournament 
}) => {
  ...
  const participationPercentage = tournament.max_participants > 0 
    ? Math.round((stats.current_participants / tournament.max_participants) * 100) 
    : 0;

  const remainingSlots = tournament.max_participants - stats.current_participants;
      ...
      <CardContent>
        <div className="grid grid-cols-4 gap-4">
          ...
          <div className="text-center p-4 bg-gradient-to-br from-yellow-50 to-yellow-100 rounded-lg border">
            ...
            <div className="text-xs text-yellow-700 font-medium">Chờ xác nhận</div>
          </div>
        ...
        <div className="mt-4">
          ...
          <div className="flex justify-between text-xs text-gray-500 mt-1">
            <span>0%</span>
            <span className="font-medium text-gray-700">{participationPercentage}% hoàn thành</span>
            <span>100%</span>
          </div>
        </div>
        ...
        <div className="flex items-center justify-between mt-4 p-3 bg-gray-50 rounded-lg">
          ...
          <div className="text-xs text-gray-500">
            {remainingSlots > 0 ? (
              <span className="text-green-600 font-medium">🟢 Còn chỗ trống</span>
            ) : (
