...
export const TournamentDataSyncButton: React.FC<TournamentDataSyncButtonProps> = ({
  tournamentId,
  tournamentName,
  onSyncComplete
}) => {
  ...
  const handleSyncTournament = async (action: 'generate_bracket' | 'update_matches' | 'sync_all' = 'sync_all') => {
    if (!tournamentId) {
    ...
    try {
      ...
      const { data, error } = await supabase.functions.invoke('tournament-data-sync', {
        body: {
  ...
  const getSyncStatusBadge = () => {
    ...
    if (syncResult.success) {
      return <Badge className="bg-green-600 text-white">Đồng bộ thành công</Badge>;
    } else {
                ...
                <div className="mt-2">
                  ...
                  <ul className="list-disc list-inside ml-4">
                    {syncResult.operations.map((op: any, index: number) => (
                      <li key={index}>
                        {op.action === 'generate_bracket' && `Tạo bảng đấu (${op.participant_count} người chơi)`}
                        {op.action === 'update_matches' && `Cập nhật ${op.updated_matches} trận đấu`}
                        {op.action === 'status_update' && `Cập nhật trạng thái: ${op.old_status} → ${op.new_status}`}
                      </li>
                    ))}
                  </ul>
                </div>
    ...
    <Card>
      ...
      <CardContent className="space-y-4">
        <div className="text-sm text-muted-foreground">
          Đồng bộ dữ liệu giải đấu với Tournament Management Hub, tạo bảng đấu và cập nhật kết quả trận đấu.
          {lastSync && (
            <div className="mt-2">
