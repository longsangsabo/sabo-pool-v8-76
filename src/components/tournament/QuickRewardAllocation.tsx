...
import { SPA_TOURNAMENT_REWARDS, TOURNAMENT_ELO_REWARDS } from '@/utils/eloConstants';
...
const PRIZE_TEMPLATES = {
  ...
  'even-distribution': { 1: 0.25, 2: 0.20, 3: 0.15, 4: 0.15, 8: 0.15, 16: 0.1 }
};
...
export const QuickRewardAllocation: React.FC<QuickRewardAllocationProps> = ({
  ...
  const [tournamentType, setTournamentType] = useState<'normal' | 'season' | 'open'>('normal');
  const [positionCount, setPositionCount] = useState<3 | 4 | 8 | 16>(3);
  const [prizeTemplate, setPrizeTemplate] = useState<keyof typeof PRIZE_TEMPLATES>('50-30-20');
  ...
  const calculatePreview = () => {
    ...
    const newPreview: QuickAllocationPreview[] = positions.map(pos => {
      ...
      const baseSpaPoints = SPA_TOURNAMENT_REWARDS[selectedRank]?.[tournamentPosition] || 0;
      ...
      const cashPercentage = template[pos.position as keyof typeof template] || 0;
  ...
  React.useEffect(() => {
    calculatePreview();
  }, [selectedRank, tournamentType, positionCount, prizeTemplate, totalPrizePool]);
          ...
          <div className="space-y-4">
            <Card>
              ...
              <CardContent className="space-y-4">
                <div>
                  ...
                  <Select value={selectedRank} onValueChange={(value: RankCode) => setSelectedRank(value)}>
                    <SelectTrigger>
                      <SelectValue />
                    </SelectTrigger>
                    <SelectContent>
                      ...
                      <SelectItem value="I+">Hạng I+ (Trung cấp cao)</SelectItem>
                      ...
                      <SelectItem value="E+">Hạng E+ (Chuyên nghiệp cao)</SelectItem>
                    </SelectContent>
                  </Select>
                </div>
                ...
                <div>
                  ...
                  <Select value={tournamentType} onValueChange={(value: 'normal' | 'season' | 'open') => setTournamentType(value)}>
                    <SelectTrigger>
                      <SelectValue />
                    </SelectTrigger>
                    <SelectContent>
                      <SelectItem value="normal">Thường (1x điểm)</SelectItem>
                      <SelectItem value="season">Mùa giải (1.5x điểm)</SelectItem>
                ...
                <div>
                  ...
                  <Select value={positionCount.toString()} onValueChange={(value) => setPositionCount(parseInt(value) as 3 | 4 | 8 | 16)}>
                    <SelectTrigger>
                      <SelectValue />
                    </SelectTrigger>
                    <SelectContent>
                      ...
                      <SelectItem value="16">16 vị trí (Giải chuyên nghiệp)</SelectItem>
                    </SelectContent>
                  </Select>
                </div>
                ...
                <div>
                  ...
                  <Select value={prizeTemplate} onValueChange={(value: keyof typeof PRIZE_TEMPLATES) => setPrizeTemplate(value)}>
                    <SelectTrigger>
                      <SelectValue />
                    </SelectTrigger>
                    <SelectContent>
                      <SelectItem value="50-30-20">50-30-20% (3 vị trí)</SelectItem>
                      <SelectItem value="40-30-20-10">40-30-20-10% (4 vị trí)</SelectItem>
                      <SelectItem value="40-25-15-10-5-5">40-25-15-10-5-5% (6 vị trí)</SelectItem>
                      <SelectItem value="even-distribution">Phân bổ đều (Nhiều vị trí)</SelectItem>
                    </SelectContent>
                  </Select>
                </div>
              </CardContent>
            ...
            <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
              ...
              <div className="space-y-1 text-sm text-yellow-700">
                <div>Tổng tiền thưởng: {totalPrizePool.toLocaleString()} VND</div>
                <div>Đã phân bổ: {getTotalCashDistributed().toLocaleString()} VND</div>
                <div>Còn lại: {(totalPrizePool - getTotalCashDistributed()).toLocaleString()} VND</div>
              </div>
            </div>
          </div>
            ...
            <Card>
              ...
              <CardContent>
                <div className="space-y-3 max-h-96 overflow-y-auto">
                  {preview.map((item) => (
                    <div key={item.position} className="border rounded-lg p-3 bg-gray-50">
                      ...
                      <div className="grid grid-cols-2 gap-2 text-sm">
                        <div>
                          ...
                          <span className="ml-1 font-medium text-blue-600">+{item.eloPoints}</span>
                        </div>
                        <div>
                          <span className="text-gray-600">SPA:</span>
                          <span className="ml-1 font-medium text-green-600">+{item.spaPoints}</span>
                        </div>
                        ...
                        {item.items.length > 0 && (
                          <div className="col-span-2">
                            ...
                            <span className="ml-1 text-purple-600">{item.items.join(', ')}</span>
                          </div>
                        )}
                      </div>
                    </div>
          ...
          <div className="text-sm text-gray-600">
            {currentAllocations.length > 0 && (
              <span className="text-amber-600">
                ⚠️ Thao tác này sẽ thay thế {currentAllocations.length} vị trí hiện tại
              </span>
            )}
          </div>
