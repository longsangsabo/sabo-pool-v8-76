...
const TournamentLifecycleManager = ({ tournamentId }: { tournamentId: string }) => {
  const { user } = useAuth();
  const { tournament, registrations, hasBracket, isLoading, generateBracket, startTournament } = useTournamentManagement(tournamentId);
  ...
  const generateLifecycleStages = () => {
    ...
    const stages: TournamentLifecycleStage[] = [
      ...
      },
      {
        ...
        canProgress: hasBracket && registrations.length >= tournament.current_participants * 0.8,
  ...
  const fetchTournamentStats = async () => {
    try {
      ...
      const stats: TournamentStats = {
        ...
        checkedInPlayers: registrations.filter(r => r.registration_status === 'confirmed').length || 0
      };
            ...
            <TabsContent value="workflow" className="space-y-6">
              <div className="space-y-4">
                ...
                <p className="text-muted-foreground">
                  Hệ thống đang tự động hóa toàn bộ quy trình giải đấu từ đăng ký đến trao giải.
                </p>
              </div>
            </TabsContent>
          </Tabs>
