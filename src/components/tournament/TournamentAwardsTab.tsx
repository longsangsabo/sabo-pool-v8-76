...
export const TournamentAwardsTab: React.FC<TournamentAwardsTabProps> = ({ tournament }) => {
  ...
  const getAwardDetails = (): AwardDetail[] => {
    ...
    const hasPrizeData = Object.keys(prizeDistribution).length > 0 || 
    ...
    for (let position = 1; position <= positionsToShow; position++) {
      ...
      if (Object.keys(eloConfig).length > 0 || !hasPrizeData) {
        ...
        else if (position === 2) eloPoints = Math.round(kFactor * multiplier * 0.8);
        else if (position === 3) eloPoints = Math.round(kFactor * multiplier * 0.6);
        else if (position <= 8) eloPoints = Math.round(kFactor * multiplier * 0.4);
        else eloPoints = Math.round(kFactor * multiplier * 0.2);
      }

      // Get physical prizes
      const physicalPrize = physicalPrizes.find((p: any) => p.position === position);
                ...
                {award.spaPoints > 0 && (
                  <div className="flex items-center gap-2">
                    <div className="w-8 h-8 bg-blue-100 rounded-full flex items-center justify-center">
                      <span className="text-blue-600 text-xs font-semibold">SPA</span>
                    </div>
                    <div>
                ...
                {award.eloPoints > 0 && (
                  <div className="flex items-center gap-2">
                    <div className="w-8 h-8 bg-purple-100 rounded-full flex items-center justify-center">
                      <span className="text-purple-600 text-xs font-semibold">ELO</span>
                    </div>
                    <div>
