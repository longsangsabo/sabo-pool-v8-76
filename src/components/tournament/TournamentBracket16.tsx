...
import { Trophy, Users, Target, Hash, Clock, Gavel, Medal, Star, Crown } from 'lucide-react';
...
const TournamentBracket16: React.FC<TournamentBracket16Props> = ({
  ...
  const MatchCard: React.FC<{ match: Match; isFinal?: boolean; isBronze?: boolean }> = ({ 
    match, 
    isFinal = false, 
    isBronze = false 
  }) => {
      ...
      >
        <CardContent className="p-0">
          <Badge 
            ...
            `}
          >
            {isFinal ? 'CHUNG KẾT' : isBronze ? 'TRANH HẠNG 3' : `Trận ${match.matchNumber}`}
          </Badge>
  ...
  const InfoCard: React.FC<{ icon: React.ReactNode; title: string; value: string }> = ({ 
    icon, 
    title, 
    value 
  }) => (
    ...
    <div className="max-w-7xl mx-auto p-6 space-y-8">
      ...
      <Card className="bg-gradient-primary text-primary-foreground">
        <CardContent className="p-8 text-center">
          ...
          <p className="text-lg opacity-90">16 Người chơi - Loại trực tiếp (Single Elimination)</p>
        </CardContent>
      </Card>
      ...
      <Card className="border-primary/20">
        <CardContent className="p-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            <InfoCard icon={<Users className="w-5 h-5" />} title="Số người chơi" value="16" />
            <InfoCard icon={<Target className="w-5 h-5" />} title="Loại giải" value="Loại trực tiếp" />
            <InfoCard icon={<Hash className="w-5 h-5" />} title="Tổng số vòng" value="4 vòng" />
            <InfoCard icon={<Trophy className="w-5 h-5" />} title="Tổng số trận" value="16 trận" />
          </div>
        </CardContent>
      </Card>
      ...
      <Card>
        <CardContent className="p-6">
          <h3 className="text-xl font-bold text-foreground mb-6 text-center">🏆 SƠ ĐỒ GIẢI ĐẤU</h3>
      ...
      <Card className="border-primary/20">
        <CardContent className="p-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
            <InfoCard icon={<Crown className="w-5 h-5 text-yellow-500" />} title="🥇 Nhất giải" value="Vô địch" />
            <InfoCard icon={<Medal className="w-5 h-5 text-gray-400" />} title="🥈 Nhì giải" value="Á quân" />
            <InfoCard icon={<Medal className="w-5 h-5 text-amber-600" />} title="🥉 Ba giải" value="Thắng tranh hạng 3" />
            <InfoCard icon={<Star className="w-5 h-5 text-gray-500" />} title="4️⃣ Hạng 4" value="Thua tranh hạng 3" />
            <InfoCard icon={<Trophy className="w-5 h-5 text-green-600" />} title="🏆 Top 8" value="Vào tứ kết" />
          </div>
        </CardContent>
      </Card>
      ...
      <Card className="border-primary/20">
        <CardContent className="p-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            <InfoCard icon={<Target className="w-5 h-5" />} title="🎱 Môn thi đấu" value="Bi-a 8 bi" />
            <InfoCard icon={<Clock className="w-5 h-5" />} title="⏱️ Thời gian trận" value="45 phút/trận" />
            <InfoCard icon={<Trophy className="w-5 h-5" />} title="🏁 Điều kiện thắng" value="Best of 3 games" />
            <InfoCard icon={<Gavel className="w-5 h-5" />} title="⚖️ Trọng tài" value="Có trọng tài chính thức" />
          </div>
        </CardContent>
      </Card>
    </div>
