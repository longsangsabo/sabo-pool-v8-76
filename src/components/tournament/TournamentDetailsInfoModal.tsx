...
const TournamentDetailsInfoModal: React.FC<TournamentDetailsInfoModalProps> = ({
  ...
  const legacyTournament = tournament.first_prize !== undefined 
    ? tournament 
    : TournamentAdapter.toLegacy(tournament);
    
  const prizeDistribution = formatPrizeDistribution(legacyTournament);
  const totalPrizePool = tournament.prize_pool || calculateTotalPrizePool(legacyTournament);

  const getStatusBadge = (status: string) => {
    const statusConfig = {
      'upcoming': { label: 'Sắp diễn ra', className: 'bg-blue-100 text-blue-800' },
      'registration_open': { label: 'Đang mở đăng ký', className: 'bg-green-100 text-green-800' },
      'registration_closed': { label: 'Đã đóng đăng ký', className: 'bg-yellow-100 text-yellow-800' },
      'ongoing': { label: 'Đang diễn ra', className: 'bg-orange-100 text-orange-800' },
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <p className="font-medium">{tournament.max_participants || 'Không giới hạn'} người</p>
              </div>
            </div>
            ...
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <p className="font-medium">{formatCurrency(tournament.entry_fee || 0)}</p>
              </div>
            </div>
            ...
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <p className="font-medium">{tournament.venue_address || 'Chưa xác định'}</p>
              </div>
            </div>
          </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <p className="font-medium capitalize">
                  {tournament.tournament_type?.replace('_', ' ') || 'Single Elimination'}
                </p>
              </div>
              <div>
                <p className="text-sm text-muted-foreground">Cấp độ</p>
                <p className="font-medium">
                  {tournament.tier_level ? `Cấp ${tournament.tier_level}` : 'Mở rộng'}
                </p>
              </div>
            </div>
          </div>
