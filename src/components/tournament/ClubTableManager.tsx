...
const ClubTableManager = ({ onTablesInitialized }: ClubTableManagerProps) => {
  ...
  const fetchTables = async () => {
    ...
    try {
      ...
      console.log('Table fetching temporarily disabled until database types are updated');
      setTables([]);
    } catch (error) {
  ...
  const getStatusBadge = (status: string) => {
    const statusConfig = {
      ...
      occupied: { label: '🔴', variant: 'destructive' as const, tooltip: 'Đang sử dụng' },
      maintenance: { label: '🔧', variant: 'secondary' as const, tooltip: 'Bảo trì' }
    };

    const config = statusConfig[status as keyof typeof statusConfig] || 
                  { label: '❔', variant: 'outline' as const, tooltip: status };
      ...
      <CardContent className="space-y-6">
        {tables.length === 0 ? (
          <div className="text-center py-8 space-y-4">
            ...
            <div>
              <h3 className="text-lg font-semibold">Chưa có bàn nào được khởi tạo</h3>
          ...
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-2 p-3 bg-muted/50 rounded-lg">
              <div className="text-center">
                <div className="text-lg font-bold text-primary">{tables.length}</div>
                <div className="text-xs text-muted-foreground">Tổng số bàn</div>
              </div>
              <div className="text-center">
                <div className="text-lg font-bold text-green-600">{tables.filter(t => t.status === 'available').length}</div>
              ...
              <div className="text-center">
                <div className="text-lg font-bold text-yellow-600">{tables.filter(t => t.status === 'maintenance').length}</div>
                ...
                <Card key={table.id} className="ultra-compact-table-card">
                  <CardContent className="p-2">
                    ...
                    <div className="flex items-center justify-between mb-1">
                      <span className="font-semibold text-sm">Bàn {table.table_number}</span>
                    ...
                    <div className="space-y-0.5 text-xs text-muted-foreground">
                      ...
                      {table.last_used_at && (
                        <div className="truncate" title={`Sử dụng lần cuối: ${new Date(table.last_used_at).toLocaleString('vi-VN')}`}>
                          🕙 {new Date(table.last_used_at).toLocaleDateString('vi-VN', { 
                            month: 'short', 
                            day: 'numeric',
                            hour: '2-digit',
                            minute: '2-digit'
