...
export const TournamentAutomationTest: React.FC<TournamentAutomationTestProps> = ({
  tournamentId,
  tournament
}) => {
  ...
  const simulateMatchCompletion = async () => {
    ...
    try {
      ...
      if (!incompleteMatches || incompleteMatches.length === 0) {
        setTestResults(['⚠️ Không có trận đấu nào để test (tất cả đã hoàn thành)']);
      ...
      setTestResults(prev => [...prev, `🎯 Testing match: Round ${testMatch.round_number}, Match ${testMatch.match_number}`]);
      ...
      setTestResults(prev => [...prev, '⚡ Database trigger should automatically advance winner...']);
      ...
      setTimeout(async () => {
        ...
        if (nextError) {
          setTestResults(prev => [...prev, `❌ Error checking next round: ${nextError.message}`]);
        } else {
          const updatedMatch = nextMatches?.find(m => 
            m.player1_id === winnerId || m.player2_id === winnerId
          );
          
          if (updatedMatch) {
            setTestResults(prev => [...prev, '🎉 SUCCESS: Winner automatically advanced to next round!']);
            setTestResults(prev => [...prev, `📍 Found in Round ${updatedMatch.round_number}, Match ${updatedMatch.match_number}`]);
            toast.success('🎉 Automation test thành công!');
          } else {
            setTestResults(prev => [...prev, '⚠️ Winner not found in next round - automation may not be working']);
      ...
      <CardContent>
        <div className="space-y-4">
          <Alert>
            ...
            <AlertDescription>
              Tính năng này sẽ test automation system bằng cách hoàn thành một trận đấu ngẫu nhiên 
              và kiểm tra xem winner có được tự động chuyển sang vòng tiếp theo không.
            </AlertDescription>
          </Alert>
              ...
              <div className="bg-muted p-4 rounded-lg font-mono text-sm space-y-1">
                {testResults.map((result, index) => (
                  <div key={index} className="flex items-center gap-2">
                    {result.includes('✅') && <CheckCircle className="h-4 w-4 text-green-600" />}
                    {result.includes('❌') && <AlertTriangle className="h-4 w-4 text-red-600" />}
                    {result.includes('⚠️') && <AlertTriangle className="h-4 w-4 text-orange-600" />}
                    <span>{result}</span>
                  </div>
                ))}
              </div>
          ...
          <div className="pt-4 border-t">
            ...
            <div className="text-sm text-muted-foreground space-y-1">
              ...
              <div>3. Database trigger tự động chạy advance_tournament_winner()</div>
              <div>4. Kiểm tra xem winner có được chuyển sang vòng tiếp theo</div>
            </div>
          </div>
        </div>
      </CardContent>
