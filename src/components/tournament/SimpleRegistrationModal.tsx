...
export const SimpleRegistrationModal: React.FC<SimpleRegistrationModalProps> = ({
  tournament,
  isOpen,
  onClose,
  onSuccess
  ...
  const handleCashRegistration = async () => {
    ...
    try {
      console.log("💵 Processing cash registration for tournament:", tournament.id);
      ...
      <DialogContent className="max-w-md">
        ...
        <div className="space-y-4">
          ...
          <div className="bg-blue-50 p-4 rounded-lg">
            <h3 className="font-medium text-blue-900 mb-2">Thông tin giải đấu</h3>
            <div className="space-y-1 text-sm text-blue-800">
              <div>📅 Bắt đầu: {formatDate(tournament.tournament_start)}</div>
              <div>📍 Địa điểm: {tournament.location || 'Chưa có thông tin'}</div>
              <div>💰 Phí tham gia: {tournament.entry_fee?.toLocaleString('vi-VN') || '0'} VNĐ</div>
              <div>👥 Số người: {tournament.current_participants || 0}/{tournament.max_participants}</div>
            </div>
          </div>
