...
export const EnhancedBracketViewer: React.FC<EnhancedBracketViewerProps> = ({
  ...
  const loadBracketData = async () => {
    try {
      ...
      const transformedMatches = (matchesData || []).map((match: any) => ({
        ...
        status: match.status as 'scheduled' | 'ongoing' | 'completed' | 'cancelled',
  ...
  const renderMatch = (match: MatchData) => {
    ...
    return (
      <Card key={match.id} className="w-full max-w-sm mb-4 hover:shadow-md transition-shadow">
                ...
                {!seed.is_bye ? (
                  <>
                    <Avatar className="h-10 w-10">
                      ...
                      <AvatarFallback>
                        {seed.player?.full_name?.[0] || seed.player?.display_name?.[0] || '?'}
                      </AvatarFallback>
                    </Avatar>
                    <div className="flex-1 min-w-0">
                      <p className="font-medium truncate">
                        {seed.player?.full_name || seed.player?.display_name || 'Unknown Player'}
                      </p>
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-xs text-muted-foreground">Tự động vào vòng sau</p>
                  </div>
                )}
              </div>
            </CardContent>
    ...
    <div className="space-y-6">
        ...
        <CardContent>
          {bracketData && (
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-4">
              <div className="text-center">
                <div className="text-2xl font-bold text-primary">{bracketData.participant_count}</div>
                <div className="text-sm text-muted-foreground">Người tham gia</div>
              </div>
              <div className="text-center">
                <div className="text-2xl font-bold text-blue-600">{bracketData.bracket_size}</div>
                <div className="text-sm text-muted-foreground">Kích thước bracket</div>
              </div>
              <div className="text-center">
                <div className="text-2xl font-bold text-green-600">{bracketData.rounds}</div>
          ...
          <div className="flex items-center gap-4 text-sm text-muted-foreground">
            <div className="flex items-center gap-1">
              ...
              Phương thức: {bracketData?.seeding_method === 'elo_ranking' ? 'ELO Ranking' : 
                          bracketData?.seeding_method === 'registration_order' ? 'Thứ tự đăng ký' : 'Ngẫu nhiên'}
            </div>
            {bracketData && (
              <div className="flex items-center gap-1">
                <Calendar className="h-4 w-4" />
                Tạo lúc: {new Date(bracketData.generated_at).toLocaleDateString('vi-VN')}
              </div>
            )}
          </div>
        </CardContent>
      ...
      <Tabs defaultValue="matches" className="w-full">
        ...
        <TabsContent value="seeding" className="space-y-4">
          {seeding.length > 0 ? renderSeeding() : (
            <div className="text-center py-8 text-muted-foreground">
