...
import { RefreshCw, Trophy, Users, Settings, Play, CheckCircle } from 'lucide-react';
        ...
        <CardContent>
          ...
          <div className="grid grid-cols-4 gap-4 mt-4 text-sm">
            <div className="text-center">
              <div className="text-2xl font-bold text-primary">{participantCount}</div>
            ...
            <div className="text-center">
              <div className="text-2xl font-bold text-green-600">{seeding.length}</div>
          ...
          {canManage && (
            <div className="flex gap-2 mt-4 pt-4 border-t">
              ...
              {bracketData && tournament?.status !== 'ongoing' && tournament?.status !== 'completed' && (
                <Button
                    ...
                    {seeding.map((seed) => (
                      <div key={seed.id} className="flex items-center justify-between p-2 border rounded">
                        <span className="font-medium">
                          #{seed.seed_position} - {seed.player?.full_name || seed.player?.display_name || 'TBD'}
                        </span>
                ...
                {bracketData ? (
                  <div className="space-y-3">
                    <div className="flex justify-between">
                      ...
                      <span className="font-medium">{bracketData.bracket_type}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-muted-foreground">Tổng số vòng:</span>
                      <span className="font-medium">{bracketData.total_rounds}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-muted-foreground">Vòng hiện tại:</span>
                      <span className="font-medium">{bracketData.current_round}</span>
                    </div>
                    <div className="flex justify-between">
                    ...
                    <div className="flex justify-between">
                      ...
                      <span className="text-sm">
                        {new Date(bracketData.created_at).toLocaleString('vi-VN')}
                      </span>
                    </div>
                  </div>
                ) : (
