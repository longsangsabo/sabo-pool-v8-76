    ...
    <div className="space-y-3">
      ...
      <div className="space-y-2">
        ...
        {errors.max_participants && (
          <p className="text-xs text-destructive">{String(errors.max_participants.message)}</p>
        )}
        ...
        {watchedData.max_participants && watchedData.tournament_type && (
          <div className="flex items-center gap-1 text-xs text-muted-foreground">
            ...
            <span>
              Ước tính: {getEstimatedDuration(watchedData.max_participants, watchedData.tournament_type)}
            </span>
          </div>
        )}
      </div>
        ...
        <div className="space-y-3">
          <div className="grid grid-cols-2 gap-3">
            <div className="space-y-1">
              <Label className="text-xs text-muted-foreground">Hạng thấp nhất</Label>
              <select
                value={watchedData.min_rank_requirement || ''}
                onChange={(e) => setValue('min_rank_requirement', e.target.value as any, { shouldValidate: true })}
            ...
            <div className="space-y-1">
              <Label className="text-xs text-muted-foreground">Hạng cao nhất</Label>
              <select
                value={watchedData.max_rank_requirement || ''}
                onChange={(e) => setValue('max_rank_requirement', e.target.value as any, { shouldValidate: true })}
                className="w-full h-8 px-2 text-sm border border-border rounded bg-background hover:bg-muted/50 focus:border-primary focus:outline-none"
              >
                <option value="">Không giới hạn</option>
                {Object.keys(RANK_ELO).map((rank) => (
          ...
          {(watchedData.min_rank_requirement || watchedData.max_rank_requirement) && (
      ...
      <div className="space-y-2">
        ...
        {errors.tournament_type && (
          <p className="text-xs text-destructive">{String(errors.tournament_type.message)}</p>
        )}
      </div>
      ...
      <div className="space-y-2">
        ...
        {errors.game_format && (
          <p className="text-xs text-destructive">{String(errors.game_format.message)}</p>
        )}
      </div>
