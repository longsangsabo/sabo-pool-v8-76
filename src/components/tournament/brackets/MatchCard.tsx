...
export const MatchCard: FC<MatchCardProps> = ({
  ...
  const [player1Score, setPlayer1Score] = useState(match.score_player1?.toString() || '');
  const [player2Score, setPlayer2Score] = useState(match.score_player2?.toString() || '');
  ...
  const getPlayerName = (playerId: string | null, playerData?: any) => {
    ...
    if (playerData) {
      return playerData.display_name || playerData.full_name || 'Unknown Player';
    }
    return `Player ${playerId.slice(-4)}`;
  };

  const isCompleted = match.status === 'completed';
  const canSubmitScore = allowInput && !isCompleted && match.player1_id && match.player2_id;

  return (
    <Card className={cn(
      "transition-all duration-200",
      isHighlighted && "ring-2 ring-primary/20 bg-primary/5",
      isFinal && "border-yellow-300 bg-gradient-to-br from-yellow-50 to-orange-50 dark:from-yellow-950/20 dark:to-orange-950/20",
      ...
      <CardContent className="p-4">
        ...
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center space-x-2">
            ...
            {branch && <Badge variant="outline" className="text-xs">{branch}</Badge>}
          </div>
          
          <Badge variant="secondary" className={cn("text-xs", getStatusColor(match.status))}>
            {match.status}
          </Badge>
        </div>
        ...
        <div className="space-y-3">
          ...
          <div className={cn(
            ...
            match.winner_id === match.player1_id && "bg-green-100 dark:bg-green-900/30 border-green-300"
          )}>
            <div className="flex items-center space-x-2">
          ...
          <div className={cn(
            "flex items-center justify-between p-2 rounded border",
            match.winner_id === match.player2_id && "bg-green-100 dark:bg-green-900/30 border-green-300"
          )}>
            <div className="flex items-center space-x-2">
