...
const ForceCompleteTournamentButton: React.FC<ForceCompleteTournamentButtonProps> = ({
  tournamentId,
  tournamentName,
  currentStatus,
  onStatusChanged
  ...
  const calculateTournamentResults = async () => {
    ...
    try {
      ...
      if (result?.success) {
        toast.success(`🎉 Đã tính toán kết quả cho ${result.results_created} người chơi`);
        console.log(`🏆 Champion: ${result.champion_id}, Runner-up: ${result.runner_up_id}`);
