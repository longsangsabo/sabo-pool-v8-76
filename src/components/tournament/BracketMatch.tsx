...
export const BracketMatch: React.FC<BracketMatchProps> = ({ 
  ...
  const getStatusBadge = () => {
    switch (match.status) {
      case 'completed':
        return <Badge variant="default" className="gap-1"><CheckCircle className="w-3 h-3" />Hoàn thành</Badge>;
      case 'ongoing':
        return <Badge variant="secondary" className="gap-1"><PlayCircle className="w-3 h-3" />Đang đấu</Badge>;
      case 'scheduled':
        return <Badge variant="outline" className="gap-1"><Clock className="w-3 h-3" />Chờ đấu</Badge>;
      default:
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            <span className="text-sm font-medium">
              {match.is_third_place_match ? 'Tranh hạng 3' : `Trận ${match.match_number}`}
            </span>
            {getStatusBadge()}
          </div>
