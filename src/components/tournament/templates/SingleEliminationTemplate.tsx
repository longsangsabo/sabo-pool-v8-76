...
import { Trophy, Users, RefreshCw, CheckCircle, Clock, Loader2 } from 'lucide-react';
...
export const SingleEliminationTemplate: React.FC<SingleEliminationTemplateProps> = ({
  participants,
  tournamentId,
  tournamentData,
      ...
      const loadLiveBracketData = async () => {
        ...
        try {
          ...
          if (matches && matches.length > 0) {
            console.log('🏆 [SingleEliminationTemplate] Loaded live tournament matches:', matches);
            setLiveBracketData({ matches });
            toast.success(`🏆 Tải sơ đồ giải đấu thành công!`);
          }
        } catch (error) {
          console.error('❌ [SingleEliminationTemplate] Error loading live bracket:', error);
        } finally {
          setIsLoading(false);
        }
      };
  ...
  useEffect(() => {
    if (isCompletedTemplate && completedTournamentId) {
      setIsLoading(true);
      loadTournamentBracketData(completedTournamentId).then((data) => {
        if (data) {
          console.log('🏆 [SingleEliminationTemplate] Loaded completed tournament data:', data);
          setCompletedBracketData(data);
          toast.success(`🏆 Dữ liệu giải đấu đã hoàn thành được tải thành công!`);
        }
        setIsLoading(false);
      }).catch((error) => {
        console.error('❌ [SingleEliminationTemplate] Error loading data:', error);
        setIsLoading(false);
      });
    }
  ...
  const generateDemoMatches = (participantList: Participant[], bracketSize: number) => {
    const matches: any[] = [];
  ...
  const { processedMatches, matchesByRound, displayParticipants, bracketSize } = useMemo(() => {
    if (isCompletedTemplate && completedBracketData) {
      ...
      const participantCount = completedBracketData.participants?.length || participants.length;
      const size = Math.max(8, Math.min(32, Math.pow(2, Math.ceil(Math.log2(participantCount)))));
      ...
      const size = Math.max(8, Math.min(32, Math.pow(2, Math.ceil(Math.log2(participantCount || 8)))));
      ...
      const { matches, matchesByRound: demoRounds } = generateDemoMatches(participants, size);
  ...
  }, [isCompletedTemplate, completedBracketData, liveBracketData, participants]);
  ...
  const updateMatchScore = (matchId: string, player: 'player1' | 'player2', value: string) => {
  ...
  const submitMatchScore = async (matchId: string) => {
    ...
    try {
      ...
      // Validation
      if (!matchId || typeof scores.player1 !== 'number' || typeof scores.player2 !== 'number') {
      ...
      const { data, error } = await supabase.rpc('emergency_complete_tournament_match', {
        p_match_id: matchId,
        p_winner_id: scores.player1 > scores.player2 ? match.player1_id : match.player2_id
      });
        ...
        <Card>
          ...
          <CardContent>
            <div className="flex items-center justify-center py-8">
              ...
              <span className="ml-2 text-sm text-gray-600">Đang tải dữ liệu giải đấu...</span>
            </div>
          </CardContent>
        </Card>
      </div>
  ...
  return (
    <div className="space-y-6">
      {/* Template Info */}
      <Card className={`${isCompletedTemplate ? 'border-blue-200 bg-blue-50/50' : 'border-green-200 bg-green-50/50'}`}>
        <CardHeader className="pb-3">
          <CardTitle className="flex items-center gap-2 text-lg">
            ...
            {isCompletedTemplate ? (
              <>🏆 Giải đấu đã hoàn thành - Single Elimination ({bracketSize} người)</>
            ) : (
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4 text-sm">
            <div className="flex items-center gap-2">
              <Users className={`h-4 w-4 ${isCompletedTemplate ? 'text-blue-600' : 'text-green-600'}`} />
              <span>Participants: {displayParticipants.length}/{bracketSize}</span>
            </div>
            <div className="flex items-center gap-2">
              <Trophy className={`h-4 w-4 ${isCompletedTemplate ? 'text-blue-600' : 'text-green-600'}`} />
          ...
          {isCompletedTemplate && completedBracketData?.results && (
            <div className="mt-4 p-3 bg-blue-50 border border-blue-200 rounded-lg">
              <h4 className="font-semibold text-blue-800 mb-2">📊 Thống kê giải đấu</h4>
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                <div>
                  <span className="font-medium">Tổng trận:</span>
                  <br />
                  <span className="text-blue-700">{processedMatches.length}</span>
                </div>
                <div>
                  <span className="font-medium">Hoàn thành:</span>
                  <br />
                  <span className="text-blue-700">{processedMatches.filter((m: any) => m.status === 'completed').length}</span>
                </div>
                <div>
                  <span className="font-medium">Người thắng:</span>
                  <br />
                  <span className="text-blue-700">
                    {completedBracketData.final_standings?.[0]?.player_name || 'TBD'}
                  </span>
                </div>
                <div>
                  ...
                  <br />
                  <span className="text-blue-700">
                    {completedBracketData.final_standings?.[1]?.player_name || 'TBD'}
                  </span>
                </div>
              </div>
            </div>
          ...
          {!isCompletedTemplate && displayParticipants.length < bracketSize && (
            <div className="mt-3 p-3 bg-yellow-50 border border-yellow-200 rounded-lg">
              <p className="text-sm text-yellow-800">
                ⚠️ Template hiển thị với {displayParticipants.length} người tham gia thực tế. 
                Bracket được điều chỉnh cho {bracketSize} vị trí với dữ liệu thật thay vì "TBD".
              </p>
            </div>
          )}
        ...
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center gap-2">
              ...
              {isCompletedTemplate ? 'Sơ đồ Single Elimination - Kết quả thực tế' : 'Sơ đồ Single Elimination - Đang diễn ra'}
              {!isCompletedTemplate && (
              ...
              {!isCompletedTemplate && (
                <div className="mt-6 space-y-2">
                  <div className="text-sm text-gray-600">
                    📊 {displayParticipants.length} người tham gia sẽ được sắp xếp trong bracket {bracketSize} vị trí
                  </div>
                  <div className="text-sm text-gray-600">
                    🏆 {Math.ceil(Math.log2(bracketSize))} vòng đấu, {bracketSize - 1} trận tổng cộng
                  </div>
                </div>
              )}
            </div>
