...
import { Trophy, Users, Crown, Award, RefreshCw, CheckCircle, Clock, Loader2 } from 'lucide-react';
...
export const DoubleEliminationTemplate: React.FC<DoubleEliminationTemplateProps> = ({
  participants,
  tournamentId,
  tournamentData,
  ...
  useEffect(() => {
    console.log('🔍 [DoubleEliminationTemplate] Debug info:', {
      ...
      shouldLoad: tournamentId && (isCompletedTemplate || tournamentData?.status === 'in_progress')
    });
    
    if (tournamentId && (isCompletedTemplate || tournamentData?.status === 'in_progress')) {
      setIsLoading(true);
      console.log('📊 [DoubleEliminationTemplate] Loading bracket data for tournament:', tournamentId);
      loadTournamentBracketData(tournamentId).then((data) => {
        console.log('🏆 [DoubleEliminationTemplate] Loaded tournament data:', data);
        if (data) {
          setCompletedBracketData(data);
          if (isCompletedTemplate) {
            toast.success(`🏆 Dữ liệu giải đấu Double Elimination đã được tải thành công!`);
          }
        } else {
          console.warn('⚠️ [DoubleEliminationTemplate] No bracket data returned');
        }
        setIsLoading(false);
      }).catch((error) => {
        console.error('❌ [DoubleEliminationTemplate] Error loading data:', error);
        setIsLoading(false);
      });
    } else {
  ...
  const { 
    ...
    if (completedBracketData && (isCompletedTemplate || tournamentData?.status === 'in_progress')) {
      ...
      const winnerMatches = matches.filter((m: any) => m.bracket_type === 'winners' || !m.bracket_type);
      const loserMatches = matches.filter((m: any) => m.bracket_type === 'losers');
      const semifinalMatches = matches.filter((m: any) => m.bracket_type === 'semifinals');
      const finalMatches = matches.filter((m: any) => m.bracket_type === 'finals');
        ...
        <Card>
          ...
          <CardContent>
            <div className="flex items-center justify-center py-8">
              ...
              <span className="ml-2 text-sm text-gray-600">Đang tải dữ liệu giải đấu Double Elimination...</span>
            </div>
          </CardContent>
        </Card>
      </div>
  ...
  return (
    <div className="space-y-6">
      {/* Template Info */}
      <Card className={`${isCompletedTemplate ? 'border-blue-200 bg-blue-50/50' : 'border-purple-200 bg-purple-50/50'}`}>
        <CardHeader className="pb-3">
          <CardTitle className="flex items-center gap-2 text-lg">
            ...
            {isCompletedTemplate ? (
              <>🏆 Giải đấu đã hoàn thành - Double Elimination ({bracketSize} người)</>
            ) : (
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4 text-sm">
            <div className="flex items-center gap-2">
              <Users className={`h-4 w-4 ${isCompletedTemplate ? 'text-blue-600' : 'text-purple-600'}`} />
              <span>Participants: {displayParticipants.length}/{bracketSize}</span>
            </div>
            <div className="flex items-center gap-2">
              <Trophy className={`h-4 w-4 ${isCompletedTemplate ? 'text-blue-600' : 'text-purple-600'}`} />
              <span>Type: Double Elimination</span>
            </div>
            <div className="flex items-center gap-2">
              <Crown className={`h-4 w-4 ${isCompletedTemplate ? 'text-blue-600' : 'text-purple-600'}`} />
          ...
          {isCompletedTemplate && completedBracketData?.results && (
            <div className="mt-4 p-3 bg-blue-50 border border-blue-200 rounded-lg">
              <h4 className="font-semibold text-blue-800 mb-2">📊 Thống kê Double Elimination</h4>
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                <div>
                  <span className="font-medium">Winner Bracket:</span>
                  <br />
                  <span className="text-blue-700">{winnerBracketMatches.length} trận</span>
                </div>
                <div>
                  <span className="font-medium">Loser Bracket:</span>
                  <br />
                  <span className="text-blue-700">{loserBracketMatches.length} trận</span>
                </div>
                <div>
          ...
          {!isCompletedTemplate && displayParticipants.length < bracketSize && (
            <div className="mt-3 p-3 bg-yellow-50 border border-yellow-200 rounded-lg">
              <p className="text-sm text-yellow-800">
                ...
                Double Elimination cần {bracketSize} vị trí để có cấu trúc chuẩn.
              </p>
            </div>
          )}
          ...
          {isCompletedTemplate ? (
            <div className="mt-3 p-3 bg-green-50 border border-green-200 rounded-lg">
              <div className="flex items-center justify-between">
                <p className="text-sm text-green-800">
                  ✅ Hiển thị kết quả thực tế từ giải đấu Double Elimination đã hoàn thành
                </p>
            ...
            <div className="mt-3 p-3 bg-blue-50 border border-blue-200 rounded-lg">
              <p className="text-sm text-blue-800">
                💡 Double Elimination cho phép mỗi người chơi thua 1 lần trước khi bị loại
              </p>
            </div>
          )}
        </CardContent>
