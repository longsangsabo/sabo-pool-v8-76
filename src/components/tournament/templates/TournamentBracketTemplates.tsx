...
export const TournamentBracketTemplates: React.FC<TournamentBracketTemplatesProps> = ({ className }) => {
  const [selectedTournamentId, setSelectedTournamentId] = useState<string>('');
  const [activeTemplate, setActiveTemplate] = useState('single');
  const { tournaments, loading: tournamentsLoading } = useTournamentsForTemplates();
  const { 
    registrations, 
      ...
      <Card className="mb-6 border-blue-200 bg-blue-50/50">
          ...
          <CardTitle className="flex items-center justify-between text-lg">
            <div className="flex items-center gap-2">
              ...
              <div className="flex gap-2">
                ...
                >
                  <RefreshCw className={`h-4 w-4 mr-2 ${registrationsLoading ? 'animate-spin' : ''}`} />
                  Làm mới
                </Button>
                ...
                <Button 
                  ...
                  disabled={isGenerating || !selectedTournament || (selectedTournament.status !== 'locked' && selectedTournament.status !== 'ready' && selectedTournament.status !== 'registration_open') || participants.length < 2}
                >
                  <Settings className={`h-4 w-4 mr-2 ${isGenerating ? 'animate-spin' : ''}`} />
                  Tạo sơ đồ
                </Button>
              </div>
          </CardTitle>
        ...
        <CardContent className="space-y-3">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div>
              <p className="font-semibold text-blue-800">{selectedTournament.name}</p>
              <p className="text-sm text-blue-600">
                {selectedTournament.tournament_type === 'single_elimination' ? 'Loại trực tiếp' : 
                 selectedTournament.tournament_type === 'double_elimination' ? 'Loại kép' : 
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm">
                {participants.length}/{selectedTournament.max_participants} người tham gia
              </span>
            </div>
            <div>
              <Badge 
                variant={selectedTournament.status === 'completed' ? 'default' : 'secondary'}
                className="text-xs"
              >
                 {selectedTournament.status === 'completed' ? 'Đã hoàn thành' :
                  selectedTournament.status === 'ongoing' ? 'Đang diễn ra' :
                  selectedTournament.status === 'locked' ? 'Đã khóa đăng ký' :
                  selectedTournament.status === 'registration_open' ? 'Sẵn sàng' : selectedTournament.status}
              </Badge>
            </div>
          </div>
          ...
          {participants.length > 0 && (
            <div>
              <p className="text-sm text-blue-700 mb-2">Thứ tự hạt giống (theo đăng ký + ELO):</p>
              <div className="flex flex-wrap gap-2">
                {participants.slice(0, 8).map((participant, index) => (
                  <Badge key={participant.id} variant="outline" className="text-xs">
                    ...
                    <span className="ml-1 text-muted-foreground">({participant.elo})</span>
                  </Badge>
                ))}
                        ...
                        <div className="flex items-center gap-2 ml-4">
                          ...
                          <Badge 
                            variant={
                              ...
                              tournament.status === 'cancelled' ? 'secondary' :
                              'outline'
                            }
                            className="text-xs"
                          >
                             {tournament.status === 'completed' ? 'Hoàn thành' :
                              tournament.status === 'ongoing' ? 'Đang chơi' : 
                              tournament.status === 'cancelled' ? 'Đã hủy' :
                              tournament.status === 'upcoming' ? 'Sắp diễn ra' :
                              tournament.status === 'locked' ? 'Đã khóa' :
                              tournament.status === 'registration_open' ? 'Đăng ký' : 'Sẵn sàng'}
                          </Badge>
                        </div>
                      </div>
                    </SelectItem>
              ...
              <Tabs value={activeTemplate} onValueChange={setActiveTemplate} className="space-y-6">
                ...
                <TabsContent value="single" className="space-y-6">
                  <SingleEliminationTemplate 
                    ...
                    completedTournamentId={selectedTournament?.status === 'completed' ? selectedTournamentId : undefined}
                    isCompletedTemplate={selectedTournament?.status === 'completed'}
                  />
                </TabsContent>
