...
export const ParticipantListRealtime: React.FC<ParticipantListRealtimeProps> = ({ 
  tournamentId, 
  maxParticipants 
}) => {
  ...
  const displayedParticipants = showAll ? participants : participants.slice(0, 6);
  const confirmedParticipants = participants.filter(p => p.registration_status === 'confirmed');
  const pendingParticipants = participants.filter(p => p.registration_status === 'pending');
      ...
      <CardContent>
        {participants.length === 0 ? (
          <div className="text-center py-8">
            ...
            <p className="text-sm text-gray-400">Hãy là người đầu tiên đăng ký!</p>
          </div>
        ) : (
          <>
            <div className="space-y-3">
                  ...
                  <div className="relative">
                    <Avatar className="h-10 w-10">
                      <AvatarImage 
                        ...
                        alt={participant.profiles?.display_name || participant.profiles?.full_name} 
                      />
                      <AvatarFallback className="bg-blue-100 text-blue-600">
                        {getInitials(participant.profiles?.display_name || participant.profiles?.full_name || 'UN')}
                      </AvatarFallback>
                    </Avatar>
                  ...
                  <div className="flex-1 ml-3">
                    <div className="flex items-center gap-2">
                      <h4 className="font-medium text-gray-900">
                        {participant.profiles?.display_name || participant.profiles?.full_name || 'Người chơi'}
                      </h4>
                      {index === 0 && participant.registration_status === 'confirmed' && (
                    ...
                    <div className="text-sm text-gray-500 flex items-center gap-3">
                      ...
                      <span>
                        {new Date(participant.registration_date).toLocaleDateString('vi-VN', {
                          day: '2-digit',
                          month: '2-digit',
                          hour: '2-digit',
                          minute: '2-digit'
            ...
            {participants.length > 6 && (
              <div className="mt-4 text-center">
                <Button 
                  ...
                  className="w-full"
                >
                  {showAll ? 'Thu gọn' : `Xem thêm ${participants.length - 6} người`}
                </Button>
              </div>
            )}
