...
export const MatchManagement: React.FC<MatchManagementProps> = ({
  ...
  const handleUpdateMatch = async () => {
    ...
    setUpdating(true);
    try {
      const winnerId = scorePlayer1 > scorePlayer2 ? editingMatch.player1_id : editingMatch.player2_id;
      
      console.log('🏆 [MatchManagement] Updating match with auto-advancement:', {
      ...
      console.log('✅ [MatchManagement] Match updated successfully, auto-advancement triggered by database');
      
      toast.success('🎯 Kết quả đã được cập nhật! Người thắng sẽ tự động chuyển sang vòng tiếp theo.');
      ...
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center justify-between">
            ...
            {canEdit && (
              <Badge variant="outline">Chế độ chỉnh sửa</Badge>
            )}
          </CardTitle>
        </CardHeader>
        <CardContent>
          <div className="grid grid-cols-3 gap-4 text-sm">
            ...
            <div className="text-center">
              <div className="text-2xl font-bold text-blue-600">
                {matches.filter(m => m.status === 'pending' && m.player1_id && m.player2_id).length}
              </div>
                    ...
                    <CardContent className="pt-4">
                      ...
                      <div className="grid grid-cols-2 gap-4">
                        ...
                        <div className={`p-3 rounded-lg border ${
                          match.winner_id === match.player1_id ? 'bg-green-50 border-green-200' : 'bg-muted/50'
                        }`}>
                        ...
                        {/* Player 2 */}
                        <div className={`p-3 rounded-lg border ${
                          match.winner_id === match.player2_id ? 'bg-green-50 border-green-200' : 'bg-muted/50'
                        }`}>
                          <div className="flex items-center justify-between">
                            <div className="flex items-center gap-2">
                              <User className="h-4 w-4 text-muted-foreground" />
                      ...
                      {match.scheduled_time && (
                        <div className="mt-3 flex items-center gap-2 text-sm text-muted-foreground">
                          ...
                          <span>
                            Lịch đấu: {new Date(match.scheduled_time).toLocaleString('vi-VN')}
                          </span>
                        </div>
                      )}
                    </CardContent>
          ...
          {editingMatch && (
            <div className="space-y-4">
              <div className="text-center text-sm text-muted-foreground">
                Vòng {editingMatch.round_number} - Trận {editingMatch.match_number}
              </div>
              ...
              <div className="space-y-4">
                ...
                <div className="flex items-center justify-between">
                  ...
                  <Input
                    ...
                    min="0"
                    value={scorePlayer1}
                    onChange={(e) => setScorePlayer1(parseInt(e.target.value) || 0)}
                ...
                <div className="flex items-center justify-between">
                  ...
                  <Input
                    ...
                    min="0"
                    value={scorePlayer2}
                    onChange={(e) => setScorePlayer2(parseInt(e.target.value) || 0)}
                    className="w-20 text-center"
                  />
                </div>
              </div>
