...
export const TournamentParticipantsTab: React.FC<TournamentParticipantsTabProps> = ({
  ...
  const { registrations, loading, error, refetch } = useTournamentRegistrations(tournamentId);
  ...
  return (
    <div className="space-y-6">
        ...
        <CardContent>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <div className="text-center">
              <div className="text-2xl font-bold text-primary">{confirmedRegistrations.length}</div>
            ...
            <div className="text-center">
              <div className="text-2xl font-bold text-blue-600">{waitlistRegistrations.length}</div>
              <div className="text-sm text-muted-foreground">Danh sách chờ</div>
            </div>
            <div className="text-center">
              <div className="text-2xl font-bold text-muted-foreground">{maxParticipants - confirmedRegistrations.length}</div>
          ...
          <div className="mt-4">
            <div className="flex justify-between text-sm mb-2">
              ...
              <span>{confirmedRegistrations.length}/{maxParticipants}</span>
            </div>
            <div className="w-full bg-gray-200 rounded-full h-2">
              <div 
                className="bg-primary h-2 rounded-full transition-all duration-300" 
                style={{ width: `${Math.min((confirmedRegistrations.length / maxParticipants) * 100, 100)}%` }}
              />
            </div>
          </div>
        </CardContent>
          ...
          <CardContent>
            <div className="space-y-3">
              {confirmedRegistrations.map((registration, index) => (
                <div key={registration.id} className="flex items-center space-x-3 p-3 border rounded-lg hover:bg-gray-50 transition-colors">
                    ...
                    <Avatar className="h-10 w-10">
                      ...
                      <AvatarFallback>
                        {registration.player?.full_name?.charAt(0) || 
                         registration.player?.display_name?.charAt(0) || 'U'}
                      </AvatarFallback>
                    </Avatar>
                  </div>
                  ...
                  <div className="flex-1">
                    <div className="font-medium">
                      {registration.player?.display_name || registration.player?.full_name || 'Người chơi ẩn danh'}
                    </div>
                    <div className="text-sm text-muted-foreground">
                      Hạng: {registration.player?.verified_rank || registration.player?.current_rank || 'Chưa xác định'} • 
                      ELO: {registration.player?.elo || 1000} • 
                      Đăng ký: {new Date(registration.registration_date || registration.created_at).toLocaleDateString('vi-VN')}
                    </div>
                  </div>
                  
                  <Badge variant={getStatusVariant(registration.registration_status)} className="gap-1">
      ...
      {waitlistRegistrations.length > 0 && (
        <Card>
          <CardHeader>
            <CardTitle>Danh sách chờ ({waitlistRegistrations.length})</CardTitle>
          </CardHeader>
          <CardContent>
            <div className="space-y-3">
              {waitlistRegistrations.map((registration, index) => (
                <div key={registration.id} className="flex items-center space-x-3 p-3 border rounded-lg opacity-75">
                  <div className="text-sm text-muted-foreground">
                    #{index + 1}
                  </div>
      ...
      {registrations.length === 0 && (
        <Card>
          <CardContent className="text-center py-12">
            ...
            <h3 className="text-lg font-semibold mb-2">Chưa có người tham gia</h3>
