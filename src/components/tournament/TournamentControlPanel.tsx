...
export const TournamentControlPanel: React.FC<TournamentControlPanelProps> = ({
  ...
  const { matches, loading: matchesLoading, refetch: refetchMatches } = useTournamentMatches(tournamentId);
  const { isConnected, automationActivity } = useRealtimeTournamentSync(tournamentId);
  ...
  const pendingPlayerAssignment = matches.filter(m => 
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      case 'ongoing': return 'bg-green-500/20 text-green-700 border-green-200';
      case 'registration_closed': return 'bg-blue-500/20 text-blue-700 border-blue-200';
      case 'completed': return 'bg-purple-500/20 text-purple-700 border-purple-200';
      default: return 'bg-gray-500/20 text-gray-700 border-gray-200';
    }
  };

  const canStartTournament = tournament.status === 'registration_closed' && totalMatches > 0;
  const needsRecovery = tournament.status === 'ongoing' && pendingPlayerAssignment > 0;
      ...
      <CardContent className="space-y-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <div className="flex items-center gap-2 p-3 bg-white/50 rounded-lg">
            ...
            <div>
              <div className="text-sm font-medium">{completedMatches}/{totalMatches}</div>
          ...
          <div className="flex items-center gap-2 p-3 bg-white/50 rounded-lg">
            ...
            <div>
              <div className="text-sm font-medium">{pendingPlayerAssignment}</div>
        ...
        <div>
          <div className="flex justify-between text-sm text-muted-foreground mb-2">
            ...
            <span>{completedMatches} of {totalMatches} matches</span>
          </div>
            ...
            <div className="flex items-start gap-2">
              ...
              <div>
                ...
                <div className="text-xs text-orange-700 dark:text-orange-300 mt-1">
                  Some matches are missing player assignments. Click "Khôi phục automation" to fix.
                </div>
              </div>
            </div>
          </div>
