...
const RepairBracketButton: React.FC<RepairBracketButtonProps> = ({
  ...
  const { createBracket, isCreatingBracket } = useDoubleEliminationBracket(tournamentId);

  // Only show for double elimination tournaments that might need repairs
  const shouldShowButton = ['ongoing', 'registration_closed', 'completed'].includes(tournamentStatus.toLowerCase()) 
      ...
      <AlertDialogContent>
        <AlertDialogHeader>
          ...
          <AlertDialogDescription className="space-y-2">
            <p>
              Bạn có chắc chắn muốn sửa chữa bảng đấu cho giải <strong>{tournamentName}</strong>?
            </p>
            ...
            <div className="mt-3 p-3 bg-muted rounded-md">
              <p className="text-xs text-muted-foreground">
                <strong>Lưu ý:</strong> Chức năng này đã được cập nhật với algorithm mới để sửa 
                chính xác vấn đề ghép cặp Loser Bracket. An toàn và chỉ sửa các lỗi thực sự. 
                Không ảnh hưởng đến kết quả trận đã hoàn thành.
              </p>
            </div>
