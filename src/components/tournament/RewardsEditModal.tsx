...
const rewardsSchema = z.object({
  ...
  specialAwards: z.array(z.object({
    name: z.string(),
...
export const RewardsEditModal: React.FC<RewardsEditModalProps> = ({
  ...
  const form = useForm<TournamentRewards>({
    ...
    defaultValues: {
      ...
      positions: (rewards?.positions?.length > 0) ? rewards.positions : getDefaultPositions(),
  ...
  const removePhysicalItem = (positionIndex: number, itemIndex: number) => {
    const currentPositions = getValues('positions') || [];
    const position = currentPositions[positionIndex];
    const updatedItems = (position.items || []).filter((_, i) => i !== itemIndex);
    updatePosition(positionIndex, 'items', updatedItems);
  };

  const updatePhysicalItem = (positionIndex: number, itemIndex: number, value: string) => {
    const currentPositions = getValues('positions') || [];
    const position = currentPositions[positionIndex];
    const updatedItems = [...(position.items || [])];
  ...
  const handleSave = async () => {
    try {
      ...
      const totalCashPrizes = formData.positions.reduce((sum, pos) => sum + (pos.cashPrize || 0), 0);
  ...
  const hasValidationErrors = () => {
    ...
    const hasDuplicates = positionNumbers.some((pos, index) => positionNumbers.indexOf(pos) !== index);
    const totalPrize = watch('totalPrize') || 0;
    const usedPrize = calculateTotalPrize();
    const exceedsTotal = watchedShowPrizes && totalPrize > 0 && usedPrize > totalPrize;
  ...
  const autoDistributePrizes = () => {
    ...
    if (totalPrize > 0 && positions.length > 0) {
      const updatedPositions = positions.map((pos, index) => {
        let percentage = 0;
        if (index === 0) percentage = 0.5; // 50% for 1st
        else if (index === 1) percentage = 0.3; // 30% for 2nd  
        else if (index === 2) percentage = 0.2; // 20% for 3rd
                ...
                <div className="space-y-2">
                  ...
                  <div className="space-y-2">
                    <div className="text-sm text-muted-foreground">
                      Tổng tiền thưởng từ các vị trí: {formatPrizeAmount(calculateTotalPrize())}
                    </div>
                    <div className={`text-sm font-medium ${calculateRemainingPrize() < 0 ? 'text-red-600' : 'text-green-600'}`}>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-4 gap-4 items-center">
                      <div className="flex items-center gap-3">
                        <div className="w-8 h-8 bg-primary/10 rounded-full flex items-center justify-center">
                          <span className="font-bold text-primary text-sm">{position.position}</span>
                        </div>
                         ...
                         <div className="space-y-2">
                           {position.items.map((item, itemIndex) => (
                             <div key={itemIndex} className="flex items-center gap-2">
                               <Input
                                 value={item}
                                 onChange={(e) => updatePhysicalItem(index, itemIndex, e.target.value)}
                               ...
                               <Button
                                 ...
                                 variant="ghost"
                                 size="sm"
                                 onClick={() => removePhysicalItem(index, itemIndex)}
                     ...
                     <div className="mt-3 pt-3 border-t">
                       <div className="flex items-center gap-2 text-sm flex-wrap">
                         ...
                         {position.items && position.items.filter(item => item.trim()).length > 0 && (
                           <Badge variant="secondary" className="bg-purple-50 text-purple-700">
                             <Gift className="w-3 h-3 mr-1" />
                             {position.items.filter(item => item.trim()).length} hiện vật
                           </Badge>
                         )}
                       </div>
                     </div>
