...
export const ConfirmationStep: React.FC<ConfirmationStepProps> = ({
  ...
  const getRankLabel = (rankValue: string) => {
    return REGISTRATION_RANKS.find(r => r.value === rankValue)?.label || rankValue;
  };
  ...
  const getExperienceLabel = (experience: string) => {
    return EXPERIENCE_LEVELS.find(e => e.value === experience)?.label || experience;
  };
  ...
  const getStatusMessage = () => {
    if (watchedData.payment_method === 'vnpay' && paymentUrl) {
      return {
        ...
        message: 'Đăng ký đã được ghi nhận. Vui lòng thanh toán để xác nhận tham gia.',
        color: 'warning'
      };
      ...
      return {
        ...
        message: 'Vui lòng thanh toán tiền mặt tại địa điểm thi đấu trước giờ thi đấu 30 phút.',
        color: 'success'
      };
      ...
      return {
        ...
        message: 'Thông tin chuyển khoản sẽ được gửi qua email. Vui lòng hoàn tất trong 24h.',
        color: 'info'
      };
    }
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="space-y-3">
              <div>
                ...
                <Badge variant="outline">{getRankLabel(watchedData.current_rank)}</Badge>
              </div>
              ...
              <div>
                ...
                <p className="font-medium">{getExperienceLabel(watchedData.tournament_experience)}</p>
              </div>
            </div>
          </div>
          ...
          {watchedData.notes && (
            <>
              ...
              <div>
                ...
                <p className="text-sm bg-muted p-3 rounded-lg">{watchedData.notes}</p>
              </div>
            </>
          )}
        </CardContent>
        ...
        <CardContent>
          <div className="space-y-3">
            <div className="flex justify-between items-center">
              ...
              <span className="font-medium">{getPaymentMethodLabel(watchedData.payment_method)}</span>
            </div>
            
            <div className="flex justify-between items-center">
              <span className="text-sm text-muted-foreground">Trạng thái:</span>
              <Badge variant={status.type === 'success' ? 'default' : 'secondary'}>
                {watchedData.payment_method === 'vnpay' && paymentUrl ? 'Chờ thanh toán' : 
                 watchedData.payment_method === 'cash' ? 'Thanh toán tại chỗ' :
                 watchedData.payment_method === 'transfer' ? 'Chờ chuyển khoản' : 'Hoàn tất'}
              </Badge>
            </div>
          </div>
        </CardContent>
