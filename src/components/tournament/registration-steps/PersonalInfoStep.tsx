      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          ...
          <div className="space-y-2">
            ...
            />
            {errors.player_name && (
              <p className="text-sm text-destructive">{errors.player_name.message}</p>
            )}
          </div>
        ...
        <CardContent className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="space-y-2">
              ...
              >
                <SelectTrigger className={errors.current_rank ? 'border-destructive' : ''}>
                    ...
                    return (
                      ...
                      >
                        <div className="flex items-center gap-2">
                          ...
                          <span className={!isEligible ? 'text-muted-foreground' : ''}>
                            {rank.label}
                          </span>
                          {!isEligible && (
                            <span className="text-xs text-destructive">(Không đủ điều kiện)</span>
                          )}
                        </div>
                      </SelectItem>
                    );
              ...
              {errors.current_rank && (
                <p className="text-sm text-destructive">{errors.current_rank.message}</p>
              )}
            </div>
            ...
            <div className="space-y-2">
              ...
              <Select
                ...
                onValueChange={(value) => setValue('tournament_experience', value as any)}
              >
                <SelectTrigger className={errors.tournament_experience ? 'border-destructive' : ''}>
              ...
              {errors.tournament_experience && (
                <p className="text-sm text-destructive">{errors.tournament_experience.message}</p>
              )}
            </div>
          </div>

          {/* Rank Requirements Alert */}
          {selectedTournament && (selectedTournament.min_rank_requirement || selectedTournament.max_rank_requirement) && (
            <Alert>
              ...
              <AlertDescription>
                ...
                {selectedTournament.min_rank_requirement && selectedTournament.max_rank_requirement && <br />}
        ...
        <CardContent className="space-y-4">
          <div className="flex items-start space-x-2">
            ...
            <Label htmlFor="agree_terms" className="text-sm leading-relaxed cursor-pointer">
              Tôi đồng ý với <strong>điều lệ giải đấu</strong> và cam kết tuân thủ 
              các quy định về thời gian, địa điểm và cách thức thi đấu.
            </Label>
          </div>
          {errors.agree_terms && (
            <p className="text-sm text-destructive ml-6">{errors.agree_terms.message}</p>
          )}
          ...
          <div className="flex items-start space-x-2">
            ...
            />
            <Label htmlFor="agree_rules" className="text-sm leading-relaxed cursor-pointer">
              Tôi đồng ý với <strong>quy tắc thi đấu</strong> và chấp nhận quyết định 
              của ban tổ chức là quyết định cuối cùng.
            </Label>
          </div>
          {errors.agree_rules && (
            <p className="text-sm text-destructive ml-6">{errors.agree_rules.message}</p>
          )}
