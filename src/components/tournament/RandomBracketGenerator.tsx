...
import { Shuffle, Trophy, Users, Target, RefreshCw, Save, User } from 'lucide-react';
...
const RandomBracketGenerator = () => {
  ...
  }, [tournaments, selectedTournament, availablePlayers, selectedPlayers, generatedBracket]);
  ...
  const fetchTournamentParticipants = async () => {
    ...
    try {
      console.log('🔄 Fetching tournament participants for:', selectedTournament);
      ...
      console.log('👥 Tournament participants result:', { participants, error });
      ...
      console.log('✅ All formatted tournament participants:', formattedPlayers);
  ...
  const generateSeededBracket = () => {
    ...
    try {
      ...
      console.log("📊 Players sorted by ELO:", sortedPlayers.map(p => `${p.full_name}: ${p.elo}`));
      ...
      console.log("🔢 Seeded arrangement:", seededPlayers.map(p => p?.full_name || 'BYE'));
        ...
        <CardContent className="space-y-6">
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              <Label>Người chơi tham gia giải đấu ({selectedPlayers.length}/{availablePlayers.length})</Label>
            ...
            <div className="max-h-64 overflow-y-auto border rounded-md p-4">
              {availablePlayers.length === 0 ? (
                <div className="text-center py-8 text-muted-foreground">
                  ...
                  <p className="text-lg mb-2">Chưa có người nào đăng ký giải đấu này</p>
                  <p className="text-sm mb-4">Vui lòng chọn giải đấu khác hoặc kiểm tra lại danh sách đăng ký</p>
                  <Button 
                    variant="outline" 
                    onClick={fetchTournamentParticipants}
                    size="sm"
                      ...
                      >
                        <div className="flex items-center gap-3">
                          <Avatar className="h-10 w-10">
                            <AvatarImage src={player.avatar_url} alt={player.display_name || player.full_name} />
                            <AvatarFallback>
                          ...
                          <div className="flex-1">
                            <div className="flex items-center justify-between">
                              <div>
                                <div className="font-medium">{player.display_name || player.full_name}</div>
                       ...
                       <div className="space-y-2">
                           ...
                           {match.player1 ? (
                             <>
                               <Avatar className="h-8 w-8">
                                 <AvatarImage src={match.player1.avatar_url} alt={match.player1.display_name || match.player1.full_name} />
                                 <AvatarFallback>
                                   <User className="h-4 w-4" />
                                 </AvatarFallback>
                               </Avatar>
                               <div className="flex-1">
                                 <div className="font-medium">{match.player1.display_name || match.player1.full_name}</div>
                                 <div className="text-sm text-muted-foreground">Hạng: {match.player1.rank}</div>
                               </div>
                               <Badge variant="outline">{match.player1.elo}</Badge>
                             </>
                           ) : (
                             <div className="flex-1 text-center text-muted-foreground py-2">BYE</div>
                           )}
                         </div>
                         ...
                         <div className="flex items-center gap-3 p-3 border rounded-md">
                           {match.player2 ? (
                             <>
                               <Avatar className="h-8 w-8">
                                 <AvatarImage src={match.player2.avatar_url} alt={match.player2.display_name || match.player2.full_name} />
                                 <AvatarFallback>
                                   <User className="h-4 w-4" />
                                 </AvatarFallback>
                               </Avatar>
                               <div className="flex-1">
                                 <div className="font-medium">{match.player2.display_name || match.player2.full_name}</div>
                                 <div className="text-sm text-muted-foreground">Hạng: {match.player2.rank}</div>
                               </div>
                               <Badge variant="outline">{match.player2.elo}</Badge>
                             </>
                           ) : (
                             <div className="flex-1 text-center text-muted-foreground py-2">BYE</div>
                           )}
