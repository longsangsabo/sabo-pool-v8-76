...
interface Match {
  ...
  status: string;
  player1?: { full_name: string; display_name?: string; verified_rank?: string };
  player2?: { full_name: string; display_name?: string; verified_rank?: string };
...
export const EmergencyMatchCompletion: React.FC<EmergencyMatchCompletionProps> = ({
  matches,
  onMatchUpdated,
  isClubOwner
}) => {
  ...
  const incompleteMatches = matches.filter(m => 
    m.status !== 'completed' && m.player1_id && m.player2_id
  );
  ...
  const handleEmergencyCompletion = async () => {
    if (!selectedMatch || player1Score === player2Score) {
      toast.error('Vui lòng chọn trận đấu và nhập tỷ số hợp lệ (không được hòa)');
      return;
    }
    ...
    try {
      const { data, error } = await supabase.rpc('emergency_complete_tournament_match', {
        ...
        p_winner_id: player1Score > player2Score ? selectedMatch.player1_id : selectedMatch.player2_id
      });
      ...
      if (result?.success) {
        toast.success(result.message || 'Trận đấu đã được hoàn thành thành công!');
  ...
  const getMatchLabel = (match: Match) => {
    const player1Name = match.player1?.display_name || match.player1?.full_name || 'TBD';
    const player2Name = match.player2?.display_name || match.player2?.full_name || 'TBD';
    
    let prefix = '';
    if (match.is_third_place_match) {
      prefix = '🥉 Tranh hạng 3: ';
    } else if (match.bracket_type === 'finals') { // SABO_REBUILD: Updated bracket type
      prefix = '🏆 Chung kết: ';
    } else if (match.bracket_type === 'semifinals') { // SABO_REBUILD: Updated bracket type
      prefix = '🎯 Bán kết: ';
    } else {
      ...
      <CardContent className="space-y-4">
        <div>
          ...
          <select
            ...
            onChange={(e) => {
              const match = incompleteMatches.find(m => m.id === e.target.value);
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <Label htmlFor="player1-score">
                  Tỷ số {selectedMatch.player1?.display_name || selectedMatch.player1?.full_name}
                </Label>
                <Input
                  id="player1-score"
                  type="number"
              ...
              <div>
                <Label htmlFor="player2-score">
                  Tỷ số {selectedMatch.player2?.display_name || selectedMatch.player2?.full_name}
                </Label>
                <Input
                  id="player2-score"
                  type="number"
