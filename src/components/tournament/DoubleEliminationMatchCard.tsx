...
interface Match {
  ...
  bracket_type?: string;
  branch_type?: string;
  player1?: { full_name: string; display_name?: string; verified_rank?: string; ranking_verified_rank?: string };
  player2?: { full_name: string; display_name?: string; verified_rank?: string; ranking_verified_rank?: string };
...
export const DoubleEliminationMatchCard: React.FC<DoubleEliminationMatchCardProps> = ({
  match,
  className = '',
  isClubOwner = false,
  currentUserId
}) => {
  const [player1Score, setPlayer1Score] = useState<string>(match.score_player1?.toString() || '');
  const [player2Score, setPlayer2Score] = useState<string>(match.score_player2?.toString() || '');
  const [isSubmitting, setIsSubmitting] = useState(false);
  
  // Enhanced validation for match readiness - allow both scheduled and in_progress
  const isMatchReady = (match.status === 'scheduled' || match.status === 'in_progress') && 
    match.player1_id && 
    match.player2_id && 
  ...
  const getPlayerName = (player?: { full_name: string; display_name?: string }) => {
  ...
  return (
    <Card className={`relative border border-border/60 rounded-lg transition-all duration-300 hover:shadow-md hover:border-primary/40 ${className}`}>
      <CardContent className="p-4">
        ...
        <div className="flex items-center justify-between mb-3">
          ...
          {(match.assigned_table?.table_number || match.assigned_table_number) && (
            <div className="bg-emerald-500 text-white px-2 py-0.5 rounded-full text-xs font-medium">
              Bàn {match.assigned_table?.table_number || match.assigned_table_number}
            </div>
          )}
        </div>
        ...
        <div className="space-y-2">
          ...
          <div className={`p-2 rounded-lg border-l-2 ${
            match.player1 ? 'bg-blue-50/50 border-blue-400' : 'bg-muted/30 border-muted-foreground'
            ...
            <div className="flex items-center gap-2">
              {match.player1 ? (
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="font-medium text-sm truncate flex items-center gap-1">
                      ...
                      <span className="text-xs bg-primary text-primary-foreground px-1.5 py-0.5 rounded font-medium">
                        {match.player1.verified_rank || match.player1.ranking_verified_rank || 'K'}
                      </span>
                    </div>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs text-muted-foreground">Chờ kết quả</span>
                </div>
              )}
            ...
            {canInputScore && (
              <div className="mt-2 flex items-center gap-2">
                <span className="text-xs font-medium text-blue-700">Tỷ số:</span>
                <input
                  type="number"
                  min="0"
                  max="50"
          ...
          {/* Player 2 */}
          <div className={`p-2 rounded-lg border-l-2 ${
            match.player2 ? 'bg-purple-50/50 border-purple-400' : 'bg-muted/30 border-muted-foreground'
                ...
                <div className="flex items-center gap-2 flex-1">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="font-medium text-sm truncate flex items-center gap-1">
                      ...
                      <span className="text-xs bg-primary text-primary-foreground px-1.5 py-0.5 rounded font-medium">
                        {match.player2.verified_rank || match.player2.ranking_verified_rank || 'K'}
                      </span>
                    </div>
                  </div>
                </div>
            ...
            {canInputScore && (
              <div className="mt-2 flex items-center gap-2">
                <span className="text-xs font-medium text-purple-700">Tỷ số:</span>
                <input
                  type="number"
                  min="0"
                  max="50"
        ...
        <div className={`mt-3 p-2 rounded border ${
          ...
          <div className="text-center">
            <div className={`text-sm font-bold ${
              ...
              {getDisplayScore(match.score_player1)} - {getDisplayScore(match.score_player2)}
            </div>
            <div className={`text-xs ${
              ...
              {match.status === 'completed' ? 'Đã xác nhận' :
               match.status === 'ongoing' || match.status === 'in_progress' ? 'Đang thi đấu' :
               'Chưa bắt đầu'}
            </div>
          </div>
        </div>
        ...
        <div className="text-center mt-3">
          ...
          >
            {match.status === 'completed' ? '🏆 Hoàn thành' :
             match.status === 'ongoing' ? '⚡ Đang thi đấu' :
             match.status === 'scheduled' ? '📅 Đã lên lịch' : match.status}
          </Badge>
        </div>
          ...
          <div className="mt-3 space-y-2">
            ...
            <Button 
              onClick={async () => {
                ...
                try {
                  console.log('🎯 [DoubleEliminationMatchCard] Submitting DE score:', {
                    matchId: match.id,
                  ...
                  if (error) {
                    console.error('❌ [DoubleEliminationMatchCard] RPC Error:', error);
                    throw error;
                  }

                  console.log('✅ [DoubleEliminationMatchCard] RPC Response:', data);

                  if (data && typeof data === 'object' && 'error' in data && data.error) {
                    console.error('❌ [DoubleEliminationMatchCard] RPC returned error:', data.error);
                    throw new Error(data.error as string);
                  }

                  // Check if tournament is complete
                  if (data && typeof data === 'object' && 'advancement' in data && data.advancement) {
                    const advancement = data.advancement as any;
                    console.log('🎯 [DoubleEliminationMatchCard] Advancement result:', advancement);
                    if (advancement.tournament_complete) {
                      toast.success('🏆 Giải đấu đã hoàn thành! Chúc mừng nhà vô địch!');
                    } else {
                      toast.success('🎯 Đã cập nhật tỷ số và tiến hành bracket!');
                    }
                  } else {
                  ...
                  setTimeout(() => {
                    console.log('🔄 [DoubleEliminationMatchCard] Database should have updated, realtime should refresh');
                  }, 500);
                } catch (error) {
