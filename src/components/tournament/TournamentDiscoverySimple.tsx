...
const TournamentDiscoverySimple = () => {
  ...
  const filteredTournaments = tournaments.filter(tournament => {
    ...
    if (filter === 'upcoming') return matchesSearch && tournament.status === 'upcoming';
    if (filter === 'open') return matchesSearch && tournament.status === 'registration_open';
    if (filter === 'ongoing') return matchesSearch && tournament.status === 'ongoing';
    ...
    <div className="container mx-auto p-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
        <Card>
          <CardContent className="flex items-center p-4">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground">tournament.stats.total</p>
            </div>
          </CardContent>
        </Card>
        ...
        <Card>
          <CardContent className="flex items-center p-4">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground">tournament.stats.active</p>
            </div>
          </CardContent>
        </Card>
        ...
        <Card>
          <CardContent className="flex items-center p-4">
            ...
            <div>
              <p className="text-2xl font-bold">{tournaments.filter(t => t.status === 'registration_open').length}</p>
              <p className="text-xs text-muted-foreground">tournament.stats.registered</p>
            </div>
          </CardContent>
        </Card>
      </div>
      ...
      {filteredTournaments.length === 0 ? (
        <div className="text-center py-12">
          ...
          <h3 className="text-xl font-semibold mb-2">Không tìm thấy giải đấu</h3>
          <p className="text-muted-foreground">
            {searchTerm ? 'Thử tìm kiếm với từ khóa khác' : 'Hiện tại chưa có giải đấu nào'}
          </p>
        </div>
      ) : (
            ...
            <Card key={tournament.id} className="hover:shadow-lg transition-shadow">
              ...
              <CardContent>
                <div className="space-y-3">
                  ...
                  >
                    {tournament.status === 'registration_open' ? 'Đăng ký' : 'Xem chi tiết'}
                  </Button>
                </div>
              </CardContent>
            </Card>
