...
export const TournamentMatchManager: React.FC<TournamentMatchManagerProps> = ({
  ...
  const stats = useMemo(() => {
    ...
    const needingPlayers = matches.filter(m => 
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      case 'completed': return 'bg-green-500/20 text-green-700 border-green-200';
      case 'ongoing': return 'bg-blue-500/20 text-blue-700 border-blue-200';
      case 'scheduled': return 'bg-orange-500/20 text-orange-700 border-orange-200';
  ...
  const getBracketTypeLabel = (bracketType: string) => {
    switch (bracketType) {
      ...
      case 'losers_branch_a': return 'Losers A';
      case 'losers_branch_b': return 'Losers B';
    ...
    <div className="space-y-6">
        ...
        <CardContent>
          <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
            ...
            <div className="flex items-center gap-2 p-3 bg-white/50 rounded-lg">
              ...
              <div>
                <div className="text-sm font-medium">{stats.needingPlayers}</div>
        ...
        <CardContent className="p-4">
          <div className="flex flex-wrap items-center gap-4">
            ...
            <div className="flex items-center gap-2">
              ...
              <Tabs value={activeRound.toString()} onValueChange={(value) => setActiveRound(value === 'all' ? 'all' : parseInt(value))}>
                <TabsList className="h-8">
                  <TabsTrigger value="all" className="text-xs">All</TabsTrigger>
                  {rounds.map(round => (
                    <TabsTrigger key={round} value={round.toString()} className="text-xs">
                      R{round}
                    </TabsTrigger>
                  ))}
                </TabsList>
      ...
      {activeRound === 'all' ? (
                  ...
                  <div className={`grid gap-4 ${
                    ...
                    {roundMatches.map(match => (
                      <DoubleEliminationMatchCard
                        ...
                        variant={match.bracket_type === 'winners' ? 'blue' : 
                                match.bracket_type === 'finals' ? 'gold' : 'orange'} // SABO_REBUILD: Updated bracket type
                      />
                    ))}
                  </div>
                </CardContent>
        ...
        <Card>
          <CardHeader>
            <CardTitle>
              {typeof activeRound === 'string' ? 'All Matches' : `Round ${activeRound} Matches`}
