...
export const EnhancedMatchCard: React.FC<EnhancedMatchCardProps> = ({
  ...
  const isParticipant = currentUserId && (match.player1_id === currentUserId || match.player2_id === currentUserId);
  ...
  const getMatchStatusBadge = () => {
    switch (match.status) {
      ...
      case 'ready':
        return <Badge className="bg-orange-500 hover:bg-orange-600">Sẵn sàng</Badge>;
      case 'scheduled':
    ...
    >
      <CardContent className="p-5">
        ...
        <div className="flex items-center justify-between mb-4">
          <Badge variant="outline" className="px-3 py-1 text-sm font-bold bg-primary/10 border-primary/30 text-primary">
        ...
        <div className="space-y-3">
          {/* Player 1 */}
          <div className={`p-3 rounded-xl border-l-4 transition-all duration-200 ${
            match.player1 ? 'bg-blue-50/50 border-blue-400 hover:bg-blue-50' : 'bg-muted/30 border-muted-foreground'
              ...
              <div className="flex items-center gap-3 flex-1">
                {match.player1 ? (
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <span className="text-sm text-muted-foreground font-medium">⏳ Chờ kết quả</span>
                  </div>
                )}
              </div>
            ...
            {isClubOwner && match.player1_id && match.player2_id && (
              <div className="mt-2 p-3 bg-blue-50 border border-blue-200 rounded-lg">
                <div className="flex items-center gap-3">
                  <span className="text-sm font-medium text-blue-700 min-w-[60px]">Tỷ số:</span>
                  <input
          ...
          {/* Player 2 */}
          <div className={`p-3 rounded-xl border-l-4 transition-all duration-200 ${
            match.player2 ? 'bg-purple-50/50 border-purple-400 hover:bg-purple-50' : 'bg-muted/30 border-muted-foreground'
            ...
            {isClubOwner && match.player1_id && match.player2_id && (
              <div className="mt-2 p-3 bg-purple-50 border border-purple-200 rounded-lg">
                <div className="flex items-center gap-3">
                  <span className="text-sm font-medium text-purple-700 min-w-[60px]">Tỷ số:</span>
                  <input
                    ...
                    tabIndex={0}
                  />
                  <span className="text-xs text-purple-600 font-medium">P2</span>
                </div>
              </div>
            )}
          </div>
        ...
        <div className={`mt-4 p-3 rounded-lg border ${
          ...
          <div className="text-center">
            <div className={`text-lg font-bold mb-1 ${
              ...
              Kết quả: {getDisplayScore(match.score_player1)} - {getDisplayScore(match.score_player2)}
            </div>
          ...
          <div className="mt-4 space-y-3">
            ...
            <Button 
              ...
              onClick={async (e) => {
                ...
                try {
                  console.log('🎯 [EnhancedMatchCard] Submitting match score via RPC:', {
                    matchId: match.id,
                    score1,
                    score2,
                    currentUserId
                  ...
                  if (error) {
                    console.error('❌ [EnhancedMatchCard] Error calling submit_match_score RPC:', error);
                    throw error;
                  }
                  ...
                  if (data && typeof data === 'object' && 'tournament_complete' in data && data.tournament_complete) {
