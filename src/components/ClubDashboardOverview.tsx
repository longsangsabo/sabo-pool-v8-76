      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          ...
          <CardContent className="p-3">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-green-600">+{data.memberStats.thisMonth}</p>
              </div>
        ...
        <Card className="ultra-compact-stats-card">
          <CardContent className="p-3">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-xs font-medium text-muted-foreground">Trận đấu</p>
                <p className="text-xl font-bold">{data.matchStats.total}</p>
                <p className="text-xs text-blue-600">+{data.matchStats.thisWeek}</p>
              </div>
        ...
        <Card className="ultra-compact-stats-card">
          <CardContent className="p-3">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-xs font-medium text-muted-foreground">Giải đấu</p>
                <p className="text-xl font-bold">{data.tournamentStats.hosted}</p>
        ...
        <Card className="ultra-compact-stats-card">
          <CardContent className="p-3">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-xs font-medium text-muted-foreground">Tin cậy</p>
                <p className="text-xl font-bold text-yellow-600">{data.trustScore.toFixed(1)}%</p>
                <p className="text-xs text-muted-foreground">Đánh giá</p>
              </div>
              <TrendingUp className="h-8 w-8 text-muted-foreground" />
            </div>
            <Badge 
              variant={data.trustScore >= 90 ? 'default' : data.trustScore >= 75 ? 'secondary' : 'destructive'}
              className="text-xs mt-1 px-1 py-0"
            >
              {data.trustScore >= 90 ? 'Xuất sắc' : data.trustScore >= 75 ? 'Tốt' : 'Cần cải thiện'}
            </Badge>
          </CardContent>
        </Card>
      </div>
            ...
            {data.pendingVerifications.length === 0 ? (
              ...
              <div className="space-y-2">
                {data.pendingVerifications.slice(0, 3).map((verification) => (
                  <div key={verification.id} className="flex items-center justify-between p-2 border rounded-md text-sm">
                    <div className="flex-1 min-w-0">
                      <p className="font-medium truncate">
                        {verification.profiles?.display_name || verification.profiles?.full_name || 'Người chơi'}
                      </p>
                    ...
                    <Badge variant="outline" className="text-xs px-1 py-0">
                      {new Date(verification.created_at).toLocaleDateString('vi-VN', { month: 'short', day: 'numeric' })}
                    </Badge>
                  </div>
                ))}
