...
import {
  ...
  BulkAction,
} from '@/features/admin/components/shared/AdminBulkActions';
...
const TournamentParticipantManager = ({
  tournament,
  isOpen,
  onClose,
  ...
  const bulkActions: BulkAction[] = [
    ...
    },
    {
      ...
      onExecute: async (ids: string[]) => {
        ...
        const csv = selectedParticipants
          .map(
            p =>
  ...
  const actions = [
    ...
    },
    {
      ...
      icon: <CheckCircle className='h-4 w-4' />,
      condition: (participant: TournamentParticipant) =>
  ...
  const stats = {
    ...
    confirmed: participants.filter(p => p.registration_status === 'confirmed')
      .length,
    pending: participants.filter(p => p.registration_status === 'pending')
      .length,
    waitlisted: participants.filter(p => p.registration_status === 'waitlisted')
      .length,
