...
interface TournamentStats {
  ...
  historicalComparison: { metric: string; current: number; previous: number; change: number }[];
}

const TournamentAnalyticsDashboard: React.FC<TournamentAnalyticsDashboardProps> = ({
  tournament,
  onClose
}) => {
  ...
  const stats: TournamentStats = {
    ...
    historicalComparison: [
      ...
      { metric: 'Prize Pool', current: 50000000, previous: 35000000, change: 42.9 },
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-green-600">
                  +{((stats.totalParticipants / 95 - 1) * 100).toFixed(1)}% vs last tournament
                </p>
              </div>
          ...
          <CardContent className="p-4">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold">{formatCurrency(stats.totalPrizePool)}</p>
                <p className="text-xs text-green-600">
                  +{((stats.totalPrizePool / 35000000 - 1) * 100).toFixed(1)}% vs last tournament
                </p>
              </div>
          ...
          <CardContent className="p-4">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold">{formatNumber(stats.averageElo)}</p>
                <p className="text-xs text-green-600">
                  +{((stats.averageElo / 1580 - 1) * 100).toFixed(1)}% vs last tournament
                </p>
              </div>
        ...
        <Card>
          <CardContent className="p-4">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm text-muted-foreground">Registration Progress</p>
                <p className="text-2xl font-bold">{stats.registrationProgress}%</p>
                <p className="text-xs text-muted-foreground">
                  {stats.confirmedParticipants}/{stats.totalParticipants} confirmed
                </p>
              </div>
                ...
                <div key={index} className="flex items-center justify-between">
                  ...
                  <div className="text-right">
                    ...
                    <div className="text-xs text-muted-foreground">
                      {((item.count / stats.totalParticipants) * 100).toFixed(1)}%
                    </div>
                  </div>
                </div>
              ))}
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {stats.historicalComparison.map((item, index) => (
              <div key={index} className="p-4 border rounded-lg">
                ...
                <div className="text-xs text-muted-foreground">
                  Previous: {item.metric === 'Prize Pool' 
