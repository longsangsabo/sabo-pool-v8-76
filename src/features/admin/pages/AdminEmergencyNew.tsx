...
interface EmergencyAction {
  ...
  title: string;
  description: string;
  type: 'immediate' | 'investigation' | 'communication' | 'mitigation' | 'prevention';
...
const AdminEmergencyNew = () => {
  ...
  const [incidents, setIncidents] = useState<EmergencyIncident[]>([
    {
      ...
      priority: 'p1',
      impact: 'system_wide',
      affectedServices: ['user_authentication', 'tournament_system', 'payment_processing'],
      affectedUsers: 2340,
      ...
      timeline: [
        ...
        },
        {
          ...
          type: 'action_taken',
          content: 'Increased connection pool size temporarily. Investigating root cause.'
        },
        {
          id: 'update_003',
          ...
          type: 'comment',
          content: 'Found memory leak in tournament service causing connections to not be released properly.'
        }
      ],
      actions: [
        {
          ...
          description: 'Temporarily increase database connection pool size to handle load',
          type: 'immediate',
          status: 'completed',
          assignee: 'Database Admin',
          priority: 'high',
          completedAt: new Date('2025-08-03T14:45:00'),
          results: 'Connection pool increased from 100 to 200. Service restored.'
        },
        {
          id: 'action_002',
    ...
    },
    {
      ...
      description: 'Automated fraud detection system flagged unusual payment patterns indicating potential fraudulent activity.',
      severity: 'high',
      category: 'security',
      status: 'resolving',
      priority: 'p2',
      ...
      timeline: [
        {
          ...
          type: 'status_change',
          content: 'Suspicious payment pattern detected - Multiple failed high-value transactions'
        },
        {
          id: 'update_005',
    ...
    },
    {
      ...
      description: 'Hardware failure detected on application server node 3. System has automatically failed over to backup nodes.',
      severity: 'medium',
      category: 'technical',
      status: 'resolved',
      priority: 'p2',
      ...
      timeline: [
        ...
        },
        {
          ...
          type: 'action_taken',
          content: 'Node 3 removed from load balancer. Hardware replacement scheduled.'
        }
      ],
      actions: [
        {
  ...
  const [procedures] = useState<EmergencyProcedure[]>([
    {
      ...
      description: 'Standard procedure for handling database connectivity issues and outages',
      category: 'technical',
      severity: 'critical',
      steps: [
        {
    ...
    },
    {
      ...
      description: 'Immediate response protocol for suspected security breaches',
      category: 'security',
      severity: 'critical',
      steps: [
        {
  ...
  const handleIncidentStatusChange = (incidentId: string, newStatus: string) => {
    setIncidents(prev =>
      prev.map(incident => {
  ...
  const activeIncidents = incidents.filter(i => ['open', 'investigating', 'resolving'].includes(i.status));
        ...
        <div className="space-y-6">
          ...
          {alertMode && (
            <Alert className="border-red-200 bg-red-50">
              ...
              <AlertTitle className="text-red-800">Emergency Alert Mode Active</AlertTitle>
              <AlertDescription className="text-red-700">
                Emergency response protocols are now active. All critical notifications will be sent immediately.
                Emergency contacts will be notified of any new critical incidents.
              </AlertDescription>
            </Alert>
          )}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            {stats.map((stat, index) => (
              <Card key={index} className={activeIncidents.length > 0 && stat.title === 'Active Incidents' ? 'border-red-200' : ''}>
                      ...
                      {filteredIncidents.length === 0 && (
                        <div className="text-center py-8">
                          ...
                          <p className="text-muted-foreground">
                            {searchQuery || filterSeverity !== 'all' || filterStatus !== 'all' || filterCategory !== 'all'
                  ...
                  {selectedIncident ? (
                    <Card>
                        ...
                        <div className="flex items-center justify-between">
                          ...
                          <div className="flex items-center gap-2">
                            {selectedIncident.status === 'open' && (
                              <Button 
                                size="sm"
                                onClick={() => handleIncidentStatusChange(selectedIncident.id, 'investigating')}
                              >
                            ...
                            {selectedIncident.status === 'investigating' && (
                              <Button 
                                size="sm"
                                onClick={() => handleIncidentStatusChange(selectedIncident.id, 'resolving')}
                              >
                      ...
                      <CardContent className="space-y-6">
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                          <div>
                            ...
                            <div className="mt-1">
                              <Badge className={getSeverityColor(selectedIncident.severity)}>
                                {getSeverityIcon(selectedIncident.severity)}
                                <span className="ml-1 capitalize">{selectedIncident.severity}</span>
                              </Badge>
                            </div>
                          </div>
                          <div>
                            ...
                            <div className="mt-1">
                              <Badge className={getCategoryColor(selectedIncident.category)}>
                                ...
                                <span className="ml-1 capitalize">{selectedIncident.category}</span>
                              </Badge>
                            </div>
                          </div>
                          <div>
                          ...
                          <div>
                            <Label className="text-sm text-muted-foreground">Affected Users</Label>
                        ...
                        {selectedIncident.affectedServices.length > 0 && (
                          <div>
                            <Label className="text-sm text-muted-foreground">Affected Services</Label>
                              ...
                              {selectedIncident.actions.map((action) => (
                                <div key={action.id} className="p-3 border rounded-lg">
                                  ...
                                  <div className="flex items-center justify-between text-sm">
                                    ...
                                    {action.deadline && (
                                      <span>Due: {action.deadline.toLocaleString()}</span>
                                    )}
                                  </div>
                                  {action.results && (
                        ...
                        <div className="text-sm text-muted-foreground">
                          <div>Created: {selectedIncident.createdAt.toLocaleString()} by {selectedIncident.reporter}</div>
                          <div>Updated: {selectedIncident.updatedAt.toLocaleString()}</div>
                    ...
                    <Card>
                      <CardContent className="p-12 text-center">
                        ...
                        <p className="text-muted-foreground">
                          Choose an incident to view details and manage response actions.
                        </p>
                      </CardContent>
                    </Card>
                  )}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {procedures.map((procedure) => (
                  <Card key={procedure.id}>
                    <CardHeader>
                      <div className="flex items-center justify-between">
                        <CardTitle className="text-lg line-clamp-1">{procedure.name}</CardTitle>
                        <Badge className={getSeverityColor(procedure.severity)}>
                          {getSeverityIcon(procedure.severity)}
                          <span className="ml-1 capitalize">{procedure.severity}</span>
                        </Badge>
                      </div>
                    ...
                    <CardContent className="space-y-4">
                      <div className="flex items-center justify-between">
                        ...
                        <span className="text-sm font-medium">{formatDuration(procedure.estimatedTime)}</span>
                      </div>
                    ...
                    <CardContent className="space-y-4">
                      <div className="space-y-2">
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-sm capitalize">{contact.availability.replace('_', ' ')}</span>
                        </div>
                      </div>
            ...
            <TabsContent value="settings">
              <Card>
                <CardContent className="p-12 text-center">
                  ...
                  <h3 className="text-lg font-medium mb-2">Emergency Settings</h3>
                  <p className="text-muted-foreground">
                    Configure emergency response settings, escalation rules, and notification preferences.
                  </p>
                </CardContent>
              </Card>
            </TabsContent>
