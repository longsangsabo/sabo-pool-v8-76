...
const AdminDatabaseNew = () => {
  ...
  const tables = [
    ...
    { name: 'tournaments', rows: 234, size: '12.8 MB', last_update: '5 mins ago' },
    { name: 'clubs', rows: 89, size: '3.4 MB', last_update: '1 hour ago' },
    { name: 'transactions', rows: 8932, size: '67.1 MB', last_update: '30 secs ago' },
    { name: 'challenges', rows: 1567, size: '8.9 MB', last_update: '15 mins ago' }
  ];
  ...
  const executeQuery = async () => {
    ...
    try {
      ...
      const newResult: QueryResult = {
        ...
        timestamp: new Date(),
        error: Math.random() > 0.2 ? undefined : 'Syntax error: unexpected token'
      };

      setQueryResults(prev => [newResult, ...prev.slice(0, 9)]);
      
      if (newResult.status === 'success') {
        toast.success(`Query executed successfully in ${newResult.duration.toFixed(2)}ms`);
      } else {
                        ...
                        <div key={result.id} className="p-4 border rounded-lg">
                          ...
                          {result.status === 'success' ? (
                            <div className="text-sm text-muted-foreground mt-2">
                              Returned {result.rows} rows in {result.duration.toFixed(2)}ms
                            </div>
                          ) : (
                      ...
                      <div key={index} className="flex items-center justify-between p-4 border rounded-lg">
                        <div className="space-y-1">
                          ...
                          <div className="text-sm text-muted-foreground">
                            {table.rows.toLocaleString()} rows • {table.size} • Updated {table.last_update}
                          </div>
                        </div>
                        <div className="flex items-center gap-2">
                          <Button variant="outline" size="sm">
                    ...
                    {backups.map((backup) => (
                      <div key={backup.id} className="flex items-center justify-between p-4 border rounded-lg">
                        <div className="space-y-1">
                          ...
                          <div className="text-sm text-muted-foreground">
                            {backup.size} • {backup.date.toLocaleString()} • {backup.type} backup
                          </div>
                        </div>
                        <div className="flex items-center gap-2">
