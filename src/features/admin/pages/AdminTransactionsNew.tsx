...
const AdminTransactionsNew = () => {
  ...
  return (
    <AdminCoreProvider>
      ...
      >
        {/* Stats Grid */}
        <AdminStatsGrid stats={statsConfig} />
          ...
          <Tabs value={activeTab} onValueChange={setActiveTab}>
            <TabsList className='grid w-full grid-cols-5'>
              <TabsTrigger value='all'>
                {t('transactions.all_transactions')} ({transactions.length})
              </TabsTrigger>
...
const TransactionManagementTableWithBulkActions = ({
    ...
    },
    {
      ...
      onExecute: async (ids: string[]) => {
        const selectedTransactions = transactions.filter(t =>
          ids.includes(t.transaction_id)
        );
        const csv = selectedTransactions
          .map(
            t =>
    ...
    },
    {
      ...
      render: (_, transaction) => (
        <div className='flex items-center space-x-3'>
          ...
          <div>
            <div className='font-medium text-sm'>
              {transaction.user_name || 'Chưa xác định'}
            </div>
            <div className='text-xs text-gray-500'>
  ...
  const actions = [
    ...
    },
    {
      ...
      icon: <CheckCircle className='h-4 w-4' />,
      condition: (transaction: AdminTransaction) =>
