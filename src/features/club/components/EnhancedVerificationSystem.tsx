...
export const EnhancedVerificationSystem = () => {
  ...
  const getSubmissionTypeBadge = (
    type: VerificationRequest['submission_type']
  ) => {
    const variants = {
            ...
            <div className='w-full bg-gray-200 rounded-full h-2'>
              <div
                className={`h-2 rounded-full ${
                  request.elo_current >= request.elo_required
                    ? 'bg-green-500'
                    : 'bg-blue-500'
                }`}
                style={{
                  width: `${Math.min((request.elo_current / request.elo_required) * 100, 100)}%`,
                }}
              ></div>
            </div>
          </div>
  ...
  const reviewRequests = verificationRequests.filter(
    r => r.status === 'requires_review'
  );
  const completedRequests = verificationRequests.filter(r =>
      ...
      <div className='flex justify-between items-center'>
        <div>
          ...
          <p className='text-muted-foreground'>
            Quản lý và xét duyệt các yêu cầu nâng cấp trình độ
          </p>
        </div>
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab}>
        <TabsList>
          ...
          <TabsTrigger value='review'>
            Cần xem lại ({reviewRequests.length})
          </TabsTrigger>
