...
export const Settings: React.FC<SettingsProps> = ({ clubId }) => {
  const [settings, setSettings] = useState<ClubSettings>({
    name: 'Sabo Pool Club',
    description: 'CLB Pool chuyên nghiệp với không gian hiện đại và dịch vụ tốt nhất',
    ...
    <div className="space-y-6">
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          ...
          <div>
            <label className="block text-sm font-medium mb-3">Ngày hoạt động</label>
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div>
              <label className="block text-sm font-medium mb-2">Giá bàn thường (VNĐ/giờ)</label>
              <Input
                type="number"
                value={settings.standardRate}
                onChange={(e) => handleInputChange('standardRate', parseInt(e.target.value))}
                placeholder="50000"
              />
            </div>
            <div>
              <label className="block text-sm font-medium mb-2">Giá bàn VIP (VNĐ/giờ)</label>
              <Input
                type="number"
                value={settings.vipRate}
                onChange={(e) => handleInputChange('vipRate', parseInt(e.target.value))}
                placeholder="80000"
              />
            </div>
            <div>
              <label className="block text-sm font-medium mb-2">Giảm giá thành viên (%)</label>
              <Input
                type="number"
                value={settings.memberDiscount}
                onChange={(e) => handleInputChange('memberDiscount', parseInt(e.target.value))}
                placeholder="10"
                min="0"
                max="100"
              />
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <label className="flex items-center gap-3 cursor-pointer">
              <input
                ...
                onChange={(e) => handleInputChange('allowReservations', e.target.checked)}
                className="w-4 h-4"
              />
            ...
            <label className="flex items-center gap-3 cursor-pointer">
              <input
                ...
                onChange={(e) => handleInputChange('requireMembership', e.target.checked)}
                className="w-4 h-4"
              />
            ...
            <label className="flex items-center gap-3 cursor-pointer">
              <input
                ...
                onChange={(e) => handleInputChange('enableTournaments', e.target.checked)}
                className="w-4 h-4"
              />
            ...
            <label className="flex items-center gap-3 cursor-pointer">
              <input
                ...
                onChange={(e) => handleInputChange('autoCalculateElo', e.target.checked)}
                className="w-4 h-4"
              />
