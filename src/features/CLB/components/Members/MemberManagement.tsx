...
import { Users, Search, MessageSquare, UserCheck, Plus, Filter, Edit, Trash2, Eye } from 'lucide-react';
...
export const MemberManagement: React.FC<MemberManagementProps> = ({ clubId }) => {
  const { members, loading, error, addMember, updateMember, removeMember, getMemberStats } = useClubMembers(clubId);
  ...
  const handleAddMember = async () => {
    try {
      ...
      setNewMemberData({ user_id: '', membership_type: 'regular', membership_fee: 0 });
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <Card>
          <CardContent className="p-6">
            <div className="flex items-center">
              <div className="ml-4">
                <p className="text-sm font-medium text-muted-foreground">Doanh thu thành viên</p>
              ...
              <DialogContent>
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    <Select value={newMemberData.membership_type} onValueChange={(value) => setNewMemberData({...newMemberData, membership_type: value})}>
                      <SelectTrigger>
                        <SelectValue />
                      </SelectTrigger>
                      <SelectContent>
                  ...
                  <div>
                    ...
                    <Input
                      ...
                      onChange={(e) => setNewMemberData({...newMemberData, membership_fee: parseInt(e.target.value)})}
                      placeholder="0"
                    />
                  </div>
              ...
              <Card key={member.id} className="hover:shadow-md transition-shadow">
                <CardContent className="p-4">
                    ...
                    <div className="flex items-center space-x-4">
                      ...
                      <div className="space-y-1">
                        ...
                        <div className="flex items-center gap-2 text-sm text-muted-foreground">
                          <span>📞 {member.profiles?.phone || 'Chưa có SĐT'}</span>
                          <span>•</span>
                          <span>Tham gia: {member.join_date ? new Date(member.join_date).toLocaleDateString('vi-VN') : 'N/A'}</span>
                        </div>
                  ...
                  <div className="mt-4 grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                    <div>
                      <span className="text-muted-foreground">Tổng giờ chơi:</span>
                      <span className="ml-2 font-medium">{member.total_hours_played || 0}h</span>
                    </div>
                    <div>
                      <span className="text-muted-foreground">Lượt thăm:</span>
                      <span className="ml-2 font-medium">{member.total_visits || 0}</span>
                    </div>
                    <div>
                      <span className="text-muted-foreground">Phí thành viên:</span>
                      <span className="ml-2 font-medium">{member.membership_fee?.toLocaleString() || 0}đ</span>
                    </div>
                    <div>
                      <span className="text-muted-foreground">Lần cuối:</span>
                      <span className="ml-2 font-medium">
                        {member.last_visit ? new Date(member.last_visit).toLocaleDateString('vi-VN') : 'Chưa có'}
                      </span>
                    </div>
                  </div>
                </CardContent>
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="mt-1 text-xs text-muted-foreground">
              ...
              <div className="flex items-center gap-2 mt-1">
                ...
                <span>{member.status === 'active' ? 'Hoạt động' : 'Không hoạt động'}</span>
              </div>
            </div>
