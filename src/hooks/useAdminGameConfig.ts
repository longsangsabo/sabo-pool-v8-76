...
type RankDefinitionRow = Database['public']['Tables']['rank_definitions']['Row'];
...
export const useAdminGameConfig = () => {
  ...
  const createConfig = useCallback(async (configData: AdminGameConfigData) => {
    try {
      setLoading(true);
      setError(null);
  ...
  const updateConfig = useCallback(async (id: string, updates: Partial<AdminGameConfigData>) => {
    try {
      setLoading(true);
      setError(null);
  ...
  // Create ELO rule
  const createEloRule = useCallback(async (ruleData: AdminEloRuleData) => {
    try {
      setLoading(true);
      setError(null);
  ...
  // Update ELO rule
  const updateEloRule = useCallback(async (id: string, updates: Partial<AdminEloRuleData>) => {
    try {
      setLoading(true);
      setError(null);
  ...
  // Delete ELO rule
  const deleteEloRule = useCallback(async (id: string) => {
    try {
      setLoading(true);
      setError(null);
  ...
  const createRank = useCallback(async (rankData: AdminRankData) => {
    try {
      setLoading(true);
      setError(null);
  ...
  const updateRank = useCallback(async (id: string, updates: Partial<AdminRankData>) => {
    try {
      setLoading(true);
      setError(null);
  ...
  const getConfigStats = useCallback(async (): Promise<GameConfigStats> => {
    try {
      ...
      if (totalConfigs === 0 || totalEloRules === 0 || totalRanks === 0) {
        systemStatus = 'error';
      } else if (activeConfigs < totalConfigs * 0.8 || activeEloRules < totalEloRules * 0.8) {
  ...
  // Bulk operations
  const bulkActivateConfigs = useCallback(async (ids: string[]) => {
    try {
      setLoading(true);
      const { error } = await supabase
        .from('game_configurations')
  ...
  const bulkDeactivateConfigs = useCallback(async (ids: string[]) => {
    try {
      setLoading(true);
      const { error } = await supabase
        .from('game_configurations')
