...
export const useMonitoring = () => {
  ...
  const trackTournamentRegistration = useCallback((tournamentId: string, tournamentName: string, entryFee: number) => {
    trackEvent('tournament_registration', { tournamentId, tournamentName, entryFee });
  }, [trackEvent]);

  const trackChallengeCreated = useCallback((challengeType: string, stakeAmount?: number) => {
    trackEvent('challenge_created', { challengeType, stakeAmount });
  }, [trackEvent]);

  const trackMatchResult = useCallback((matchId: string, isWinner: boolean, matchDuration: number) => {
    trackEvent('match_result', { matchId, isWinner, matchDuration });
  }, [trackEvent]);

  const trackPayment = useCallback((amount: number, paymentMethod: string, transactionType: string) => {
    trackEvent('payment', { amount, paymentMethod, transactionType });
  }, [trackEvent]);

  const trackClubJoin = useCallback((clubId: string, clubName: string) => {
  ...
  const trackSearchQuery = useCallback((query: string, resultCount: number, category?: string) => {
