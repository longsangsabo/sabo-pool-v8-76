...
export interface TournamentRegistration
  extends Omit<TournamentRegistrationRow, 'user_id'> {
  user_id: string;
  user?: {
    id: string;
    full_name: string;
...
export const useAdminTournaments = () => {
  ...
  const createTournament = useCallback(
    async (tournamentData: AdminTournamentData) => {
      try {
        setLoading(true);
        setError(null);
  ...
  const updateTournament = useCallback(
    async (id: string, updates: Partial<AdminTournamentData>) => {
      try {
        setLoading(true);
        setError(null);
  ...
  const getTournamentRegistrations = useCallback(
    async (tournamentId: string): Promise<TournamentRegistration[]> => {
      try {
        const { data, error } = await supabase
  ...
  const getTournamentStats = useCallback(async (): Promise<TournamentStats> => {
    try {
      ...
      const upcoming =
        tournamentsData?.filter(t => t.status === 'registration_open').length ||
        0;
      const active =
      ...
      const cancelled =
        tournamentsData?.filter(t => t.status === 'cancelled').length || 0;
      ...
      const total_participants = Object.values(participantsByTournament).reduce(
        (sum, count) => sum + count,
        0
      );
      ...
      // Calculate revenue
      const total_revenue =
        tournamentsData?.reduce((sum, tournament) => {
          const participants = participantsByTournament[tournament.id] || 0;
          return sum + participants * (tournament.entry_fee || 0);
        }, 0) || 0;

      const avg_participants_per_tournament =
        total > 0 ? total_participants / total : 0;
      ...
      return {
        ...
        avg_participants_per_tournament:
          Math.round(avg_participants_per_tournament * 100) / 100,
      };
    } catch (err: any) {
