...
export const useClubDashboard = () => {
    ...
    const setupRealtimeSubscriptions = async () => {
      try {
        ...
        if (!clubData) return;
         ...
         channels.push(challengesChannel, notificationChannel, tournamentsChannel, rankRequestsChannel, accountabilityChannel);
  ...
  const fetchDashboardData = async () => {
    ...
    try {
      ...
      const { data: statsData, error: statsError } = await supabase.rpc('calculate_club_dashboard_stats', {
        p_club_id: clubId
      });
      ...
      const [
        ...
        supabase
          .from('notifications')
          .select('*')
          .or(`club_id.eq.${clubId},and(type.in.(rank_verification_request,challenge_at_club,member_join),metadata->>club_id.eq.${clubId})`)
          .order('created_at', { ascending: false })
          .limit(10)
      ]);
