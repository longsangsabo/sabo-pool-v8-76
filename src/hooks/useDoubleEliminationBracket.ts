...
import { SABOLogicCore, SABO_STRUCTURE, type SABOMatch, type SABOOrganizedMatches } from '@/tournaments/sabo/SABOLogicCore';
...
export const useDoubleEliminationBracket = (tournamentId: string) => {
  ...
  const { matches, loading, error, refetch } = useTournamentMatches(tournamentId);
  ...
  const createBracket = useMutation({
    ...
    onSuccess: (data, variables) => {
      ...
      if (data && typeof data === 'object' && 'success' in data && data.success) {
        ...
        toast.success(`🏆 Bracket created with ${successData.total_matches} matches!`);
      } else {
  ...
  const submitScore = useMutation({
    mutationFn: async ({ matchId, player1Score, player2Score }: SubmitScoreParams) => {
      console.log('🎯 Submitting score:', { matchId, player1Score, player2Score });
  ...
  const checkProgression = async (): Promise<AutomationResult | null> => {
    try {
      const { data, error } = await supabase.rpc('get_double_elimination_status', {
        p_tournament_id: tournamentId
      });
  ...
  const organizedMatches: SABOOrganizedMatches = SABOLogicCore.organizeMatches(matches as SABOMatch[]);

  // Validate SABO structure
  const saboValidation = SABOLogicCore.validateSABOStructure(matches as SABOMatch[]);
  ...
  const saboProgress = SABOLogicCore.getTournamentProgress(matches as SABOMatch[]);
  const { totalMatches, completedMatches, progressPercentage, currentStage } = saboProgress;
  ...
  return {
    ...
    // Status
    isLoading: loading || createBracket.isPending || submitScore.isPending || advanceWinner.isPending
  };
};