...
export const useRankNotificationAutomation = () => {
  ...
  useEffect(() => {
    ...
    console.log('[RankNotificationAutomation] Setting up rank approval listener for user:', user.id);
    ...
    const notificationChannel = supabase
      ...
      .on(
        ...
        (payload) => {
          console.log('[RankNotificationAutomation] Notification received:', payload);
          ...
          if (notification.type === 'rank_approved' || 
            ...
            console.log('[RankNotificationAutomation] Rank approval detected, triggering profile refresh');
            ...
            setTimeout(() => {
              console.log('[RankNotificationAutomation] Backup refresh triggered');
              refreshProfile();
            }, 2000);
            
            // Final refresh after 5 seconds to ensure consistency
            setTimeout(() => {
              console.log('[RankNotificationAutomation] Final refresh triggered');
              refreshProfile();
            }, 5000);
          }
        }
      ...
      .subscribe((status) => {
        console.log('[RankNotificationAutomation] Notification subscription status:', status);
      });

    return () => {
      console.log('[RankNotificationAutomation] Cleaning up notification listener');
