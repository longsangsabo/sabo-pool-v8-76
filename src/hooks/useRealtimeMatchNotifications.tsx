...
export const useRealtimeMatchNotifications = () => {
  const { user } = useAuth();
  const [currentMatchNotification, setCurrentMatchNotification] = useState<MatchNotification | null>(null);

  const isUserInMatch = useCallback((match: any, userId: string) => {
    return match.player1_id === userId || match.player2_id === userId;
  }, []);

  const getOpponentData = useCallback(async (match: any, currentUserId: string) => {
  ...
  const openMatchNotification = useCallback(async (matchId: string) => {
    if (!user?.id) return;
    ...
    try {
      ...
      const [opponent, tournament, player1Profile, player2Profile] = await Promise.all([
  ...
  }, [user?.id, isUserInMatch, getOpponentData, getTournamentData, getTableData, getUserProfile]);
