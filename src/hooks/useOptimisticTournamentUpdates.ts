...
export const useOptimisticTournamentUpdates = () => {
  const [optimisticUpdates, setOptimisticUpdates] = useState<Map<string, OptimisticMatchUpdate>>(new Map());
  const [pendingAdvancements, setPendingAdvancements] = useState<Set<string>>(new Set());

  const applyOptimisticMatchUpdate = useCallback((update: OptimisticMatchUpdate) => {
  ...
  const getOptimisticMatch = useCallback((matchId: string) => {
    return optimisticUpdates.get(matchId);
  }, [optimisticUpdates]);

  const isPendingAdvancement = useCallback((matchId: string) => {
