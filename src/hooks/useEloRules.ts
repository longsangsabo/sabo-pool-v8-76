...
import type { EloRule, EloRuleFormData, EloSystemInfo, EloValidationResult } from '@/types/elo';
  ...
  const validateSystem = async (): Promise<EloValidationResult> => {
    try {
      ...
      const inconsistencies = data?.filter(player => {
        ...
        const currentRank = Array.isArray(player.ranks) ? player.ranks[0]?.code : (player.ranks as any)?.code;
