...
export const useAvailableUsers = () => {
  ...
  const generateUsers = useCallback(async (
    ...
    try {
      console.log(`🎯 Generating ${targetCount} users for tournament ${tournamentId}`);
      ...
      const excludedIds = existingRegistrations?.map(reg => reg.user_id) || [];
      ...
      if (allPlayers) {
        const users = allPlayers?.map(player => ({
          id: player.user_id,
        ...
        if (skillLevel) {
          filteredUsers = mergedUsers.filter(user => user.skill_level === skillLevel);
        }
        ...
        console.log(`✅ Selected ${selectedUsers.length} users for tournament`);
