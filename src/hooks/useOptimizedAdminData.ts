    ...
    const results = await Promise.allSettled(
      queries.map(async (query) => {
        ...
        let supabaseQuery = query.count 
          ? supabase.from(query.table as any).select(query.select, { count: 'exact' })
  ...
  const fetchDashboardStats = useCallback(async () => {
    try {
      ...
      const queries: BatchQuery[] = [
        ...
        { key: 'pendingClubs', table: 'club_registrations', select: 'status', filters: { status: 'pending' }, count: true },
        { key: 'approvedClubs', table: 'club_profiles', select: '*', count: true },
        { key: 'activeTournaments', table: 'tournaments', select: 'status', filters: { status: 'active' }, count: true },
        { key: 'totalTournaments', table: 'tournaments', select: '*', count: true },
        { key: 'transactions', table: 'payment_transactions', select: 'amount, status, created_at' }
      ];
      ...
      const totalRevenue = successfulTransactions.reduce((sum: number, t: any) => sum + (t.amount || 0), 0);
