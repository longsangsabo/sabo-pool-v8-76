...
export const useUnifiedTournamentState = (tournamentId?: string) => {
  ...
  const { processAutomaticAdvancement, isProcessing: isAutoAdvancing } = useAutoAdvancement();
  ...
  return {
    ...
    matchesLoading,
    isAnyLoading: tournament.isLoading || registrationsLoading || matchesLoading || isAutoAdvancing,
    isAutoAdvancing,
    ...
    registrationErrors,
    matchErrors,
    hasErrors: tournament.hookErrors.length > 0 || registrationErrors.length > 0 || matchErrors.length > 0,
    ...
    refreshAll: async () => {
      ...
      if (effectiveTournamentId) {
        ...
        try {
          const { data: repairResult } = await supabase.rpc('repair_double_elimination_bracket', {
