...
export const useAutoMatchNotifications = () => {
  const { user } = useAuth();
  const [currentAutoNotification, setCurrentAutoNotification] = useState<AutoMatchNotification | null>(null);

  // Function to create and show auto notification for tournament matches
  const createMatchNotification = useCallback(async (match: any, type: 'match_scheduled' | 'match_starting' | 'match_ready') => {
    if (!user?.id) return;
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on('postgres_changes', {
        ...
        if (match.status === 'scheduled' && (match.player1_id === user.id || match.player2_id === user.id)) {
