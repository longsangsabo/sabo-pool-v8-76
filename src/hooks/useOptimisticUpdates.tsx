...
export const useOptimisticUpdates = () => {
  ...
  const performOptimisticUpdate = async <T,>(
    ...
    const { queryKeys, operation, entityId, optimisticUpdate, rollbackMessage } = config;
      ...
      if (optimisticUpdate && previousData[key]) {
        ...
        if (operation === 'delete' && previousData[key]) {
          if (Array.isArray(previousData[key])) {
            ...
            queryClient.setQueryData(queryKey, {
              ...previousData[key],
              tournaments: previousData[key].tournaments.filter((item: any) => item.id !== entityId)
            });
          }
        }
      }
    ...
    try {
      ...
      toast.error(rollbackMessage || 'Thao tác thất bại, đã khôi phục trạng thái trước đó');
      throw error;
    }
  };
  ...
  const createStandardOptimisticUpdate = (entityType: string) => {
    return (entityId: string, updates: any, asyncUpdateFn: () => Promise<any>) => {
      return performOptimisticUpdate(
        {
          queryKeys: [
            [`${entityType}s`],
