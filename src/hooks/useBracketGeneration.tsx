...
import { callTournamentFunction, TOURNAMENT_FUNCTIONS, getFunctionForTournamentType } from '@/services/tournament/TournamentFunctionResolver';
...
export const useBracketGeneration = () => {
    ...
    try {
      ...
      if (tournament.tournament_type === 'double_elimination') {
        ...
        if (regError || !registrations || registrations.length !== 16) {
          toast.error(`SABO Double Elimination cần đúng 16 người chơi đã thanh toán. Hiện có: ${registrations?.length || 0}`);
          return { error: 'Insufficient participants' };
        }

        const playerIds = registrations.map(r => r.user_id);
        
        const { data, error } = await supabase.rpc('initialize_sabo_tournament', {
          p_tournament_id: tournamentId,
        ...
        const result = await TournamentAtomicOperations.generateBracket(
          tournamentId,
          user.id,
          tournament.tournament_type as 'single_elimination' | 'double_elimination',
          {
  ...
  const reseedTournament = useCallback(async (
    ...
    try {
      ...
      const result = await generateBracket(tournamentId, { method: seedingMethod, forceRegenerate: true });
