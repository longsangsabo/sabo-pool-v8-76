...
export const useSABOAdvancement = () => {
  ...
  const processLosersR101Completion = useCallback(async (tournamentId: string): Promise<SABOAdvancementResult> => {
    setIsProcessing(true);
    try {
      console.log('🔄 Processing Losers R101 completion for tournament:', tournamentId);
  ...
  const processLosersR102Completion = useCallback(async (tournamentId: string): Promise<SABOAdvancementResult> => {
    setIsProcessing(true);
    try {
      console.log('🔄 Processing Losers R102 completion for tournament:', tournamentId);
  ...
  const processLosersR103Completion = useCallback(async (tournamentId: string): Promise<SABOAdvancementResult> => {
    setIsProcessing(true);
    try {
      console.log('🔄 Processing Losers R103 completion for tournament:', tournamentId);
  ...
  const processLosersR201Completion = useCallback(async (tournamentId: string): Promise<SABOAdvancementResult> => {
    setIsProcessing(true);
    try {
      console.log('🔄 Processing Losers R201 completion for tournament:', tournamentId);
  ...
  const processLosersR202Completion = useCallback(async (tournamentId: string): Promise<SABOAdvancementResult> => {
    setIsProcessing(true);
    try {
      console.log('🔄 Processing Losers R202 completion for tournament:', tournamentId);
  ...
  const setupSemifinalsPairings = useCallback(async (tournamentId: string): Promise<SABOAdvancementResult> => {
    setIsProcessing(true);
    try {
      console.log('🔄 Setting up semifinals pairings for tournament:', tournamentId);
  ...
  const processSemifinalsCompletion = useCallback(async (tournamentId: string): Promise<SABOAdvancementResult> => {
    setIsProcessing(true);
    try {
      console.log('🔄 Processing semifinals completion for tournament:', tournamentId);
  ...
  const finalizeTournament = useCallback(async (tournamentId: string): Promise<SABOAdvancementResult> => {
    setIsProcessing(true);
    try {
      ...
      if (result?.success) {
        ...
        console.log('ℹ️ Tournament finalization not needed:', result?.message);
        return (result || { success: false }) as SABOAdvancementResult;
      }
    } catch (err: any) {
  ...
  const updateTournamentStatus = useCallback(async (tournamentId: string): Promise<SABOAdvancementResult> => {
    setIsProcessing(true);
    try {
      ...
      if (result?.success) {
        ...
        console.log('ℹ️ Tournament status update not needed:', result?.message);
        return (result || { success: false }) as SABOAdvancementResult;
      }
    } catch (err: any) {
  ...
  const processComprehensiveAdvancement = useCallback(async (tournamentId: string) => {
    if (!tournamentId) return;

    setIsProcessing(true);
    try {
      console.log('🤖 Processing comprehensive SABO advancement for tournament:', tournamentId);
      ...
      const semifinalsSetupResult = await setupSemifinalsPairings(tournamentId);
      
      // Process semifinals completion if ready
      const semifinalsCompletionResult = await processSemifinalsCompletion(tournamentId);
