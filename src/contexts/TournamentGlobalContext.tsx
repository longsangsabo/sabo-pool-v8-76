...
import { TournamentType, GameFormat, TournamentStatus } from '@/types/tournament-enums';
...
const TournamentGlobalContext = createContext<TournamentGlobalContextType | undefined>(undefined);

export const useTournamentGlobal = () => {
  const context = useContext(TournamentGlobalContext);
  if (!context) {
    throw new Error('useTournamentGlobal must be used within TournamentGlobalProvider');
  }
  return context;
};
...
export const TournamentGlobalProvider: React.FC<TournamentGlobalProviderProps> = ({ 
  children, 
  clubId 
}) => {
  const { user } = useAuth();
  ...
  const fetchTournaments = useCallback(async () => {
    try {
      ...
      console.log('🔍 [TournamentGlobal] Fetching tournaments for clubId:', clubId);
      ...
      console.log('📊 [TournamentGlobal] Raw tournaments data:', data?.length || 0);
      console.log('📊 [TournamentGlobal] Raw tournaments:', data?.map(t => ({ id: t.id, name: t.name, club_id: t.club_id })));

      // Transform to EnhancedTournament format
      const enhancedTournaments: EnhancedTournament[] = (data || []).map(tournament => ({
        ...tournament,
        tournament_type: tournament.tournament_type as TournamentType,
        game_format: tournament.game_format as GameFormat,
        status: tournament.status as TournamentStatus,
        contact_info: typeof tournament.contact_info === 'string' ? tournament.contact_info : JSON.stringify(tournament.contact_info || ''),
        min_rank_requirement: tournament.min_rank_requirement as "E" | "F" | "G" | "H" | "I" | "K" | "K+" | "I+" | "H+" | "G+" | "F+" | "E+" | null,
        max_rank_requirement: tournament.max_rank_requirement as "E" | "F" | "G" | "H" | "I" | "K" | "K+" | "I+" | "H+" | "G+" | "F+" | "E+" | null,
        tournament_start: tournament.start_date,
        tournament_end: tournament.end_date,
        club: tournament.club && typeof tournament.club === 'object' && !(tournament.club as any)?.error ? {
        ...
        rewards: (() => {
          ...
          console.log('🔄 [TournamentGlobal] Using default rewards:', defaultRewards);
        ...
        is_public: true,
        available_slots: Math.max(0, (tournament.max_participants || 0) - (tournament.current_participants || 0)),
        registration_status: calculateRegistrationStatus(tournament),
        time_until_start: tournament.start_date ? 
          calculateTimeUntilStart(tournament.start_date) : undefined
      }));
  ...
  const refreshTournaments = useCallback(async () => {
    try {
      ...
      console.log('✅ [TournamentGlobal] refreshTournaments completed successfully');
    } catch (error) {
      console.error('❌ [TournamentGlobal] Error in refreshTournaments:', error);
      setError(error instanceof Error ? error.message : 'Failed to refresh tournaments');
    } finally {
      setLoading(false);
    }
  }, [fetchTournaments]);
  ...
  // Helper functions
  const calculateRegistrationStatus = (tournament: any): 'not_started' | 'open' | 'closed' | 'ended' => {
    const now = new Date();
    ...
    const tournamentEnd = tournament.tournament_end ? new Date(tournament.tournament_end) : null;
  ...
  console.log('🔍 [TournamentGlobal] Debug filtering:', {
    ...
    tournamentsWithClubIds: tournaments.map(t => ({ id: t.id, name: t.name, club_id: t.club_id }))
  });
