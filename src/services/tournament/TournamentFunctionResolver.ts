...
export type TournamentFunction = typeof TOURNAMENT_FUNCTIONS[keyof typeof TOURNAMENT_FUNCTIONS];
 ...
 */
export async function validateFunction(functionName: TournamentFunction): Promise<boolean> {
  ...
  try {
    ...
    if (error.message?.includes('function') && error.message?.includes('does not exist')) {
      ...
      for (const fallback of fallbacks) {
        try {
          const { data: fallbackData, error: fallbackError } = await supabase.rpc(fallback as any, params);
          
          if (!fallbackError) {
            console.info(`Successfully used fallback function: ${fallback}`);
            return { data: fallbackData as T, error: null, usedFunction: fallback };
          }
        } catch (fallbackErr) {
...
export async function preloadTournamentFunctions(): Promise<{
  ...
  console.info(`Tournament Functions Status: ${available.length}/${allFunctions.length} available`);
  if (missing.length > 0) {
