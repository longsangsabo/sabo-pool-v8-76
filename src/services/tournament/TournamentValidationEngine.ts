...
export class TournamentValidationEngine {
   ...
   */
  static async validateTournament(tournamentId: string): Promise<TournamentValidation> {
    try {
      ...
      if (error || !tournament) {
        return {
          ...
          timeValidation: { valid: false, errors: ['Tournament not found'], warnings: [], suggestions: [] },
          structureValidation: { valid: false, errors: ['Tournament not found'], warnings: [], suggestions: [] }
        };
      }
      
      // Run all validations
      const participantValidation = await this.validateParticipants(tournamentId, tournament);
      ...
      return {
        ...
        tournamentData: null,
        participantValidation: this.createEmptyParticipantValidation(),
        timeValidation: { valid: false, errors: ['Validation failed'], warnings: [], suggestions: [] },
        structureValidation: { valid: false, errors: ['Validation failed'], warnings: [], suggestions: [] }
      };
    }
  }
   ...
   */
  static async validateParticipants(tournamentId: string, tournament?: any): Promise<ParticipantValidation> {
    try {
      if (!tournament) {
      ...
      if (tournamentType === 'double_elimination') {
        ...
        if (participantCount === 16) {
          ...
          warnings.push('Large double elimination tournaments can be very long');
          suggestions.push('Consider single elimination for faster completion');
        } else if (!this.isPowerOfTwo(participantCount)) {
          warnings.push(`${byesRequired} bye(s) will be added to reach ${optimalSize} participants`);
        }
      ...
      if (participantCount > tournament.max_participants) {
        errors.push(`Participant count (${participantCount}) exceeds maximum (${tournament.max_participants})`);
      }
      
      // Even distribution suggestions
      if (tournamentType !== 'round_robin' && participantCount >= 8) {
        const perfectSizes = [8, 16, 32, 64];
        const nearestPerfect = perfectSizes.find(size => size >= participantCount);
        
        if (nearestPerfect && nearestPerfect !== participantCount) {
          suggestions.push(`Consider ${nearestPerfect} participants for perfect bracket balance`);
        }
      }
      ...
      return {
        ...
        metadata: {
          ...
          estimatedDuration: this.estimateTournamentDuration(participantCount, tournamentType)
        }
      };
  ...
  static validateTiming(tournament: any): ValidationResult {
    ...
    try {
      ...
      if (regDurationDays < 1) {
        ...
        suggestions.push('Consider extending registration for better participation');
      }
      
      const tournamentDuration = tournamentEnd.getTime() - tournamentStart.getTime();
      ...
      if (tournament.status === 'ongoing' && now > tournamentEnd) {
        warnings.push('Tournament end time has passed but status is still ongoing');
      ...
      return {
        ...
        metadata: {
          ...
          timeUntilStart: Math.max(0, tournamentStart.getTime() - now.getTime()) / (1000 * 60 * 60),
          timeUntilEnd: Math.max(0, tournamentEnd.getTime() - now.getTime()) / (1000 * 60 * 60)
        }
      };
  ...
  static validateStructure(tournament: any): ValidationResult {
    ...
    try {
      ...
      if (tournament.prize_pool > expectedPool * 1.1) {
        warnings.push('Prize pool is significantly higher than expected entry fee collection');
      }
      
      // Format specific validations
      const validTournamentTypes = ['single_elimination', 'double_elimination', 'round_robin'];
      ...
      if (tournament.has_third_place_match && tournament.tournament_type === 'round_robin') {
        warnings.push('Third place match not applicable for round robin tournaments');
      }
  ...
  private static estimateTournamentDuration(participantCount: number, type: string): number {
  ...
  private static calculateStructureComplexity(tournament: any): 'simple' | 'moderate' | 'complex' {
    let complexity = 0;
