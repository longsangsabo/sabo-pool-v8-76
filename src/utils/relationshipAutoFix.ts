...
import { autoFixRelationships, buildSelectWithRelationships } from './relationshipMapper';
...
function applySmartFixes(content: string): string {
  ...
  const challengeSelectPattern = /\.select\(\s*`[\s\S]*?challenger_profile:profiles![^(]*\([\s\S]*?\)[\s\S]*?challenged_profile:profiles![^(]*\([\s\S]*?\)[\s\S]*?club:[^(]*\([\s\S]*?\)/g;
 ...
 */
export function autoFixProject(files: Array<{ path: string; content: string }>): AutoFixReport {
 ...
 */
export function previewFixes(files: Array<{ path: string; content: string }>): string {
 ...
 */
export function applyFixes(files: Array<{ path: string; content: string }>): Promise<AutoFixReport> {
