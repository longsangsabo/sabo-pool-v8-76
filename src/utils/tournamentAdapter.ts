...
export class TournamentAdapter {
  ...
  static toEnhanced(tournament: Tournament): EnhancedTournament {
    return {
      ...
      time_until_start: tournament.tournament_start ? 
        this.calculateTimeUntilStart(tournament.tournament_start) : undefined
   ...
   */
  static createMockEnhanced(overrides: Partial<EnhancedTournament> = {}): EnhancedTournament {
  ...
  private static calculateRegistrationStatus(tournament: Tournament): 'not_started' | 'open' | 'closed' | 'ended' {
    const now = new Date();
