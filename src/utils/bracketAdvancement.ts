...
export const advanceWinner = async (matchId: string): Promise<AdvancementResult> => {
  try {
...
export const fixAllTournamentProgression = async (tournamentId: string): Promise<AdvancementResult> => {
  try {
    console.log('🔧 Starting comprehensive tournament fix for:', tournamentId);
    
    const { data, error } = await supabase.rpc('fix_all_tournament_progression', {
...
export const autoFixTournamentProgression = async (tournamentId: string): Promise<void> => {
  try {
    ...
    for (let i = 0; i < rounds.length - 1; i++) {
      ...
      const completedInCurrent = currentRound.filter(m => m.winner_id && m.status === 'completed');
      const emptyInNext = nextRound.filter(m => !m.player1_id || !m.player2_id);
      
      if (completedInCurrent.length > 0 && emptyInNext.length > 0) {
        needsFix = true;
        console.log(`🔧 Found progression issue: Round ${rounds[i]} -> ${rounds[i + 1]}`);
        break;
      }
    }
