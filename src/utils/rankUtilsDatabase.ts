...
export async function getAllRankCodes(): Promise<RankCode[]> {
  return ['K', 'K+', 'I', 'I+', 'H', 'H+', 'G', 'G+', 'F', 'F+', 'E', 'E+'] as RankCode[];
}

export async function getNextRank(currentRank: RankCode): Promise<RankCode | null> {
  const ranks = await getAllRankCodes();
  const currentIndex = ranks.indexOf(currentRank);
  return currentIndex < ranks.length - 1 ? ranks[currentIndex + 1] : null;
}

export async function getPreviousRank(currentRank: RankCode): Promise<RankCode | null> {
  const ranks = await getAllRankCodes();
  const currentIndex = ranks.indexOf(currentRank);
...
export async function isEligibleForPromotion(currentElo: number, currentRank: RankCode): Promise<boolean> {
  const nextRank = await getNextRank(currentRank);
  return nextRank !== null;
}

export async function getTournamentRewardPoints(rank: RankCode, position: number): Promise<number> {
...
export const calculateNewElo = (playerElo: number, opponentElo: number, result: number): number => {
...
export const calculateEloChange = (playerElo: number, opponentElo: number, result: number): number => {
  const expectedResult = 1 / (1 + Math.pow(10, (opponentElo - playerElo) / 400));
  const kFactor = 32;
