...
const AdminClubRegistrations = () => {
      ...
      .on('postgres_changes', {
        ...
        if (payload.eventType === 'INSERT') {
          ...
          if (payload.old?.status !== payload.new?.status) {
            const statusText = payload.new?.status === 'active' ? 'đã duyệt' : 
  ...
  const fetchRegistrations = async () => {
    ...
    try {
      ...
      if (statusFilter !== 'all') {
        ...
        const mappedFilter = statusFilter === 'approved' ? 'active' : statusFilter;
      ...
      if (clubs && clubs.length > 0) {
        const userIds = [...new Set(clubs.map(c => c.owner_id).filter(Boolean))];
  ...
  const approveRegistration = async (registration: ClubRegistration) => {
    ...
    try {
      ...
      const { data: clubCreationResult, error: clubCreationError } = await supabase
        .rpc('create_club_zero_data', {
      ...
      if (clubCreationError) {
        ...
        throw new Error(`Lỗi tạo dữ liệu câu lạc bộ: ${clubCreationError.message}`);
      }
      ...
      toast.success(`Đã duyệt thành công câu lạc bộ "${registration.name}"! Chủ CLB đã được thông báo.`);
  ...
  const rejectRegistration = async (registration: ClubRegistration) => {
    ...
    console.log('❌ Rejecting club:', registration.name, 'Reason:', rejectionReason);
    setProcessing(true);
    ...
    try {
      ...
      if (notificationError) {
        console.warn('⚠️ Could not send rejection notification:', notificationError);
      }

      console.log('❌ Club rejected successfully:', registration.name);
      toast.success(`Đã từ chối đăng ký câu lạc bộ "${registration.name}" và gửi thông báo cho chủ CLB`);
      setRejectionReason('');
      await fetchRegistrations();
      setSelectedRegistration(null);
    } catch (error: any) {
        ...
        <div className="flex gap-3 items-center">
          ...
          <Select value={statusFilter} onValueChange={setStatusFilter}>
            ...
            <SelectContent>
              ...
              <SelectItem value="approved">Đã duyệt ({registrations.filter(r => r.status === 'active').length})</SelectItem>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                      <div className="space-y-2">
                        ...
                        <div className="flex items-center gap-2 text-sm">
                          ...
                          <span>{registration.contact_info || 'Chưa có số điện thoại'}</span>
                        </div>
                      ...
                      <div className="space-y-2">
                        ...
                        <div className="text-sm">
                          <span className="font-medium">Người đăng ký:</span> {registration.profiles?.display_name || registration.profiles?.full_name || 'Chưa có thông tin'}
                        </div>
                        {registration.profiles?.phone && (
                          <div className="text-sm">
                            <span className="font-medium">SĐT:</span> {registration.profiles.phone}
                          </div>
                        )}
                      </div>
                    </div>
                              ...
                              <div>
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                  ...
                                  <div>
                                    ...
                                    <p className="text-sm">{selectedRegistration.contact_info || 'Chưa có'}</p>
                                  </div>
                                  <div className="md:col-span-2">
                                    <label className="text-sm font-medium text-gray-700">Địa chỉ</label>
                                    <p className="text-sm">{selectedRegistration.address || 'Chưa có'}</p>
                                  </div>
                          ...
                          <Dialog>
                            ...
                            <DialogContent>
                              ...
                              <div className="space-y-4">
                                <p>Bạn có chắc chắn muốn từ chối đăng ký câu lạc bộ "{registration.name}"?</p>
                                <div>
                                  <label className="block text-sm font-medium text-gray-700 mb-2">
                                    Lý do từ chối *
                                  </label>
