...
import { Target, Clock, CheckCircle, AlertCircle, Trophy, User, Users, Building2 } from 'lucide-react';
...
const ThreeStepScoreWorkflow: React.FC<ThreeStepScoreWorkflowProps> = ({
  ...
  const isOtherPlayer = isParticipant && challenge.score_entered_by && user?.id !== challenge.score_entered_by;
  ...
  const handleStep3_ClubConfirm = async () => {
    ...
    setIsSubmitting(true);
    try {
      const finalChallengerScore = editedChallengerScore || challenge.challenger_final_score || 0;
      const finalOpponentScore = editedOpponentScore || challenge.opponent_final_score || 0;
      const winnerId = finalChallengerScore > finalOpponentScore 
      ...
      toast.success('Đã xác nhận kết quả thành công! ELO/SPA đã được cập nhật.');
      ...
      if (typeof window !== 'undefined' && (window as any).queryClient) {
        await (window as any).queryClient.invalidateQueries(['club-challenges']);
      }
  ...
  const sendScoreConfirmationEmail = async (actionType: 'score_entered' | 'score_confirmed' | 'club_final_confirmation') => {
    try {
      ...
      const { data: clubProfile } = challenge.club_id ? await supabase
        .from('club_profiles')
        .select('club_name')
        .eq('id', challenge.club_id)
        .single() : { data: null };
      ...
      if (!recipientEmail) {
        console.log('No recipient email found for score confirmation notification');
        return;
      }
  ...
  const canInteract = workflowState.canEnterScore || workflowState.canConfirmScore || workflowState.canClubConfirm;
        ...
        <div className="space-y-4">
              ...
              <div className="flex items-center justify-between text-sm">
                <div className="flex items-center gap-2">
                  <div className="flex items-center gap-1">
                    ...
                    <Badge variant={workflowState.step >= 2 ? "default" : workflowState.step === 1 ? "destructive" : "secondary"}>1</Badge>
                  </div>
                  <div className="flex items-center gap-1">
                    <Users className="w-3 h-3" />
                    <Badge variant={workflowState.step >= 3 ? "default" : workflowState.step === 2 ? "destructive" : "secondary"}>2</Badge>
                  </div>
                  <div className="flex items-center gap-1">
                    <Building2 className="w-3 h-3" />
                    <Badge variant={workflowState.step >= 4 ? "default" : workflowState.step === 3 ? "destructive" : "secondary"}>3</Badge>
                  </div>
                </div>
          ...
          {challenge.score_entered_by && (
            <Alert>
              ...
              <AlertDescription>
                Tỷ số đã được nhập: {challenge.challenger_final_score}-{challenge.opponent_final_score}
          ...
          <div className="grid grid-cols-2 gap-4">
            {/* Challenger Card */}
            <Card className={`transition-all duration-200 ${isChallenger ? 'ring-2 ring-primary/50 bg-primary/5' : ''}`}>
              <CardHeader className="pb-3">
                <div className="flex items-center gap-3">
                  <Avatar className="w-12 h-12 border-2 border-background shadow-sm">
                    <AvatarImage 
                      src={workflowState.canClubConfirm && challengerProfile?.avatar_url ? challengerProfile.avatar_url : challenge.challenger?.avatar_url} 
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-1 text-xs text-muted-foreground">
                      <span className="px-1.5 py-0.5 bg-amber-100 text-amber-800 rounded font-medium">
                        {workflowState.canClubConfirm && challengerProfile?.verified_rank 
              ...
              <CardContent className="pt-0">
                <Label htmlFor="challenger-score" className="text-xs font-medium">Tỷ số</Label>
                <Input
                  id="challenger-score"
                  type="number"
                  min="0"
                  max={challenge.race_to || 22}
                  value={workflowState.canClubConfirm ? editedChallengerScore : challengerScore}
                  onChange={(e) => {
                  ...
                  disabled={!workflowState.canEnterScore && !(workflowState.canConfirmScore && !workflowState.canEnterScore) && !workflowState.canClubConfirm}
            ...
            {/* Opponent Card */}
            <Card className={`transition-all duration-200 ${isOpponent ? 'ring-2 ring-primary/50 bg-primary/5' : ''}`}>
              <CardHeader className="pb-3">
                <div className="flex items-center gap-3">
                  <Avatar className="w-12 h-12 border-2 border-background shadow-sm">
                    <AvatarImage 
                      src={workflowState.canClubConfirm && opponentProfile?.avatar_url ? opponentProfile.avatar_url : challenge.opponent?.avatar_url}
                      className="object-cover"
                    />
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-1 text-xs text-muted-foreground">
                      <span className="px-1.5 py-0.5 bg-emerald-100 text-emerald-800 rounded font-medium">
                        {workflowState.canClubConfirm && opponentProfile?.verified_rank 
              ...
              <CardContent className="pt-0">
                <Label htmlFor="opponent-score" className="text-xs font-medium">Tỷ số</Label>
                <Input
                  id="opponent-score"
                  type="number"
                  min="0"
                  max={challenge.race_to || 22}
                  value={workflowState.canClubConfirm ? editedOpponentScore : opponentScore}
                  onChange={(e) => {
                    const value = parseInt(e.target.value) || 0;
                    if (workflowState.canClubConfirm) {
          ...
          {workflowState.canClubConfirm && challengerProfile && opponentProfile && (
              ...
              <CardContent className="space-y-4">
                ...
                <div className="space-y-2">
                  <div className="flex items-center justify-between">
                    <span className="font-medium">{challengerProfile.full_name}:</span>
                    <StarRating 
                ...
                <div className="space-y-2">
                  <div className="flex items-center justify-between">
                    <span className="font-medium">{opponentProfile.full_name}:</span>
                    <StarRating 
