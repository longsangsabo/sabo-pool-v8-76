...
import { 
  Trophy, Clock, CheckCircle, MapPin, Calendar, Target, 
  Filter, Download, Trash2, RefreshCw, AlertCircle,
...
const ClubChallengesTab: React.FC<ClubChallengesTabProps> = ({ clubId }) => {
  ...
  const fetchClubChallenges = async () => {
    try {
      ...
      const { data, error } = await supabase
        ...
        `)
        .or(`club_id.eq.${clubId},club_confirmed_by.eq.${clubProfile.user_id},location.ilike.%${clubProfile.club_name}%,score_confirmation_status.eq.score_confirmed`)
  ...
  const getStatusBadge = (challenge: Challenge) => {
    switch (challenge.score_confirmation_status) {
      case 'pending':
        return <Badge variant="secondary"><Clock className="w-3 h-3 mr-1" />Chờ nhập tỷ số</Badge>;
      case 'score_entered':
        return <Badge variant="destructive"><Target className="w-3 h-3 mr-1" />Chờ xác nhận</Badge>;
      case 'score_confirmed':
        return <Badge variant="destructive"><Trophy className="w-3 h-3 mr-1" />Chờ CLB xác nhận</Badge>;
      case 'completed':
        return <Badge variant="default"><CheckCircle className="w-3 h-3 mr-1" />Đã hoàn thành</Badge>;
      default:
  ...
  const getScoreDisplay = (challenge: Challenge) => {
    ...
    if (challenge.challenger_final_score !== undefined && 
        challenge.challenger_final_score !== null &&
        challenge.opponent_final_score !== undefined && 
        challenge.opponent_final_score !== null) {
      return `${challenge.challenger_final_score} - ${challenge.opponent_final_score}`;
  ...
  const filteredChallenges = useMemo(() => {
    switch (filter) {
      ...
      case 'completed':
        return challenges.filter(c => c.score_confirmation_status === 'completed');
      case 'confirmed':
  ...
  const completedChallenges = challenges.filter(c => c.score_confirmation_status === 'completed');
  ...
  const stats = {
    ...
    today: challenges.filter(c => {
      ...
      const challengeDate = new Date(c.scheduled_time || c.created_at).toDateString();
      return challengeDate === today;
    }).length
  };

  const StatCard = ({ label, value, icon, color = 'default' }: { 
    label: string; 
    value: string | number; 
    icon: string; 
  ...
  const renderChallengeCard = (challenge: Challenge, isPriority = false) => (
    <div 
      ...
      className={`p-4 border rounded-lg transition-all duration-200 hover:shadow-md ${
        isPriority ? 'border-orange-200 bg-orange-50' : 'border-gray-200 bg-white'
      }`}
    >
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <StatCard label="Chờ xử lý" value={stats.pending} icon="⏳" color="orange" />
          ...
          <div className="flex flex-col gap-4">
            ...
            <div className="flex justify-between items-center pt-2 border-t">
              ...
              <div className="flex gap-2">
                ...
                <Button variant="outline" size="sm" onClick={fetchClubChallenges}>
        ...
        {filter === 'all' && pendingChallenges.length > 0 && (
          <Card>
            ...
            <CardContent className="space-y-4">
              {pendingChallenges.map(challenge => renderChallengeCard(challenge, true))}
            </CardContent>
          </Card>
        )}
