...
const AdminTournamentManager: React.FC = () => {
  ...
  const loadUsers = async () => {
    ...
    try {
      const { data, error } = await supabase
        .from('profiles')
        .select('id, user_id, full_name, email, phone, elo, skill_level, is_demo_user, created_at')
        .order('full_name');
    ...
    switch (template.filter) {
      case 'advanced':
        filteredUsers = users.filter(user => 
          (user.elo || 0) >= 1600 && !user.is_demo_user
        );
        break;
      
      case 'mixed':
        const advanced = users.filter(u => 
          (u.elo || 0) >= 1600 && !u.is_demo_user
        ).slice(0, 4);
        const intermediate = users.filter(u => 
          (u.elo || 0) >= 1200 && (u.elo || 0) < 1600 && !u.is_demo_user
        ).slice(0, 8);
        const beginner = users.filter(u => 
          (u.elo || 0) < 1200 && !u.is_demo_user
        ).slice(0, 4);
      ...
      case 'fill':
        if (selectedTournament) {
          const remaining = selectedTournament.max_participants - selectedTournament.current_participants;
          filteredUsers = users.filter(u => !u.is_demo_user).slice(0, remaining);
        }
        break;
    }

    const count = template.count === -1 ? filteredUsers.length : Math.min(template.count, filteredUsers.length);
    const selectedIds = filteredUsers.slice(0, count).map(u => u.user_id || u.id);
  ...
  const addUsersToTournament = async () => {
    ...
    try {
      // Check capacity
      const totalAfterAdd = selectedTournament.current_participants + selectedUsers.length;
      ...
      const newUserIds = selectedUsers.filter(id => !existingUserIds.includes(id));

      if (newUserIds.length === 0) {
        toast({ title: 'Thông báo', description: 'Tất cả người dùng đã tham gia giải đấu' });
        return;
      }
      ...
      const updatedParticipantCount = selectedTournament.current_participants + newUserIds.length;
      await supabase
        .from('tournaments')
        .update({ 
  ...
  const autoBalanceSkills = () => {
    ...
    const selectedUserData = users.filter(u => selectedUsers.includes(u.user_id || u.id));
    const sortedByElo = selectedUserData.sort((a, b) => (b.elo || 0) - (a.elo || 0));
  ...
  const getSelectionStats = () => {
    ...
    const selectedUserData = users.filter(u => selectedUsers.includes(u.user_id || u.id));
    const avgElo = selectedUserData.reduce((sum, u) => sum + (u.elo || 0), 0) / selectedUserData.length;
    const skillDistribution = {
      advanced: selectedUserData.filter(u => (u.elo || 0) >= 1600).length,
      intermediate: selectedUserData.filter(u => (u.elo || 0) >= 1200 && (u.elo || 0) < 1600).length,
              ...
              {selectedTournament ? (
                ...
                <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                  <div className="flex items-center justify-between">
                    <div>
                      <h4 className="font-semibold text-blue-900">{selectedTournament.name}</h4>
                      <div className="text-sm text-blue-700 mt-1">
                        👥 {selectedTournament.current_participants}/{selectedTournament.max_participants} người •
                        <Badge variant="secondary" className="ml-2">
                          {selectedTournament.status}
                        </Badge>
                      </div>
                      <div className="text-xs text-blue-600 mt-1">
                        💰 Phí: {selectedTournament.entry_fee?.toLocaleString('vi-VN') || 0} VNĐ •
                ...
                <div>
                  ...
                  <Select onValueChange={(tournamentId) => {
                  ...
                  {tournaments.filter(t => t.status === 'registration_open' || t.status === 'upcoming').length === 0 && (
            ...
            <CardContent>
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-4">
                ...
                <Button
                  variant="outline"
                  size="sm"
                  onClick={() => setSelectedUsers(users.map(u => u.user_id || u.id))}
                >
                  ✅ Chọn tất cả
                </Button>
              </div>
              ...
              <div className="space-y-3">
                <div className="flex gap-3">
                  <Button
                    ...
                    disabled={!selectedTournament || selectedUsers.length === 0 || loading}
                    className={`flex-1 ${
                      !selectedTournament ? 'bg-gray-400' : 
                      selectedUsers.length === 0 ? 'bg-gray-500' : 
                      'bg-blue-600 hover:bg-blue-700'
                    }`}
                  >
                    {loading ? 'Đang thêm...' : 
                     !selectedTournament ? '❌ Chưa chọn giải đấu' :
                     selectedUsers.length === 0 ? '❌ Chưa chọn người chơi' :
                ...
                {selectedTournament && (
                  <div className="mt-4 p-4 bg-green-50 border border-green-200 rounded-lg">
                    <h4 className="font-medium text-green-800 mb-3">🚀 Bắt đầu giải đấu</h4>
                    <TournamentActions
                      tournamentId={selectedTournament.id}
                      tournamentName={selectedTournament.name}
                      tournamentStatus={selectedTournament.status}
                      managementStatus={selectedTournament.management_status || 'manual'}
                      onTournamentStarted={() => {
                  ...
                  <div
                    key={user.id}
                     className={`p-3 border rounded-lg flex items-center justify-between transition-colors ${
                       selectedUsers.includes(user.user_id || user.id) ? 'border-primary bg-primary/5' : 'hover:bg-accent/50'
                     }`}
                   >
                     <div className="flex items-center space-x-3">
                      ...
                      <div>
                        ...
                        <div className="text-sm text-muted-foreground">
                          ELO: {user.elo || 'N/A'} • 
                          Skill: {user.skill_level || 'Unranked'} • 
                          📱 {user.phone}
                          {user.is_demo_user && <Badge variant="secondary" className="ml-2">Demo</Badge>}
                        </div>
                      </div>
                    </div>
                  </div>
