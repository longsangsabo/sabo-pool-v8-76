...
const EnhancedChallengesPageV2 = React.lazy(
...
const EnhancedOverview = () => {
  ...
  const calculateStats = () => {
    ...
    const newStats = {
      ...
      accepted: userChallenges.filter(
        c => c.status === 'accepted' || c.status === 'ongoing'
      ).length,
      completed: userChallenges.filter(c => c.status === 'completed').length,
      won: userChallenges.filter(
        c =>
          c.status === 'completed' &&
          ((c.challenger_id === user?.id &&
            (c as any).winner_id === user?.id) ||
            (c.opponent_id === user?.id && (c as any).winner_id === user?.id))
      ).length,
      lost: userChallenges.filter(
        c =>
          c.status === 'completed' &&
          ((c.challenger_id === user?.id &&
            (c as any).winner_id === c.opponent_id) ||
            (c.opponent_id === user?.id &&
              (c as any).winner_id === c.challenger_id))
      ).length,
      winRate: 0,
    };

    newStats.winRate =
      newStats.completed > 0 ? (newStats.won / newStats.completed) * 100 : 0;
    setStats(newStats);
  };
                  ...
                  <div>
                    <p className='font-medium'>
                      ...
                      <span className='text-blue-500'>
                        {activity.challenger_id === user?.id
                          ? activity.opponent_profile?.display_name ||
                            activity.opponent_profile?.full_name
                          : activity.challenger_profile?.display_name ||
                            activity.challenger_profile?.full_name}
                      </span>
                    </p>
...
const MyChallenges = () => {
  const { user } = useAuth();
  const { challenges, loading, acceptChallenge, declineChallenge } =
        ...
        <div className='flex items-center justify-between'>
          <div className='flex items-center space-x-2'>
            ...
            <h3 className='text-lg font-semibold'>
              Challenges của tôi ({myChallenges.length})
            </h3>
          </div>
        </div>
                  ...
                  <div className='flex-1'>
                    <div className='font-medium'>
                      ...
                      {challenge.challenger_id === user?.id
                        ? (challenge as any).opponent_profile?.display_name ||
                          'Unknown'
  ...
  const getFilteredChallenges = () => {
    return challenges
      .filter(challenge => {
        // Basic filters
        const isOpenToJoin =
          !challenge.opponent_id && challenge.challenger_id !== user?.id;
        const matchesSearch =
          !searchTerm ||
          [
        ...
        const matchesType =
          typeFilter === 'all' ||
          (challenge as any).challenge_type === typeFilter;
                  ...
                  <div className='flex-1'>
                    ...
                    <div className='text-sm text-muted-foreground grid grid-cols-2 md:grid-cols-4 gap-2'>
                      ...
                      <span className='flex items-center'>
                        <Trophy className='h-4 w-4 mr-1' />
                        ELO:{' '}
                        {(challenge as any).challenger_profile?.elo_rating ||
                          'N/A'}
                      </span>
                      <span className='flex items-center'>
          ...
          <div className='text-center py-8'>
            ...
            <p className='text-gray-500'>
              {searchTerm
                ...
                : 'Hãy tạo challenge đầu tiên của bạn'}
            </p>
          </div>
        )}
      </div>
        ...
        >
          <Card className='compact-card'>
            <div className='text-center mb-4'>
              ...
              <p className='compact-subtitle'>
                Truy cập đầy đủ tính năng challenge system với giao diện chuyên
                nghiệp
              </p>
            </div>
