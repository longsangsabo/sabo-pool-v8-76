...
import { Loader2, Trophy, Calendar, Medal, History, Search, Plus, Users, Target, Clock, Star, TrendingUp, Activity } from 'lucide-react';
...
const TournamentBracketPage = React.lazy(() => import('@/pages/TournamentBracketPage'));
...
const EnhancedTournamentOverview = () => {
  ...
  const fetchTournamentData = async () => {
    try {
      ...
      const activeTournaments = tournaments?.filter(t => t.status === 'active' || t.status === 'ongoing').length || 0;
      const totalParticipants = tournaments?.reduce((sum, t) => sum + (t.tournament_participants?.length || 0), 0) || 0;
      const totalPrize = tournaments?.reduce((sum, t) => sum + (t.prize_pool || 0), 0) || 0;
      ...
      if (user) {
        ...
        const userTournaments = tournaments?.filter(t => t.organizer_id === user.id) || [];
        ...
        <CardContent className="p-0">
          {featuredTournaments.length > 0 ? (
                    ...
                    <div className="flex-1">
                      ...
                      <div className="text-sm text-muted-foreground flex items-center space-x-4">
                        ...
                        <span className="flex items-center">
                          ...
                          {(tournament.prize_pool || 0).toLocaleString()}K giải thưởng
                        </span>
                        <span className="flex items-center">
                    ...
                    <div className="flex items-center space-x-2">
                      <Badge variant={
                        ...
                        {tournament.status === 'active' ? 'Đang diễn ra' :
                         tournament.status === 'upcoming' ? 'Sắp bắt đầu' :
                         tournament.status}
                      </Badge>
            ...
            <div className="text-center py-8">
              ...
              <p className="text-muted-foreground">Chưa có tournament nổi bật</p>
            </div>
          )}
        </CardContent>
      </Card>
...
const EnhancedMatchHistory = () => {
    ...
    try {
      ...
      const mockMatches = [
        {
          ...
          player1: { full_name: user?.user_metadata?.full_name || 'You', current_rank: 'A2' },
        ...
        },
        {
          ...
          player2: { full_name: user?.user_metadata?.full_name || 'You', current_rank: 'A2' }
        }
      ];
      ...
      if (filters.opponent) {
        filteredMatches = filteredMatches.filter(match => {
          ...
          return opponentName?.toLowerCase().includes(filters.opponent.toLowerCase());
        });
      }
  ...
  const getMatchResult = (match: any) => {
    const isWin = match.winner_id === user?.id;
    const userScore = match.player1_id === user?.id ? match.score_player1 : match.score_player2;
    const opponentScore = match.player1_id === user?.id ? match.score_player2 : match.score_player1;
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <Card className="p-4">
          <div className="text-center">
            <div className="text-2xl font-bold text-blue-500">{stats.totalMatches}</div>
        ...
        <Card className="p-4">
          <div className="text-center">
            <div className="text-2xl font-bold text-purple-500">{stats.winRate.toFixed(1)}%</div>
      ...
      <Card className="p-4">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          ...
          <div>
            ...
            <Select value={filters.tournament} onValueChange={(value) => setFilters({...filters, tournament: value})}>
              <SelectTrigger>
                <SelectValue />
              </SelectTrigger>
              <SelectContent>
      ...
      <Card className="p-6">
          ...
          {matches.length > 0 ? (
            <div className="space-y-3">
              {matches.map((match) => {
                ...
                return (
                  <Card key={match.id} className={`p-4 border-l-4 ${result.isWin ? 'border-l-green-500' : 'border-l-red-500'}`}>
                    <div className="flex items-center justify-between">
                      ...
                      <div className="text-right">
                        <div className={`text-lg font-bold ${result.isWin ? 'text-green-500' : 'text-red-500'}`}>
                          {result.scoreLine}
                        </div>
                        <div className="text-sm text-muted-foreground">
        ...
        <div className="space-y-2">
          <Card className="p-3">
            <div className="flex justify-between items-center">
              <div>
                <div className="font-medium">Spring Championship 2024</div>
                <div className="text-sm text-muted-foreground">64 người chơi • 500K giải thưởng</div>
              </div>
              <div className="text-sm text-green-500 font-medium">Đang diễn ra</div>
            </div>
          </Card>
          <Card className="p-3">
            <div className="flex justify-between items-center">
              <div>
                <div className="font-medium">Weekend Cup</div>
                <div className="text-sm text-muted-foreground">32 người chơi • 200K giải thưởng</div>
              </div>
              <div className="text-sm text-orange-500 font-medium">Sắp bắt đầu</div>
            </div>
          </Card>
        </div>
      </div>
    ...
    <div className="compact-container compact-layout desktop-high-density">
      ...
      <Tabs defaultValue="overview" className="w-full">
        <TabsList className="grid w-full grid-cols-6 h-9 md:h-10">
          ...
          <TabsTrigger value="tournaments" className="compact-nav-item flex items-center space-x-1">
            <Calendar className="h-3 w-3 md:h-4 md:w-4" />
            <span className="hidden sm:inline responsive-text-xs">Danh sách</span>
          </TabsTrigger>
          <TabsTrigger value="brackets" className="compact-nav-item flex items-center space-x-1">
            <Medal className="h-3 w-3 md:h-4 md:w-4" />
            <span className="hidden sm:inline responsive-text-xs">Brackets</span>
          </TabsTrigger>
          <TabsTrigger value="leaderboard" className="compact-nav-item flex items-center space-x-1">
            <Trophy className="h-3 w-3 md:h-4 md:w-4" />
            <span className="hidden sm:inline responsive-text-xs">Xếp hạng</span>
          </TabsTrigger>
          <TabsTrigger value="history" className="compact-nav-item flex items-center space-x-1">
            <History className="h-3 w-3 md:h-4 md:w-4" />
            <span className="hidden sm:inline responsive-text-xs">Lịch sử</span>
          </TabsTrigger>
          <TabsTrigger value="match-history" className="compact-nav-item flex items-center space-x-1">
            <Activity className="h-3 w-3 md:h-4 md:w-4" />
            <span className="hidden sm:inline responsive-text-xs">Trận đấu</span>
          </TabsTrigger>
        </TabsList>
        ...
        <TabsContent value="tournaments" className="space-y-3 md:space-y-4 mobile-compact">
          <Suspense fallback={<TabLoadingSpinner />}>
            <TournamentListPage />
          </Suspense>
        </TabsContent>

        <TabsContent value="brackets" className="space-y-3 md:space-y-4 mobile-compact">
          <Suspense fallback={<TabLoadingSpinner />}>
            <TournamentBracketPage />
          </Suspense>
        </TabsContent>

        <TabsContent value="leaderboard" className="space-y-3 md:space-y-4 mobile-compact">
          <Suspense fallback={<TabLoadingSpinner />}>
            <LeaderboardPage />
          </Suspense>
        </TabsContent>

        <TabsContent value="history" className="space-y-3 md:space-y-4 mobile-compact">
          <Suspense fallback={<TabLoadingSpinner />}>
            <SeasonHistoryPage />
          </Suspense>
        </TabsContent>

        <TabsContent value="match-history" className="space-y-3 md:space-y-4 mobile-compact">
