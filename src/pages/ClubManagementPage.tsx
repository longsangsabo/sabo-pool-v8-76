    ...
    <ClubResponsiveLayout>
      <div className="mb-8">
        ...
        <p className="text-muted-foreground mt-2">
          Quản lý {clubProfile?.club_name || 'câu lạc bộ'}, xác thực hạng và theo dõi hoạt động
        </p>
      </div>
      ...
      <Tabs defaultValue={window.location.hash === '#tournaments' ? 'tournaments' : 'overview'} className="space-y-6">
        <TabsList className="grid w-full grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-1">
          ...
          <TabsTrigger value="challenges" className="flex items-center gap-2 px-3 py-2">
            <Swords className="w-4 h-4" />
            <span className="text-sm">Thách đấu</span>
          </TabsTrigger>
          <TabsTrigger value="verification" className="flex items-center gap-2 px-3 py-2">
        ...
        <TabsContent value="tournaments">
          {(() => {
            console.log('🎯 Rendering tournaments tab with clubId:', clubProfile?.id);
            return (
