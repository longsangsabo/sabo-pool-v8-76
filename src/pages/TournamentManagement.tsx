...
import { Loader2, Users, Trophy, Calendar, DollarSign, Settings } from "lucide-react";
...
export default function TournamentManagement() {
  ...
  const getStatusBadge = (status: string) => {
    const statusMap = {
      'registration_open': { label: 'Registration Open', variant: 'secondary' as const },
      'registration_closed': { label: 'Registration Closed', variant: 'outline' as const },
  ...
  const canStartTournament = () => {
    return hasBracket && tournament.status !== 'ongoing' && tournament.status !== 'completed';
  };
  ...
  const handleManualAdvance = async () => {
    try {
      ...
      const { data, error } = await supabase.functions.invoke('manual-tournament-advance', {
      ...
      if (data?.success) {
        toast.success(`Manual advance thành công! Đã cập nhật ${data.updated_matches} trận đấu`);
        refreshData();
      } else {
  ...
  return (
    <div className="container mx-auto p-4 space-y-6">
      ...
      {/* Tournament Actions */}
      {(tournament.tournament_type === 'single_elimination' || tournament.tournament_type === 'double_elimination') && (
        <Card>
          <CardHeader>
          ...
          <CardContent className="space-y-4">
            {!hasBracket && (
              <div>
                ...
                <TournamentBracketGenerator
                  ...
                  tournamentType={tournament?.tournament_type || 'single_elimination'}
            ...
            {tournament.status === 'ongoing' && tournament.tournament_type === 'double_elimination' && (
              <div className="space-y-2">
                <h3 className="font-semibold">Manual Advancement</h3>
                <p className="text-sm text-muted-foreground">
                  Nếu các trận đấu đã hoàn tất nhưng chưa tự động advance, hãy sử dụng nút này
                </p>
                <Button 
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-2">
            {registrations.map((registration, index) => (
              <div key={registration.id} className="flex items-center gap-2 p-2 rounded border">
                <Badge variant="outline">{index + 1}</Badge>
                <span className="text-sm">{(registration.profiles as any)?.full_name || 'Unknown Player'}</span>
              </div>
            ))}
          </div>
      ...
      {!hasBracket && (tournament.tournament_type === 'single_elimination' || tournament.tournament_type === 'double_elimination') && participantCount > 0 && (
        <Card>
          <CardContent className="text-center py-8">
