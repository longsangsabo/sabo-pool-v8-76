...
export const executePostBracketSaveLogic = async (tournamentId: string, bracketData: any) => {
  console.log("🚀 Executing post-bracket save workflow for tournament:", tournamentId);
...
const updateTournamentStatus = async (tournamentId: string, bracketData: any) => {
  ...
  if (error) throw new Error(`Failed to update tournament status: ${error.message}`);
  
  console.log("✅ Tournament status updated to ongoing");
};

// ===== 3. PROCESS TOURNAMENT SEEDING =====
const processTournamentSeeding = async (tournamentId: string, participants: any[]) => {
  ...
  for (const seed of seedingData) {
    ...
    if (error) console.warn(`Seeding warning for user ${seed.user_id}:`, error.message);
  }
...
const createMatchSchedule = async (tournamentId: string, matches: any[]) => {
  ...
  const scheduledMatches = matches.map((match, index) => {
    const startTime = new Date(baseTime.getTime() + (index * (matchDuration + breakTime)));
    ...
    return {
      ...
      status: match.player1?.isBye || match.player2?.isBye ? 'bye' : 'scheduled',
  ...
  if (error) throw new Error(`Failed to create match schedule: ${error.message}`);
...
const setupTournamentMonitoring = async (tournamentId: string) => {
  ...
  if (logError) console.warn("⚠️ Failed to log monitoring setup:", logError.message);
...
export const canStartTournament = async (tournamentId: string): Promise<boolean> => {
  const { data: tournament } = await supabase
    .from('tournaments')
