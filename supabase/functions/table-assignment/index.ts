...
serve(async (req) => {
  ...
  try {
    ...
    const { action, match_id, table_id, tournament_id, club_id }: AssignMatchRequest = await req.json()

    console.log('Table assignment request:', { action, match_id, table_id, tournament_id, club_id })
    ...
    switch (action) {
      ...
      case 'auto_assign_all':
        return await autoAssignAllMatches(supabaseClient, tournament_id!, club_id!)
...
async function assignMatchToTable(supabase: any, matchId: string, tableId: string) {
...
async function autoAssignAllMatches(supabase: any, tournamentId: string, clubId: string) {
  console.log('Auto-assigning all matches using database function:', { tournamentId, clubId })

  try {
    // Use the database function for consistent logic
    const { data: assignmentResult, error: assignmentError } = await supabase
      .rpc('auto_assign_tournament_tables', { 
    ...
    if (assignmentResult?.error) {
      console.error('❌ Assignment error from database:', assignmentResult.error);
      return new Response(
        JSON.stringify({
          success: false,
    ...
    return new Response(
      JSON.stringify({ 
        ...
        message: assignmentResult?.message || `Đã gán ${assignmentCount} trận đấu`,
