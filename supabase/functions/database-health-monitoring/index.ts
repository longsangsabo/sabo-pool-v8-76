...
const handler = async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    try {
      ...
      if (error) {
        healthChecks.errors_detected.push(`Database connection error: ${error.message}`);
    ...
    const tablesToCheck = [
      'profiles', 'tournaments', 'tournament_registrations', 
      'matches', 'match_results', 'challenges', 'notifications',
      'player_rankings', 'system_logs'
    ];
    ...
    for (const table of tablesToCheck) {
      try {
        ...
        if (error) {
          healthChecks.errors_detected.push(`Error checking table ${table}: ${error.message}`);
          ...
          if (table === 'profiles' && (count || 0) === 0) {
            healthChecks.warnings.push('No user profiles found - system may be empty');
            overallHealth = 'warning';
          }
          
          if (table === 'system_logs' && (count || 0) > 100000) {
            healthChecks.warnings.push('System logs table is getting large - consider archiving');
            healthChecks.recommendations.push('Archive old system logs older than 90 days');
          }
        }
      } catch (error) {
        healthChecks.errors_detected.push(`Failed to check table ${table}: ${error}`);
        overallHealth = 'warning';
      }
    }
    ...
    try {
      ...
      if (!error && recentErrors && recentErrors.length > 0) {
        healthChecks.warnings.push(`${recentErrors.length} errors detected in last 24 hours`);
    ...
    try {
      ...
      if (!error && unsentNotifications && unsentNotifications.length > 0) {
        healthChecks.warnings.push(`${unsentNotifications.length} pending notifications older than 1 hour`);
        if (unsentNotifications.length > 50) {
          overallHealth = 'warning';
          healthChecks.recommendations.push('Check notification delivery system');
        }
      }
    } catch (error) {
      healthChecks.warnings.push(`Could not check notification status: ${error}`);
    }
    ...
    try {
      ...
      if (!error && stuckTournaments && stuckTournaments.length > 0) {
        healthChecks.warnings.push(`${stuckTournaments.length} tournaments still marked as ongoing past end date`);
        healthChecks.recommendations.push('Review and close completed tournaments');
        overallHealth = 'warning';
      }
    } catch (error) {
    ...
    try {
      ...
      if (!error && orphanedRegistrations && orphanedRegistrations.length > 0) {
        healthChecks.warnings.push(`${orphanedRegistrations.length} orphaned tournament registrations found`);
        healthChecks.recommendations.push('Clean up orphaned registration records');
      }
    } catch (error) {
      healthChecks.warnings.push(`Could not check for orphaned records: ${error}`);
    }
    ...
    try {
      ...
      healthChecks.performance_metrics.query_response_time = Date.now() - performanceStart;
    ...
    if (healthChecks.table_counts.notifications > 10000) {
      healthChecks.recommendations.push('Consider implementing notification archiving');
    }
    
    if (healthChecks.table_counts.system_logs > 50000) {
      healthChecks.recommendations.push('Archive old system logs to improve performance');
    }
