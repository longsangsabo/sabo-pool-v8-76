...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { challengeId, challengerScore, opponentScore, playerId, addRackResult = false }: MatchUpdateRequest = 
      await req.json();

    console.log('SABO Match Update Request:', { challengeId, challengerScore, opponentScore, playerId });
    ...
    if (challengeError || !challenge) {
      ...
      return new Response(
        JSON.stringify({ success: false, error: 'Challenge not found or not active' }),
    ...
    if (challenge.challenger_id !== playerId && challenge.opponent_id !== playerId) {
      return new Response(
        JSON.stringify({ success: false, error: 'Player not involved in this challenge' }),
    ...
    const { data: updateResult, error: updateError } = await supabase
      .rpc('simulate_sabo_match_progress', {
