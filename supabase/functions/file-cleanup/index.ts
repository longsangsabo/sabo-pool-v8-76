...
serve(async (req) => {
  ...
  try {
    ...
    const { action = 'scan', dry_run = true, bucket_name = null } = await req.json();
    const startTime = Date.now();
    
    console.log(`File cleanup started: action=${action}, dry_run=${dry_run}, bucket=${bucket_name}`);
    ...
    const bucketsToCheck = bucket_name ? [bucket_name] : ['avatars', 'tournament-banners', 'club-photos', 'match-evidence'];
    ...
    for (const bucket of bucketsToCheck) {
      try {
        ...
        if (filesError) {
          ...
          stats.errors.push(`Failed to list files in bucket ${bucket}: ${filesError.message}`);
          continue;
        }
        ...
        const orphanedFiles = await identifyOrphanedFiles(supabase, bucket, files || []);
        stats.orphaned_files_found += orphanedFiles.length;

        console.log(`Found ${orphanedFiles.length} orphaned files in bucket ${bucket}`);

        // Delete orphaned files if not dry run
        if (!dry_run && orphanedFiles.length > 0) {
          const deletionResults = await deleteOrphanedFiles(supabase, bucket, orphanedFiles);
        ...
        stats.errors.push(`Error processing bucket ${bucket}: ${error.message}`);
      }
    }
...
async function identifyOrphanedFiles(supabase: any, bucket: string, files: any[]): Promise<OrphanedFile[]> {
...
async function isAvatarOrphaned(supabase: any, fileName: string): Promise<boolean> {
...
async function isTournamentBannerOrphaned(supabase: any, fileName: string): Promise<boolean> {
...
async function isClubPhotoOrphaned(supabase: any, fileName: string): Promise<boolean> {
...
async function isMatchEvidenceOrphaned(supabase: any, fileName: string): Promise<boolean> {
...
async function isGenericFileOrphaned(supabase: any, fileName: string): Promise<boolean> {
...
async function deleteOrphanedFiles(supabase: any, bucket: string, orphanedFiles: OrphanedFile[]) {
  let deleted_count = 0;
  let space_freed = 0;
  const errors: string[] = [];
  ...
  for (const file of orphanedFiles) {
    try {
      ...
      if (error) {
        ...
        console.log(`Deleted orphaned file: ${bucket}/${file.name} (${formatBytes(file.size)})`);
      }
    } catch (error) {
...
async function logCleanupActivity(supabase: any, bucket: string, orphanedFiles: OrphanedFile[], wasDeleted: boolean) {
  try {
    await supabase
...
async function storeCleanupStats(supabase: any, stats: CleanupStats, wasActualCleanup: boolean) {
  try {
    await supabase
      .from('system_logs')
      .insert({
        log_type: 'file_cleanup',
        message: wasActualCleanup ? 'File cleanup executed' : 'File cleanup scan performed',
        metadata: {
          stats: stats,
