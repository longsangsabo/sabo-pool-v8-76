...
const handler = async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    if (!regError && inactiveRegistrations) {
      for (const registration of inactiveRegistrations) {
        ...
        const { data: recentActivity, error: activityError } = await supabase
          .from('matches')
          .select('id')
          .or(`player1_id.eq.${registration.user_id},player2_id.eq.${registration.user_id}`)
        ...
        if (!activityError && (!recentActivity || recentActivity.length === 0)) {
          ...
          if (!deleteError) {
            ...
            console.log(`🗑️ Removed inactive registration for tournament: ${registration.tournaments?.name}`);
        ...
        if (!updateError) {
          ...
          await supabase
            ...
            .insert({
              ...
              message: 'Lời thách đấu của bạn đã hết hạn do không được phản hồi trong 7 ngày.',
              priority: 'low',
              metadata: {
    ...
    const { data: inactiveProfiles, error: profileError } = await supabase
      .rpc('get_inactive_players', { days_threshold: 30 });
