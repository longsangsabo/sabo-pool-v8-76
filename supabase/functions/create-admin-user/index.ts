    ...
    if (action === 'reset_database') {
      ...
      if (authUsers?.users) {
        for (const user of authUsers.users) {
          ...
          if (!user.email?.includes('supabase') && !user.email?.includes('admin')) {
            try {
    ...
    if (action === 'batch_create_demo_users') {
      ...
      for (let i = 0; i < demoUsers.length; i++) {
        ...
        try {
          ...
          console.log(`✅ Created demo user ${i + 1}/${count}: ${userData.email}`);
    ...
    if (batch_create && users) {
      ...
      for (let i = 0; i < users.length; i++) {
        ...
        try {
          ...
          console.log(`✅ Created user ${i + 1}/${users.length}: ${userData.email}`);
