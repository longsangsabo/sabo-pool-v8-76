    ...
    for (const config of configs || []) {
      ...
      try {
        ...
        if (cleanupResult.error) {
          throw new Error(`Cleanup failed for ${config.bucket_name}: ${cleanupResult.error.message}`);
        }
        ...
        console.log(`Completed ${config.bucket_name}: ${result.files_deleted} files deleted, ${formatBytes(result.space_freed)} freed`);
...
async function isCleanupDue(config: any): Promise<boolean> {
  ...
  console.log(`${config.bucket_name}: ${daysSinceLastCleanup.toFixed(1)} days since last cleanup, due: ${isDue}`);
...
async function logScheduledCleanupResults(supabase: any, results: ScheduledCleanupResult[], executionTime: number) {
  try {
    const totalFilesDeleted = results.reduce((sum, r) => sum + r.files_deleted, 0);
