...
const handler = async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    for (const player of eligiblePlayers || []) {
      console.log(`🎯 Checking player: ${player.user_id} with ${player.elo_points} ELO points and ${player.total_matches} matches`);
      ...
      const { data: promotionResult, error: promotionError } = await supabase
        .rpc('check_rank_promotion', { p_user_id: player.user_id });

      if (promotionError) {
        console.error(`❌ Error checking promotion for player ${player.user_id}:`, promotionError);
        continue;
      }

      if (promotionResult) {
        promotionsCount++;
        console.log(`🎉 Successfully promoted player ${player.user_id} from ${player.ranks?.code} to next rank`);
        ...
        console.log(`ℹ️ Player ${player.user_id} not promoted - may not meet ELO or verification requirements`);
      }
    }
