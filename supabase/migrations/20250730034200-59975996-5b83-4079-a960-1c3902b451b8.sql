-- Fix the SABO advancement function to handle Losers Branch A correctly
...
BEGIN
  ...
  IF v_completed_match.round_number = 1 THEN
    ...
    -- Loser goes to Losers Round 101
    SELECT id INTO v_next_match_id
    FROM tournament_matches
    WHERE tournament_id = p_tournament_id 
  ...
  ELSIF v_completed_match.round_number = 2 THEN
    ...
    -- ✅ CRITICAL FIX: Round 2 loser goes to Losers Round 102 (NOT duplicated!)
    -- Find Round 102 match that needs a Round 2 loser
    SELECT id INTO v_next_match_id
    FROM tournament_matches
    WHERE tournament_id = p_tournament_id 
      AND round_number = 102
      AND (player2_id IS NULL) -- Round 2 losers go to player2 slot
    ORDER BY match_number LIMIT 1;
  ...
  -- ✅ LOSERS BRACKET ROUND 101 ADVANCEMENT  
  ELSIF v_completed_match.round_number = 101 THEN
    -- Round 101 winner advances to Round 102 (player1 slot)
    SELECT id INTO v_next_match_id
    FROM tournament_matches
    WHERE tournament_id = p_tournament_id 
      AND round_number = 102
      AND (player1_id IS NULL) -- Round 101 winners go to player1 slot
    ORDER BY match_number LIMIT 1;
    
    IF v_next_match_id IS NOT NULL THEN
      UPDATE tournament_matches 
      SET player1_id = p_winner_id, status = 'scheduled' 
      WHERE id = v_next_match_id;
    END IF;
    
    -- Loser from Round 101 is eliminated
    
  -- ✅ LOSERS BRACKET ROUND 102 ADVANCEMENT
  ...
  RETURN jsonb_build_object(
    'success', true, 
    'message', format('Advanced %s successfully', v_round_info),
