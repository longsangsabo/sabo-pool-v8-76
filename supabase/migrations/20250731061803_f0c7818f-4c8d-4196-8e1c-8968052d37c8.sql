-- Update process_tournament_completion function to calculate match statistics
...
BEGIN
  ...
  -- Create results for all participants with match statistics
  ...
  SELECT 
    ...
    CASE pa.final_position
      ...
      WHEN 3 THEN 700   -- 3rd place
      WHEN 4 THEN 500   -- 4th place
      ELSE 200          -- Other positions
    END as spa_points,
    CASE pa.final_position
      WHEN 1 THEN 100   -- Champion ELO
      WHEN 2 THEN 50    -- Runner-up ELO
      WHEN 3 THEN 30    -- 3rd place ELO
      WHEN 4 THEN 20    -- 4th place ELO
      ELSE 10           -- Other positions ELO
    END as elo_points,
    CASE pa.final_position
      WHEN 1 THEN 5000000   -- Champion prize
      WHEN 2 THEN 3000000   -- Runner-up prize
      WHEN 3 THEN 2000000   -- 3rd place prize
      WHEN 4 THEN 1000000   -- 4th place prize
      ELSE 0                -- No prize
    END as prize_amount
  ...
  RETURN jsonb_build_object(
    ...
    'message', 'Tournament results processed with match statistics'
  );
...
-- Re-run the function for the SABO Pool 8 Ball tournament to fix existing data
