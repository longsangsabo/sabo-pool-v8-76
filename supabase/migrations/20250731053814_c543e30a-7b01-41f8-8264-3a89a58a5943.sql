-- Fix the sync function to properly count updated rows
...
BEGIN
  ...
  LOOP
    ...
    SET 
      ...
      physical_rewards = COALESCE(array_to_json(v_prize_tier.physical_items), '[]'::json),
    ...
    v_updated_count := v_updated_count + v_current_updated;
    
    RAISE NOTICE 'Updated position % with % records', v_prize_tier.position, v_current_updated;
  END LOOP;
