...
DO $$
BEGIN
  IF NOT EXISTS (SELECT FROM pg_tables WHERE tablename = 'tournament_results') THEN
    ...
    CREATE INDEX idx_tournament_results_final_position ON public.tournament_results(final_position);
...
-- Update played_at for existing records
UPDATE public.matches 
SET played_at = actual_end_time 
WHERE played_at IS NULL AND actual_end_time IS NOT NULL;
