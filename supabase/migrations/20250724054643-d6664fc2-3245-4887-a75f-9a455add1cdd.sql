-- Fix the logic error in advance_double_elimination_winner function
-- Winners from WB Round 3 should go to SEMIFINAL, not Loser Branch
...
BEGIN
  ...
  IF v_match.bracket_type = 'winner' THEN
    ...
    ELSIF v_match.round_number = 3 THEN
      -- ✅ FIX: Winner from WB Round 3 goes to Semifinal (not Branch A!)
      SELECT * INTO v_next_match
      FROM tournament_matches
      WHERE tournament_id = v_match.tournament_id
      ...
      -- ✅ FIX: Loser from WB Round 3 does NOT go to Loser Bracket (they're eliminated!)
      -- Winners from WB Round 3 advance to Semifinal, losers are OUT!
      RAISE NOTICE 'Loser % from WB Round 3 is eliminated (no Loser Bracket for Round 3)', v_loser_id;
    END IF;
    
    -- Send loser to appropriate Loser Bracket branch (ONLY from Rounds 1-2)
    IF v_match.round_number = 1 THEN
      ...
      IF FOUND THEN
        ...
        END IF;
        v_advancement_count := v_advancement_count + 1;
        RAISE NOTICE 'Sent WB Round 1 loser % to Branch A', v_loser_id;
      END IF;
    ...
    ELSIF v_match.round_number = 2 THEN
      ...
      IF FOUND THEN
        ...
        END IF;
        v_advancement_count := v_advancement_count + 1;
        RAISE NOTICE 'Sent WB Round 2 loser % to Branch B', v_loser_id;
      END IF;
    END IF;
  
  -- 🔽 LOSER BRACKET advancement logic (unchanged)
    ...
    IF v_match.branch_type = 'branch_a' THEN
      ...
      ELSIF v_match.round_number = 6 THEN
        ...
        WHERE tournament_id = v_match.tournament_id
          AND bracket_type = 'semifinal'
          AND match_number = 1
          AND (player2_id IS NULL); -- Branch A winner goes to player2 of Semifinal 1
    ...
    ELSIF v_match.branch_type = 'branch_b' THEN
      ...
      ELSIF v_match.round_number = 8 THEN
        ...
        WHERE tournament_id = v_match.tournament_id
          AND bracket_type = 'semifinal'
          AND match_number = 2
          AND (player2_id IS NULL); -- Branch B winner goes to player2 of Semifinal 2
  ...
  -- 🏁 SEMIFINAL advancement logic (unchanged)
