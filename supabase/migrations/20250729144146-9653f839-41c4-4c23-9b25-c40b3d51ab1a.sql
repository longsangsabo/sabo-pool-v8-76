-- 🚨 COMPREHENSIVE DOUBLE ELIMINATION SYSTEM FIX (Fixed)
...
BEGIN
  ...
  -- Create registrations for 16 players (without priority_order column)
  FOR i IN 1..16 LOOP
...
-- Step 3: Recreate the auto-advancement trigger with proper logic
...
BEGIN
  ...
  IF NEW.status = 'completed' AND NEW.winner_id IS NOT NULL AND 
    ...
    -- Execute advancement immediately with proper error handling
