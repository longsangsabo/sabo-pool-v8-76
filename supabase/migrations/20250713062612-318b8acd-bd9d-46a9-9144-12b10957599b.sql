-- Fix ambiguous user_id column reference in admin_credit_spa_points function
CREATE OR REPLACE FUNCTION public.admin_credit_spa_points(p_user_id uuid, p_amount integer, p_reason text, p_admin_id uuid)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path TO ''
...
BEGIN
  ...
  -- Get current SPA points from player_rankings using explicit table alias
  SELECT COALESCE(pr.spa_points, 0) INTO v_current_spa
  FROM public.player_rankings pr
  WHERE pr.user_id = p_user_id;
  
  -- If player ranking doesn't exist, create it using explicit table alias
  ...
  -- Get current wallet balance using explicit table alias
  SELECT COALESCE(w.points_balance, 0) INTO v_current_wallet_balance
  ...
  -- Update SPA points in player_rankings using explicit table alias
  UPDATE public.player_rankings pr
  SET spa_points = v_new_spa,
      updated_at = NOW()
  WHERE pr.user_id = p_user_id;
  
  -- Update wallet balance using explicit table alias
