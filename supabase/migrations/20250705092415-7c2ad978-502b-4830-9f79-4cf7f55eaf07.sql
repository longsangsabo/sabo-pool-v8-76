-- Phase 1: Database Functions & Triggers for Club Registration System
...
-- 4. Update existing trigger to use new notification system
DROP TRIGGER IF EXISTS on_club_registration_status_change ON public.club_registrations;
DROP FUNCTION IF EXISTS public.notify_club_registration_status();

-- New optimized trigger that only runs for admin notifications on new registrations
