-- Add unique constraint to tournament_brackets table for ON CONFLICT
ALTER TABLE public.tournament_brackets 
ADD CONSTRAINT tournament_brackets_tournament_id_unique UNIQUE (tournament_id);

-- Update the bracket generation function to fix ON CONFLICT and table schema mismatch
...
BEGIN
  ...
  -- Insert or update bracket record with correct column mapping
  INSERT INTO public.tournament_brackets (
    tournament_id, bracket_data, bracket_type, total_players, total_rounds, current_round, created_at, updated_at
  ) VALUES (
    p_tournament_id, v_bracket_data, v_tournament.tournament_type, v_participant_count, v_rounds, 1, now(), now()
