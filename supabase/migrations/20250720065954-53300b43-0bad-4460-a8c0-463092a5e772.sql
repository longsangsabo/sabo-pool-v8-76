...
-- Fix the complete_tournament_automatically function with comprehensive improvements
...
BEGIN
  ...
  IF v_tournament.status = 'completed' THEN
    ...
    RETURN jsonb_build_object('error', 'Tournament is already completed', 'status', v_tournament.status);
  END IF;
  ...
  RAISE NOTICE 'Tournament status updated, rows affected: %', v_results_inserted;
  ...
  RAISE NOTICE 'Tournament completion successful - Total players: %, Results: %, Rankings updated: %', 
    v_total_players, v_results_inserted, v_players_updated;
