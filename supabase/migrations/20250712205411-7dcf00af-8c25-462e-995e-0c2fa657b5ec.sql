...
CREATE TABLE public.tournament_reward_structures (
  ...
  UNIQUE(rank_code, tournament_type, position)
);
...
CREATE TABLE public.spa_reward_milestones (
  ...
  position INTEGER, -- For tournament positions (1st, 2nd, etc.)
...
-- Insert tournament reward data (based on current SPA_TOURNAMENT_REWARDS)
INSERT INTO public.tournament_reward_structures (rank_code, tournament_type, position, base_points, multiplier, description) VALUES
-- Regular tournaments
...
INSERT INTO public.spa_reward_milestones (rank_code, milestone_type, position, points_awarded, description) VALUES
-- Tournament wins
...
-- Create function to get tournament rewards by rank
CREATE OR REPLACE FUNCTION public.get_tournament_rewards_by_rank(p_rank_code TEXT, p_tournament_type TEXT DEFAULT 'regular')
RETURNS TABLE (
  position INTEGER,
  points INTEGER,
  description TEXT
...
AS $$
  SELECT 
    trs.position,
    (trs.base_points * trs.multiplier)::INTEGER as points,
    trs.description
  FROM public.tournament_reward_structures trs
  WHERE trs.rank_code = p_rank_code 
    AND trs.tournament_type = p_tournament_type
    AND trs.is_active = true
  ORDER BY trs.position;
$$;

-- Create function to get SPA rewards by rank and type
CREATE OR REPLACE FUNCTION public.get_spa_rewards_by_rank(p_rank_code TEXT, p_milestone_type TEXT)
RETURNS TABLE (
  position INTEGER,
  points INTEGER,
  description TEXT
...
AS $$
  SELECT 
    ...
    srm.points_awarded,
    srm.description
  FROM public.spa_reward_milestones srm
  WHERE srm.rank_code = p_rank_code 
    AND srm.milestone_type = p_milestone_type
    AND srm.is_active = true
  ORDER BY srm.position NULLS LAST;
$$;