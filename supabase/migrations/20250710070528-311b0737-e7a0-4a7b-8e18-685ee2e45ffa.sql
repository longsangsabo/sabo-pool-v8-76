-- Cập nhật function để tạo trận tranh hạng 3-4 và cải thiện logic tạo vòng đấu
...
BEGIN
  ...
  WHILE v_current_round < v_max_round LOOP
    ...
    IF v_current_round = v_max_round THEN
      -- Final round: Create both Final and 3rd Place matches
      
      -- 3rd Place match (between losers of semi-finals)
      ...
      -- Final match (between winners of semi-finals) 
      INSERT INTO public.tournament_matches (
        tournament_id, round_number, match_number,
        status, scheduled_time, created_at, updated_at,
        notes
      ...
      v_matches_created := v_matches_created + 2;
    ...
    -- Update round names in a separate update
    UPDATE public.tournament_matches 
    SET notes = CASE 
      WHEN match_number = 1 AND round_number = v_max_round AND notes IS NULL THEN 'Tranh hạng 3-4'
      WHEN match_number = 2 AND round_number = v_max_round AND notes IS NULL THEN 'Chung kết'
