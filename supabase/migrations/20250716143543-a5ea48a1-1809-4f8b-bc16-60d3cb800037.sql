...
-- 1. Clean up failed double elimination tournaments
...
-- 4. Create function to generate modified double elimination bracket
...
DECLARE
  ...
  v_wb_rounds JSONB := '[]'::jsonb;
  v_lb_rounds JSONB := '[]'::jsonb;
  i INTEGER;
  j INTEGER;
BEGIN
...
-- 5. Create function to advance players in modified DE system
...
BEGIN
  ...
  IF v_tournament.current_phase = 'double_elimination' THEN
    ...
    IF v_match.bracket_type = 'winner' THEN
      ...
      -- Drop loser to appropriate LB match
      -- (Logic depends on which WB round this is)
...
-- 6. Add bracket_type column to tournament_matches if not exists
