-- Fix lỗi function complete_tournament_automatically 
-- Function đang tìm field 'tier' không tồn tại trong tournaments table
...
BEGIN
  -- Get tournament info
  SELECT id, name, status, tournament_type INTO v_tournament 
  ...
  -- Nếu không tìm thấy final match với bracket_type = 'final', 
  -- tìm match cuối cùng (round cao nhất)
  IF NOT FOUND THEN
    SELECT * INTO v_final_match 
    FROM tournament_matches 
    WHERE tournament_id = p_tournament_id 
