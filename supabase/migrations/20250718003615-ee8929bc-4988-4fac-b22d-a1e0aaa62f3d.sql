...
CREATE TEMP TABLE automation_performance_log_backup AS 
SELECT * FROM public.automation_performance_log WHERE tournament_id IN (SELECT tournament_id FROM public.tournament_matches);

-- STEP 2: Drop all dependencies and constraints
...
-- STEP 4: Create new tournament_matches table with clean structure
CREATE TABLE public.tournament_matches (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  tournament_id UUID NOT NULL REFERENCES public.tournaments(id) ON DELETE CASCADE,
  round_number INTEGER NOT NULL,
  match_number INTEGER NOT NULL,
  bracket_type TEXT DEFAULT 'main' CHECK (bracket_type IN ('main', 'winner', 'loser', 'semifinal', 'final')),
...
-- STEP 9: Create table release trigger
...
-- STEP 10: Restore foreign key constraints to other tables
...
-- STEP 11: Restore data (convert old column names to new ones)
...
SELECT 
  ...
  COALESCE(player1_score, score_player1, 0), COALESCE(player2_score, score_player2, 0),
  COALESCE(status, 'scheduled'), scheduled_time, started_at, completed_at,
  COALESCE(score_status, 'pending'), score_confirmed_by, score_confirmed_at,
  referee_id, notes, created_at, updated_at
FROM tournament_matches_backup;

-- Restore match_events data
...
DROP TABLE automation_performance_log_backup;