-- STEP 1: Complete rewrite of advance_double_elimination_v9 with correct logic
...
BEGIN
  ...
  LOOP
    ...
    IF v_current_players.player1_id = v_winner_id OR v_current_players.player2_id = v_winner_id THEN
      v_errors := v_errors || format('❌ DUPLICATE: Winner % already in R%M%', 
    ...
    IF v_completed_match.match_number % 2 = 1 THEN
      ...
      IF v_current_players.player1_id IS NULL THEN
        ...
        IF v_updated_rows > 0 THEN
          RAISE NOTICE '✅ Advanced winner % to Winners R% M% player1', v_winner_id, v_next_round, v_next_match_number;
          v_advancement_count := v_advancement_count + 1;
        END IF;
      ELSE
        v_errors := v_errors || format('❌ Cannot advance winner % to R%M% player1 - slot occupied', 
          v_winner_id, v_next_round, v_next_match_number);
        CONTINUE;
      END IF;
    ELSE
      ...
      IF v_current_players.player2_id IS NULL THEN
        ...
        GET DIAGNOSTICS v_updated_rows = ROW_COUNT;
        IF v_updated_rows > 0 THEN
          RAISE NOTICE '✅ Advanced winner % to Winners R% M% player2', v_winner_id, v_next_round, v_next_match_number;
          v_advancement_count := v_advancement_count + 1;
        END IF;
      ELSE
        v_errors := v_errors || format('❌ Cannot advance winner % to R%M% player2 - slot occupied', 
          v_winner_id, v_next_round, v_next_match_number);
        CONTINUE;
      END IF;
    END IF;
    ...
    IF v_completed_match.round_number = 1 THEN
      ...
      IF v_completed_match.match_number % 2 = 1 THEN
        ...
        IF v_current_players.player1_id IS NULL AND v_current_players.player2_id != v_loser_id THEN
          ...
          RAISE NOTICE '✅ Moved loser % to Branch A R% M% player1', v_loser_id, v_next_round, v_next_match_number;
        END IF;
      ELSE
        ...
        IF v_current_players.player2_id IS NULL AND v_current_players.player1_id != v_loser_id THEN
          ...
          RAISE NOTICE '✅ Moved loser % to Branch A R% M% player2', v_loser_id, v_next_round, v_next_match_number;
        END IF;
      END IF;
    ...
    ELSIF v_completed_match.round_number = 2 THEN
      ...
      IF v_current_players.player1_id IS NULL AND v_current_players.player2_id != v_loser_id THEN
        ...
        RAISE NOTICE '✅ Moved loser % to Branch B R% M% player1', v_loser_id, v_next_round, v_next_match_number;
      ELSIF v_current_players.player2_id IS NULL AND v_current_players.player1_id != v_loser_id THEN
        ...
        RAISE NOTICE '✅ Moved loser % to Branch B R% M% player2', v_loser_id, v_next_round, v_next_match_number;
      END IF;
    END IF;
  END LOOP;
  ...
  LOOP
    ...
    IF v_completed_match.match_number % 2 = 1 THEN
      IF v_current_players.player1_id IS NULL AND v_current_players.player2_id != v_winner_id THEN
        ...
        RAISE NOTICE '✅ Advanced losers winner % to R% M% player1', v_winner_id, v_next_round, v_next_match_number;
        v_advancement_count := v_advancement_count + 1;
      END IF;
    ELSE
      IF v_current_players.player2_id IS NULL AND v_current_players.player1_id != v_winner_id THEN
        ...
        RAISE NOTICE '✅ Advanced losers winner % to R% M% player2', v_winner_id, v_next_round, v_next_match_number;
        v_advancement_count := v_advancement_count + 1;
      END IF;
    END IF;
  END LOOP;
  ...
  PERFORM public.validate_tournament_bracket_integrity(p_tournament_id);

  RAISE NOTICE '✅ Completed CORRECTED advance_double_elimination_v9_fixed: % advancements, % errors', 
...
-- STEP 2: Create validation function to check bracket integrity
CREATE OR REPLACE FUNCTION public.validate_tournament_bracket_integrity(p_tournament_id uuid)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
BEGIN
  ...
  FOR v_duplicates IN
    SELECT round_number, bracket_type, player_id, COUNT(*) as count
    FROM (
      SELECT round_number, bracket_type, player1_id as player_id
      FROM tournament_matches 
      WHERE tournament_id = p_tournament_id AND player1_id IS NOT NULL
      UNION ALL
      SELECT round_number, bracket_type, player2_id as player_id
      FROM tournament_matches 
      WHERE tournament_id = p_tournament_id AND player2_id IS NOT NULL
    ) players
    GROUP BY round_number, bracket_type, player_id
    HAVING COUNT(*) > 1
  LOOP
    v_errors := v_errors || format('❌ DUPLICATE: Player % appears %s times in %s Round %s', 
      v_duplicates.player_id, v_duplicates.count, v_duplicates.bracket_type, v_duplicates.round_number);
  END LOOP;

  IF array_length(v_errors, 1) > 0 THEN
    RAISE WARNING '❌ Bracket integrity validation failed with % errors', array_length(v_errors, 1);
    RETURN jsonb_build_object(
      'valid', false,
      'errors', v_errors
    );
...
-- STEP 3: Update the main advance_double_elimination_v9 to use the fixed version
