...
-- Fix the complete_tournament_automatically function with correct SQL syntax
...
BEGIN
  ...
  INSERT INTO player_rankings (user_id, spa_points, elo_points, total_matches, wins, losses, tournament_wins, updated_at)
  SELECT 
    tr.user_id,
    tr.spa_points_earned,
  ...
  FROM tournament_results tr
  WHERE tr.tournament_id = p_tournament_id
    AND NOT EXISTS (SELECT 1 FROM player_rankings pr WHERE pr.user_id = tr.user_id);
  
  -- Get total players count
  SELECT COUNT(*) INTO v_total_players FROM final_positions;
...
-- Also manually complete the specific tournament if it exists
DO $$
DECLARE
  tournament_exists BOOLEAN;
  completion_result JSONB;
BEGIN
  -- Check if the tournament exists and is not already completed
  SELECT EXISTS (
    SELECT 1 FROM tournaments 
    WHERE name = 'Sáng tạo nội dung' 
    AND status != 'completed'
  ) INTO tournament_exists;
  
  IF tournament_exists THEN
    -- Get the tournament ID and complete it
    SELECT complete_tournament_automatically(id) INTO completion_result
    FROM tournaments 
    WHERE name = 'Sáng tạo nội dung' 
    AND status != 'completed'
    LIMIT 1;
    
    RAISE NOTICE 'Tournament completion result: %', completion_result;
  ELSE
    RAISE NOTICE 'Tournament "Sáng tạo nội dung" either does not exist or is already completed';
  END IF;
END $$;
