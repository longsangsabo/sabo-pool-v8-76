...
-- Fix foreign key constraint issue by dropping and recreating with proper handling
-- Also improve the bracket generation function for better random seeding

-- First, drop existing problematic constraints if they exist
DO $$
BEGIN
...
-- Add proper foreign key constraints that handle NULL values correctly
ALTER TABLE public.tournament_matches 
...
-- Update the bracket generation function to handle random seeding better
...
BEGIN
  ...
  INSERT INTO public.tournament_brackets (
    tournament_id, bracket_data, created_at, updated_at
  ) VALUES (
    p_tournament_id, v_bracket_data, now(), now()
  );
  ...
  FOR match_idx IN 1..array_length(v_matches, 1) LOOP
    ...
    BEGIN
      ...
      -- Only insert if at least one player is not NULL or both are NULL (for future rounds)
