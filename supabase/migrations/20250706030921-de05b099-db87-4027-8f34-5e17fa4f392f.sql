-- PHASE 1: SABO Pool Ranking System Database Schema
...
-- Function to check and promote rank based on SPA points
...
BEGIN
  ...
  SELECT pr.spa_points, r.level, r.code, r.id 
  INTO v_spa_points, v_current_rank
  ...
  IF v_current_rank IS NULL THEN
    SELECT * INTO v_current_rank FROM public.ranks WHERE code = 'K';
    
    INSERT INTO public.player_rankings (player_id, current_rank_id, spa_points)
    VALUES (p_player_id, v_current_rank.id, COALESCE(v_spa_points, 0))
    ON CONFLICT (player_id) DO UPDATE SET current_rank_id = v_current_rank.id;
  ...
  IF v_spa_points >= 1000 THEN
    ...
    FROM public.ranks 
    WHERE level = v_current_rank.level + 1;

    IF v_next_rank.id IS NOT NULL THEN
      -- Promote player
      UPDATE public.player_rankings 
      SET current_rank_id = v_next_rank.id,
      ...
      VALUES (p_player_id, v_current_rank.id, v_next_rank.id, 1.0);
      ...
      VALUES (p_player_id, 'rank_promotion', 'Chúc mừng thăng hạng!', 
              format('Bạn đã thăng hạng từ %s lên %s', v_current_rank.code, v_next_rank.code),
