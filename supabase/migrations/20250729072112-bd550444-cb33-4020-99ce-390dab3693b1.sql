-- Fix auto_advance_to_final function - PostgreSQL array syntax
...
BEGIN
  ...
  -- Get the two semifinal winners
  WITH semifinal_winners AS (
    SELECT winner_id, match_number, ROW_NUMBER() OVER (ORDER BY match_number) as rn
    FROM tournament_matches 
    WHERE tournament_id = p_tournament_id 
      AND round_number = 250 
      AND status = 'completed' 
  ...
  SELECT 
    MAX(CASE WHEN rn = 1 THEN winner_id END),
    MAX(CASE WHEN rn = 2 THEN winner_id END)
  INTO v_semifinal_winner1, v_semifinal_winner2
  FROM semifinal_winners;
