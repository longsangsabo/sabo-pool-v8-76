-- Create complete tournament system database schema (with conflict resolution)
...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'tournament_registrations' AND column_name = 'status') THEN
    ALTER TABLE public.tournament_registrations 
    ADD COLUMN status TEXT DEFAULT 'registered' CHECK (status IN ('registered', 'confirmed', 'checked_in', 'no_show', 'disqualified'));
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'tournament_registrations' AND column_name = 'seed_number') THEN
    ALTER TABLE public.tournament_registrations ADD COLUMN seed_number INTEGER;
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'tournament_registrations' AND column_name = 'bracket_position') THEN
    ALTER TABLE public.tournament_registrations ADD COLUMN bracket_position INTEGER;
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'tournament_registrations' AND column_name = 'payment_date') THEN
    ALTER TABLE public.tournament_registrations ADD COLUMN payment_date TIMESTAMP WITH TIME ZONE;
  END IF;
  ...
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'tournament_registrations' AND column_name = 'check_in_time') THEN
    ALTER TABLE public.tournament_registrations ADD COLUMN check_in_time TIMESTAMP WITH TIME ZONE;
  END IF;
END $$;

-- 3. Create tournament_matches table if it doesn't exist
...
-- 4. Create indexes for better query performance (only if they don't exist)
...
DROP POLICY IF EXISTS "Admins and club owners can create tournaments" ON public.tournaments;
...
-- 7. Create trigger function and triggers for updating timestamps
