-- Fix the mapping logic for double elimination loser bracket
CREATE OR REPLACE FUNCTION public.advance_loser_to_bracket_fixed(
  p_match_id UUID, 
  p_loser_id UUID
) RETURNS JSONB
LANGUAGE plpgsql
...
BEGIN
  ...
  -- But we need to map 8 winner matches to 4 loser matches
  IF v_match.round_number = 1 THEN
    v_target_round := 1;
    -- Map 8 winner matches to 4 loser matches:
  ...
  -- For odd numbered winner matches (1,3,5,7), put in player1
  -- For even numbered winner matches (2,4,6,8), put in player2
  IF v_match.match_number % 2 = 1 THEN
    -- Odd match number -> player1
    ...
    ELSE
      RETURN jsonb_build_object(
        'success', false,
        'error', 'Target loser match player1 slot already filled'
      );
    END IF;
  ELSE
    -- Even match number -> player2
    ...
    ELSE
      RETURN jsonb_build_object(
        'success', false,
        'error', 'Target loser match player2 slot already filled'
      );
    END IF;
  END IF;
  ...
  RETURN jsonb_build_object(
    ...
    'mapping_rule', format('Winner match %s -> Loser match %s slot %s', v_match.match_number, v_target_match_number, v_target_slot)
  );
...
-- Update the main function to use the fixed logic
CREATE OR REPLACE FUNCTION public.fix_loser_bracket_advancements_corrected(p_tournament_id UUID)
RETURNS JSONB
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path TO 'public'
...
BEGIN
  ...
  LOOP
    ...
    IF NOT EXISTS (
      ...
      -- Advance the loser using the fixed function
      SELECT public.advance_loser_to_bracket_fixed(v_completed_match.id, v_loser_id) INTO v_advancement_result;
