-- Fix profile update error by correcting field name mismatch in SABO challenge functions

-- The issue is in the admin_create_sabo_challenge function where it references 
-- pr.player_id but should reference the correct column name from player_rankings table

-- Update the admin_create_sabo_challenge function to fix player_id references
...
BEGIN
  ...
  -- Validate challenger exists and has enough SPA (fixed player_id reference)
  SELECT 
  ...
  LEFT JOIN public.player_rankings pr ON p.user_id = pr.player_id  -- This is correct
  ...
  -- Validate opponent exists and has enough SPA (fixed player_id reference)
  SELECT 
    CASE WHEN COUNT(*) > 0 THEN true ELSE false END,
    COALESCE(MAX(COALESCE(w.points_balance, pr.spa_points)), 0),
    MAX(p.full_name)
...
-- Also fix the accept_sabo_challenge function
...
BEGIN
  ...
  -- Check if user has enough SPA points (fixed player_id reference)
