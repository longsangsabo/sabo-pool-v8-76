...
-- 3. Fix sync_spa_wallet_data function if it exists
...
DECLARE
  ...
  v_user RECORD;
BEGIN
  -- Sync SPA points from player_rankings to wallets
  FOR v_user IN 
    SELECT pr.user_id, pr.spa_points
    FROM public.player_rankings pr
    LEFT JOIN public.wallets w ON pr.user_id = w.user_id
    WHERE w.user_id IS NULL OR w.points_balance != pr.spa_points
  LOOP
    -- Create or update wallet
    INSERT INTO public.wallets (user_id, points_balance, balance, status)
    VALUES (v_user.user_id, v_user.spa_points, 0, 'active')
    ON CONFLICT (user_id)
    DO UPDATE SET
      points_balance = v_user.spa_points,
      updated_at = NOW();
