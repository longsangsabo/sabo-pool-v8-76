-- Clean up broken double elimination system and implement modified version - Type-safe approach

-- 1. Add phase tracking columns to tournaments table
...
-- 2. Add bracket_type column to tournament_matches
ALTER TABLE tournament_matches 
ADD COLUMN IF NOT EXISTS bracket_type TEXT DEFAULT 'single';

-- 3. Update existing data first
UPDATE tournaments SET current_phase = 'single_elimination' WHERE current_phase IS NULL;
UPDATE tournament_matches SET bracket_type = 'single' WHERE bracket_type IS NULL;

-- 4. Create enum for tournament phases
DO $$ BEGIN
...
-- 5. Alter column type safely with explicit casting
ALTER TABLE tournaments 
...
ALTER COLUMN current_phase SET DEFAULT 'single_elimination'::tournament_phase;
