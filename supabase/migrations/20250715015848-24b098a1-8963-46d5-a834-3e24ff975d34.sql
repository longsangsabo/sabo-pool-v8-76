-- Fix the generate_advanced_tournament_bracket function to set total_players correctly
...
BEGIN
  ...
  -- Double check - ensure participant count is never null
  ...
  -- Create tournament bracket record with total_players properly set
  INSERT INTO public.tournament_brackets (
    ...
    v_rounds,
    1,
    v_participant_count,  -- Make sure total_players is set to participant count
  ...
  -- Add bye players if needed - check if bye count is greater than 0
  ...
  -- Generate all rounds - ensure we have valid bounds
  IF v_rounds > 0 THEN
    FOR v_round IN 1..v_rounds LOOP
      v_match_in_round := 1;
      
      -- Ensure we have valid players in round
