-- Fix tournament results for test5 tournament with correct placement_type
-- Problem: placement_type 'round_of_16' not allowed in constraint
...
ranked_results AS (
  SELECT 
    ...
    CASE 
      WHEN ROW_NUMBER() OVER (ORDER BY random_order) = 1 THEN 4   -- Champion: 4 wins, 0 losses
      WHEN ROW_NUMBER() OVER (ORDER BY random_order) = 2 THEN 4   -- Runner-up: 4 wins, 1 loss 
      WHEN ROW_NUMBER() OVER (ORDER BY random_order) = 3 THEN 3   -- 3rd place: 3 wins, 1 loss
      WHEN ROW_NUMBER() OVER (ORDER BY random_order) = 4 THEN 3   -- 4th place: 3 wins, 1 loss
      WHEN ROW_NUMBER() OVER (ORDER BY random_order) <= 8 THEN 2  -- QF losers: 2 wins, 1 loss
      ELSE 1 -- Preliminary round losers: 1 win, 1 loss
    END as matches_won,
    ...
    CASE 
      ...
      ELSE 'preliminary'  -- Use 'preliminary' instead of 'round_of_16'
