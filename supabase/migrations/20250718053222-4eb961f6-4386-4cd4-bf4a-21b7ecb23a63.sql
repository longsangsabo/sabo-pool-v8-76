-- Fix table name in the function - should be tournaments_matches or similar
...
BEGIN
  ...
  -- Find final match (highest round number with a winner)
  SELECT * INTO v_final_match
  FROM (
    SELECT tm.*, ROW_NUMBER() OVER (ORDER BY round_number DESC, match_number ASC) as rn
    FROM public.tournaments_matches tm
  ...
  -- Build position mapping based on final match
  v_position_map := '{}';
  
  -- Assign champion and runner-up from final match
  ...
  FOR v_participant IN 
    ...
    WHERE tm.tournament_id = p_tournament_id
      ...
      AND tm.player2_id NOT IN (
        ...
        FROM public.tournaments_matches tm2
  ...
  LOOP
    ...
    BEGIN
      ...
      IF player_position IS NULL THEN
        ...
        FROM public.tournaments_matches
        WHERE tournament_id = p_tournament_id
          AND (player1_id = v_participant.player_id OR player2_id = v_participant.player_id)
          AND status = 'completed';
        
        -- Estimate position based on performance
        player_position := GREATEST(3, 15 - (player_wins * 2));
      END IF;
      ...
      INSERT INTO public.player_rankings (user_id, spa_points, total_matches, updated_at)
