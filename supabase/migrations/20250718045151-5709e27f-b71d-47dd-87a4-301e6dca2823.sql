-- Kiểm tra và sửa trigger advancement
...
BEGIN
  ...
  IF v_tournament.tournament_type = 'single_elimination' THEN
    ...
    IF v_next_round <= v_max_round THEN
      ...
      SELECT id, 
             CASE 
               WHEN player1_id IS NULL THEN 'player1'
               WHEN player2_id IS NULL THEN 'player2'
               ELSE 'full'
             END INTO v_next_match_id, v_opponent_id
      ...
      ELSE
        ...
        IF v_opponent_id = 'player1' THEN
          UPDATE tournament_matches 
          SET player1_id = p_winner_id, updated_at = NOW()
          WHERE id = v_next_match_id;
        ELSIF v_opponent_id = 'player2' THEN
          UPDATE tournament_matches 
