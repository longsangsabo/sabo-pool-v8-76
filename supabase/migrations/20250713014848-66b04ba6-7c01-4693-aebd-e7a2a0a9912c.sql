-- COMPLETE RANK VERIFICATION SYSTEM FIX - CORRECTED
...
-- 3. FIXED TRIGGER FUNCTION - Handle ELO values correctly
...
BEGIN
  ...
  IF NEW.status = 'approved' AND (OLD.status IS NULL OR OLD.status != 'approved') THEN
    BEGIN
      ...
      -- CRITICAL: Update profiles.verified_rank (correct column name)
      UPDATE public.profiles 
      SET 
        verified_rank = v_rank_text,
        rank_verified_at = NOW(),
        rank_verified_by = NEW.club_id,
        updated_at = NOW()
...
-- 5. FIX EXISTING DATA - Update profiles for already approved requests (CORRECTED)
DO $$
DECLARE
  v_request RECORD;
  v_rank_text TEXT;
...
BEGIN
  ...
  LOOP
    ...
    SET 
      verified_rank = v_rank_text, 
      rank_verified_at = NOW(),
      rank_verified_by = v_request.club_id,
      updated_at = NOW()
    WHERE user_id = v_request.user_id;
    
    -- Update/Insert rank verification
    INSERT INTO public.rank_verifications (
