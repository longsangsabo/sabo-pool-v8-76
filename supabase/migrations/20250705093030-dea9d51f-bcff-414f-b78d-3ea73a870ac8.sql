-- Phase 2: Real-time Improvements and Performance Optimization (Fixed)
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE pubname = 'supabase_realtime' AND tablename = 'club_registrations'
  ) THEN
    ALTER publication supabase_realtime ADD TABLE public.club_registrations;
  END IF;
END $$;

-- 2. Enable realtime for club_profiles table  
...
-- 3. Additional performance indexes for common queries
...
-- 4. Create materialized view for admin dashboard stats
...
-- 6. Function to refresh admin dashboard stats
...
-- 7. Create notification read function for better performance
...
-- 8. Create batch notification creation function
...
-- 9. Create trigger to refresh stats on club registration changes
...
-- 11. Create function for efficient notification summary
