-- Fix the generate_advanced_tournament_bracket function to handle foreign key constraints properly
...
DECLARE
  ...
  v_participant_cursor CURSOR FOR 
    SELECT 
      ...
      COALESCE(pr.elo_points, pr.elo, 1000) as elo_rating,
      tr.registration_date,
      ROW_NUMBER() OVER (
        ORDER BY 
        CASE p_seeding_method
          WHEN 'elo_ranking' THEN COALESCE(pr.elo_points, pr.elo, 1000)
    ...
    AND tr.player_id IS NOT NULL  -- Ensure player_id exists
    AND EXISTS (SELECT 1 FROM public.profiles WHERE user_id = tr.player_id); -- Ensure player exists in profiles
