-- Fix auto advancement logic cho chuẩn mới: 4→2 Semifinal
-- 1. Tạo function để auto advance 4 finalists vào semifinal
...
BEGIN
  ...
  -- Lấy 2 winners từ Winner's Bracket (Round 3)
  SELECT array_agg(winner_id) INTO v_winner_bracket_winners
  FROM tournament_matches 
  WHERE tournament_id = p_tournament_id
    AND round_number = 3
    AND bracket_type = 'winners'
...
-- 2. Function để auto advance 2 semifinal winners vào final
CREATE OR REPLACE FUNCTION auto_advance_to_final(p_tournament_id UUID)
RETURNS JSONB
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path TO 'public'
...
DECLARE
  ...
  v_semifinal_completed INTEGER;
BEGIN
  -- Kiểm tra xem đã có final match chưa
  ...
  -- Nếu đã có final match, không cần tạo thêm
  ...
  -- Kiểm tra semifinal completed (Round 250)
  SELECT COUNT(*) INTO v_semifinal_completed
  FROM tournament_matches 
  WHERE tournament_id = p_tournament_id
    AND round_number = 250
  ...
  -- Cần có đúng 2 semifinal winners
  IF v_semifinal_completed < 2 THEN
    RETURN jsonb_build_object(
      'success', false,
      'message', 'Not all semifinal matches completed',
      'semifinal_completed', v_semifinal_completed
    );
  END IF;
  
  -- Lấy 2 winners từ semifinal (Round 250)
  ...
  -- Cancel existing wrong final matches (including round 301, 302)
  UPDATE tournament_matches 
  SET status = 'cancelled', updated_at = NOW()
  WHERE tournament_id = p_tournament_id
    AND round_number IN (300, 301, 302);
  
  -- Tạo 1 final match theo chuẩn mới (Round 300)
  INSERT INTO tournament_matches (
    tournament_id, round_number, match_number, bracket_type, match_stage,
    player1_id, player2_id, status, created_at, updated_at
  ) VALUES (
    p_tournament_id, 300, 1, 'final', 'championship_final',
    v_semifinal_winners[1], v_semifinal_winners[2], 'scheduled', NOW(), NOW()
  );
  ...
  INSERT INTO tournament_automation_log (
    ...
    p_tournament_id, 'final_advancement', 'completed',
    jsonb_build_object(
      'semifinal_winners', v_semifinal_winners,
      'final_matches_created', 1
    ),
    NOW()
  );
