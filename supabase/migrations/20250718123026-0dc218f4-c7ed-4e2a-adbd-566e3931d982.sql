-- Fix bracket progression logic specifically for Round 3+ advancement
...
-- Function to fix round 3+ progression with duplicate detection
CREATE OR REPLACE FUNCTION public.fix_round_progression_from_round(
  p_tournament_id UUID, 
  p_start_round INTEGER DEFAULT 2
)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  -- Reset all matches from start_round+1 onwards
  UPDATE tournament_matches 
  SET player1_id = NULL,
      player2_id = NULL,
      score_player1 = 0,
  ...
  WHERE tournament_id = p_tournament_id 
    AND round_number > p_start_round;
  
  -- Re-advance all winners from start_round onwards
  FOR v_match IN 
  ...
  LOOP
    SELECT public.advance_winner_to_next_round(v_match.id, TRUE) INTO v_result;
  ...
  RETURN jsonb_build_object(
    ...
    'message', 'Round progression fixed from round ' || p_start_round
  );
  
EXCEPTION
  WHEN OTHERS THEN
    RETURN jsonb_build_object(
      'error', 'Failed to fix progression: ' || SQLERRM
    );
END;
$$;

-- Fix tournament test2 from round 2 onwards
SELECT public.fix_round_progression_from_round('2b252d70-5cf3-427f-92b7-48eea40753d8', 2);