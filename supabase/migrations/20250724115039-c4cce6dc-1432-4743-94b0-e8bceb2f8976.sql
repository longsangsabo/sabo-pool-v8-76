-- Fix generate_double_elimination_bracket_complete with proper participant handling
...
BEGIN
  ...
  IF v_wb_rounds >= 1 THEN
    DECLARE
      v_wb_r1_losers INTEGER := v_bracket_size / 2;
  ...
  IF v_wb_rounds >= 2 THEN
    DECLARE
      v_wb_r2_losers INTEGER := v_bracket_size / 4;
...
-- Create auto-advancement function for branched double elimination
...
DECLARE
  v_result JSONB;
BEGIN
  -- Only trigger when a winner is set
...
DROP TRIGGER IF EXISTS trigger_branched_tournament_match_completion ON tournament_matches;
