-- 🚨 COMPREHENSIVE DOUBLE ELIMINATION SYSTEM FIX (Final - Without Concurrent Indexes)
...
-- Step 4: Add performance indexes (without CONCURRENTLY)
CREATE INDEX IF NOT EXISTS idx_tournament_matches_tournament_bracket 
  ON tournament_matches(tournament_id, bracket_type, round_number, match_number);

CREATE INDEX IF NOT EXISTS idx_tournament_matches_status_winner 
...
BEGIN
  ...
  IF v_tournament_record.id IS NOT NULL THEN
  ...
  ELSE
    RAISE NOTICE '❌ Tournament "new3" not found!';
  END IF;
END $$;