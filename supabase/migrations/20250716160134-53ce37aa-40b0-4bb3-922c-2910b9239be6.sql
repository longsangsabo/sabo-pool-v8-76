-- Fix double elimination tournament generation by updating existing functions
...
-- 2. Drop existing function if it exists to avoid conflict
DROP FUNCTION IF EXISTS public.generate_single_elimination_bracket(UUID, UUID[]);

-- 3. Update the main bracket generation function to handle double elimination
