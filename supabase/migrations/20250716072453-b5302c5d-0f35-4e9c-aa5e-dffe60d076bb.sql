-- Fix tournament sabo1 results with correct ELO and SPA calculations (without spa_transactions)
...
results_with_rewards AS (
  SELECT 
    *,
    ...
    -- Correct SPA points based on rank and position using SPA_TOURNAMENT_REWARDS
    CASE 
...
WITH tournament_rewards AS (
  SELECT 
    tr.user_id,
    tr.elo_points_earned as correct_elo_points,
    -- Calculate SPA based on position and rank
    CASE 
