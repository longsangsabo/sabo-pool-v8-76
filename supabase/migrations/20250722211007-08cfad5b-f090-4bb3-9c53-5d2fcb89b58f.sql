-- Phase 2: Create Missing Functions for Tournament Management
...
BEGIN
  ...
  IF v_final_match.winner_id IS NOT NULL THEN
    ...
    INSERT INTO tournament_results (
      ...
      p_tournament_id, 
      CASE WHEN v_final_match.player1_id = v_final_match.winner_id THEN v_final_match.player2_id ELSE v_final_match.player1_id END,
      2, 80, 30,
      (SELECT COUNT(*) FROM tournament_matches WHERE tournament_id = p_tournament_id AND (player1_id = CASE WHEN v_final_match.player1_id = v_final_match.winner_id THEN v_final_match.player2_id ELSE v_final_match.player1_id END OR player2_id = CASE WHEN v_final_match.player1_id = v_final_match.winner_id THEN v_final_match.player2_id ELSE v_final_match.player1_id END) AND status = 'completed'),
      (SELECT COUNT(*) FROM tournament_matches WHERE tournament_id = p_tournament_id AND winner_id = CASE WHEN v_final_match.player1_id = v_final_match.winner_id THEN v_final_match.player2_id ELSE v_final_match.player1_id END),
      (SELECT COUNT(*) FROM tournament_matches WHERE tournament_id = p_tournament_id AND (player1_id = CASE WHEN v_final_match.player1_id = v_final_match.winner_id THEN v_final_match.player2_id ELSE v_final_match.player1_id END OR player2_id = CASE WHEN v_final_match.player1_id = v_final_match.winner_id THEN v_final_match.player2_id ELSE v_final_match.player1_id END) AND status = 'completed' AND winner_id != CASE WHEN v_final_match.player1_id = v_final_match.winner_id THEN v_final_match.player2_id ELSE v_final_match.player1_id END),
      NOW()
    );
    v_results_created := v_results_created + 1;
  END IF;
  ...
  IF v_third_place_match.winner_id IS NOT NULL THEN
    ...
    INSERT INTO tournament_results (
      ...
      p_tournament_id,
      CASE WHEN v_third_place_match.player1_id = v_third_place_match.winner_id THEN v_third_place_match.player2_id ELSE v_third_place_match.player1_id END,
      4, 40, 10,
      (SELECT COUNT(*) FROM tournament_matches WHERE tournament_id = p_tournament_id AND (player1_id = CASE WHEN v_third_place_match.player1_id = v_third_place_match.winner_id THEN v_third_place_match.player2_id ELSE v_third_place_match.player1_id END OR player2_id = CASE WHEN v_third_place_match.player1_id = v_third_place_match.winner_id THEN v_third_place_match.player2_id ELSE v_third_place_match.player1_id END) AND status = 'completed'),
      (SELECT COUNT(*) FROM tournament_matches WHERE tournament_id = p_tournament_id AND winner_id = CASE WHEN v_third_place_match.player1_id = v_third_place_match.winner_id THEN v_third_place_match.player2_id ELSE v_third_place_match.player1_id END),
      (SELECT COUNT(*) FROM tournament_matches WHERE tournament_id = p_tournament_id AND (player1_id = CASE WHEN v_third_place_match.player1_id = v_third_place_match.winner_id THEN v_third_place_match.player2_id ELSE v_third_place_match.player1_id END OR player2_id = CASE WHEN v_third_place_match.player1_id = v_third_place_match.winner_id THEN v_third_place_match.player2_id ELSE v_third_place_match.player1_id END) AND status = 'completed' AND winner_id != CASE WHEN v_third_place_match.player1_id = v_third_place_match.winner_id THEN v_third_place_match.player2_id ELSE v_third_place_match.player1_id END),
      NOW()
    );
    v_results_created := v_results_created + 1;
  END IF;
