...
CREATE TABLE IF NOT EXISTS public.wallet_transactions (
  ...
  transaction_category TEXT CHECK (transaction_category IN ('deposit', 'withdrawal', 'match', 'referral_bonus', 'signup_bonus')),
  description TEXT,
  created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()
);
...
-- Create trigger for new user referral
...
-- Create trigger for rank verification
...
FOR SELECT 
USING (auth.uid() IN (SELECT user_id FROM public.wallets WHERE id = wallet_id));
