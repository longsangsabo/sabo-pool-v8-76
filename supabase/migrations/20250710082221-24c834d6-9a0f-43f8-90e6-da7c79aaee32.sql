-- Create a simpler manual trigger function
...
BEGIN
  ...
  v_multiplier := 1.0; -- Default multiplier
  ...
  -- Process participants in order: champion, runner-up, then by wins
  FOR v_participant IN 
    WITH player_stats AS (
      SELECT 
        ...
        CASE 
          WHEN tr.player_id = v_final_match_winner THEN 1
          WHEN tr.player_id = v_final_match_loser THEN 2
          ELSE 999
        END as priority_position
    ...
    SELECT 
      ...
      priority_position
    FROM player_stats
    ORDER BY priority_position ASC, wins DESC, total_matches DESC
  LOOP
    -- Assign position
    DECLARE
      v_position INTEGER;
    BEGIN
      ...
      RAISE NOTICE 'Player % (%) gets % SPA points for position % (wins: %)', 
        v_participant.full_name, v_participant.player_id, v_points, v_position, v_participant.wins;
...
SELECT public.process_tournament_results_manual('d2b48ded-2aa6-4939-841d-3f5425662eb7');