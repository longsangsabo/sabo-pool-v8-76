-- Simple fix for tournament_id field error
-- Fix the process_tournament_results function to use NEW.id instead of tournament_id
...
BEGIN
  ...
  IF NEW.status = 'completed' AND (OLD.status IS NULL OR OLD.status != 'completed') THEN
    ...
    -- FIXED: Use NEW.id instead of non-existent tournament_id field
    SELECT winner_id, 
