-- Complete missing database tables for comprehensive pool club management
...
-- 4. User check-in streaks and loyalty system
CREATE TABLE public.user_streaks (
  ...
  milestone_60_claimed BOOLEAN DEFAULT false,
  milestone_90_claimed BOOLEAN DEFAULT false,
  created_at TIMESTAMP WITH TIME ZONE DEFAULT now(),
  updated_at TIMESTAMP WITH TIME ZONE DEFAULT now()
);

-- 5. User penalties and sanctions
CREATE TABLE public.user_penalties (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id UUID NOT NULL REFERENCES auth.users(id) ON DELETE CASCADE,
  penalty_type TEXT NOT NULL CHECK (penalty_type IN ('warning', 'match_restriction', 'temporary_ban', 'permanent_ban', 'points_deduction')),
  ...
  status TEXT DEFAULT 'active' CHECK (status IN ('active', 'completed', 'appealed', 'overturned')),
...
-- RLS Policies for User Penalties
CREATE POLICY "Users can view their own penalties" ON public.user_penalties
  FOR SELECT USING (auth.uid() = user_id);

CREATE POLICY "System can create penalties" ON public.user_penalties
  FOR INSERT WITH CHECK (true);
