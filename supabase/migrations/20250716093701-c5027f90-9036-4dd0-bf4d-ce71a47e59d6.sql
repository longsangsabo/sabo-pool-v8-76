-- Fix universal tournament completion system - correct column names
...
BEGIN
  ...
  -- Build winners array (top 3) - using correct column names
  SELECT jsonb_agg(
    jsonb_build_object(
      'position', final_position,
      'player', jsonb_build_object(
        'id', user_id,
        'name', (SELECT COALESCE(p.full_name, p.display_name, 'Unknown Player') FROM profiles p WHERE p.user_id = tr.user_id),
        'avatar_url', (SELECT p.avatar_url FROM profiles p WHERE p.user_id = tr.user_id)
      ),
  ...
  AND final_position <= 3
  ORDER BY final_position;
  
  -- Build complete rankings - using correct column names
  SELECT jsonb_agg(
    jsonb_build_object(
      'position', final_position,
      'player', jsonb_build_object(
