-- Fix handle_rank_request_approval trigger function to use correct column names
...
BEGIN
  ...
  IF NEW.status = 'approved' AND (OLD.status IS NULL OR OLD.status != 'approved') THEN
    ...
    INSERT INTO public.rank_verifications (
      ...
      'approved',          -- Use 'status' instead of 'verification_status'
      NEW.notes,
      NOW(),
      NEW.club_id,
      NOW(),
    ...
    DO UPDATE SET
      ...
      status = EXCLUDED.status,  -- Use 'status' instead of 'verification_status'
