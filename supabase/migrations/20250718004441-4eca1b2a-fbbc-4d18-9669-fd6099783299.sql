...
-- STEP 4: Create new tournament_matches table with clean structure (including all existing values)
CREATE TABLE public.tournament_matches (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  tournament_id UUID NOT NULL REFERENCES public.tournaments(id) ON DELETE CASCADE,
  round_number INTEGER NOT NULL,
  match_number INTEGER NOT NULL,
  bracket_type TEXT DEFAULT 'main' CHECK (bracket_type IN ('main', 'winner', 'loser', 'semifinal', 'final', 'single', 'single_elimination')),
  ...
  -- Status and timing (including all existing status values)
