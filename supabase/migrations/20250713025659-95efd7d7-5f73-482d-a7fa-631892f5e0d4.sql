-- COMPLETE PLATFORM RANK SYNCHRONIZATION - AUTOMATION FIRST (FIXED)
  ...
  IF v_rank_changed OR v_elo_changed THEN
    BEGIN
      ...
      INSERT INTO public.player_rankings (
        user_id, elo_points, spa_points, verified_at, club_verified, created_at, updated_at
      ) VALUES (
        NEW.user_id, NEW.elo, COALESCE((SELECT spa_points FROM public.player_rankings WHERE user_id = NEW.user_id), 50), NOW(), true, NOW(), NOW()
      )
      ON CONFLICT (user_id) 
      DO UPDATE SET
...
BEGIN
  ...
  FOR v_user IN 
    ...
    WHERE p.verified_rank IS NOT NULL
      AND (
        pr.elo_points IS NULL 
        OR pr.elo_points != p.elo
      )
  LOOP
    -- Fix player_rankings table
    INSERT INTO public.player_rankings (
      user_id, elo_points, verified_at, club_verified, created_at, updated_at
    ) VALUES (
      v_user.user_id, v_user.profile_elo, NOW(), true, NOW(), NOW()
    )
    ON CONFLICT (user_id) 
    DO UPDATE SET
      elo_points = EXCLUDED.elo_points,
...
SELECT 
  'inconsistent_elo' as issue_type,
  COUNT(*) as count
FROM public.profiles p
JOIN public.player_rankings pr ON pr.user_id = p.user_id
WHERE p.elo != pr.elo_points
