-- Fix the complete_tournament_automatically function - remove the problematic DECLARE block
...
BEGIN
  ...
  -- Get third place match winner if exists (only if tournament has third place match)
  SELECT winner_id, 
         CASE 
  ...
  position_calculated AS (
    SELECT 
      ...
      CASE 
        ...
        WHEN v_third_place_winner_id IS NOT NULL AND pa.player_id = v_third_place_winner_id THEN 3
        
        -- Fourth place: Third place match loser (if exists)
        WHEN v_third_place_loser_id IS NOT NULL AND pa.player_id = v_third_place_loser_id THEN 4
        
        -- All other positions: calculated by wins, then losses, then highest round
        ELSE 
          CASE 
            WHEN v_third_place_winner_id IS NOT NULL THEN 4 
            ELSE 2 
          END + ROW_NUMBER() OVER (
            ORDER BY 
              pa.total_wins DESC, 
  ...
  SELECT 
    ...
    CASE pc.final_position
      ...
      WHEN 3 THEN 700   -- Third place SPA points
    ...
    CASE pc.final_position
      ...
      WHEN 3 THEN 30    -- Third place ELO points
  ...
  RETURN jsonb_build_object(
    ...
    'status', 'completed',
    'message', 'Tournament completed with all player positions calculated',
    'total_players', (SELECT COUNT(*) FROM position_calculated)
  );
