-- Enhanced Tournament Bracket Generation System (Fixed)
...
BEGIN
    ...
    FOR v_round IN 1..v_rounds LOOP
      FOR match_idx IN 1..(v_players_in_round/2) LOOP
        v_matches := v_matches || jsonb_build_object(
          ...
          'player1_seed', CASE WHEN v_round = 1 THEN (2 * match_idx - 1) ELSE NULL END,
          'player2_seed', CASE WHEN v_round = 1 THEN (2 * match_idx) ELSE NULL END,
          'status', 'scheduled'
        );
        v_current_match_id := v_current_match_id + 1;
      END LOOP;
  ...
  ELSIF v_tournament.tournament_type = 'round_robin' THEN
    ...
    FOR first_player IN 1..v_participant_count LOOP
      FOR second_player IN (first_player+1)..v_participant_count LOOP
        v_matches := v_matches || jsonb_build_object(
          'match_id', v_current_match_id,
          'round', 1,
          'match_number', v_current_match_id,
          'player1_seed', first_player,
          'player2_seed', second_player,
          'status', 'scheduled'
        );
        v_current_match_id := v_current_match_id + 1;
      END LOOP;
