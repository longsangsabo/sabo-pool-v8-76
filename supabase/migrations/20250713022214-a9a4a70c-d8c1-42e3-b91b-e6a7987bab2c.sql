...
BEGIN
  ...
  SELECT display_name INTO v_player_name FROM public.profiles WHERE id = NEW.user_id;
  ...
  IF NEW.status = 'approved' AND (OLD.status IS NULL OR OLD.status != 'approved') THEN
    BEGIN
      -- CRITICAL: Update profiles.verified_rank (not current_rank)
      UPDATE public.profiles 
      SET 
        verified_rank = v_rank_text,
        current_elo = NEW.requested_rank, -- Also update ELO
        updated_at = NOW()
...
BEGIN
  ...
  LOOP
    ...
    SET 
      verified_rank = v_rank_text,
      current_elo = v_request.requested_rank,
      updated_at = NOW()
...
SELECT 
  rr.user_id,
  rr.requested_rank as elo_requested,
  rr.status,
  p.verified_rank as profile_verified_rank,
  p.current_elo as profile_elo,
  rv.verified_rank as verification_verified_rank,
  pr.elo_points as rankings_elo,
  public.elo_to_sabo_rank(rr.requested_rank) as expected_rank
FROM public.rank_requests rr
LEFT JOIN public.profiles p ON p.id = rr.user_id
LEFT JOIN public.rank_verifications rv ON rv.user_id = rr.user_id AND rv.club_id = rr.club_id
LEFT JOIN public.player_rankings pr ON pr.user_id = rr.user_id
WHERE rr.status = 'approved'
ORDER BY rr.created_at DESC;

-- Check trigger is attached
SELECT schemaname, tablename, actiontiming, event_manipulation, action_statement 
FROM information_schema.triggers 
WHERE trigger_name = 'trigger_handle_rank_approval';
